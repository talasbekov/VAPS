...
//
// Экран не считает НИЧЕГО — включая «стандартную» восьмёрку: §22.17 говорит об
// этом прямо («даже значение 8 не должно вычисляться экраном аналитики»).
// Полосы распределения, средние по группам, покрытие и количество исправленных
// оценок приходят готовыми; подавленная группа приходит БЕЗ значения, а не со
// скрытым в разметке.
//
// Чего здесь нет по §22.16: отдельной оценки, оценщика, персонального
// комментария, внутреннего основания, количества низких оценок конкретного
// сотрудника, доли ручных оценок руководителя, истории individual score,
// таблицы лидеров, места сотрудника и места подразделения. Группы поэтому
// отсортированы сервером по подписи — сортировка по значению и есть место.
import { useRatingAnalytics } from '../api/queries'
import type { RatingGroupAggregate } from '../lib/analytics'
...
const UNPUBLISHED_LABEL: Record<string, string> = {
  FEATURE_DISABLED: 'Оперативный рейтинг выключен сервером — отчёт не формируется.',
...
export function RatingAnalyticsPage() {
  ...
  return (
    <div>
      <header className="mb-6">
        ...
        <h1 className="text-2xl font-bold tracking-tight">Аналитика рейтинга</h1>
        <span className="text-sm text-muted-foreground">
          Только агрегированные значения. Отдельные оценки, оценщики и комментарии в отчёт не
          входят и с сервера не запрашиваются.
        </span>
      </header>

      {query.isLoading && <p className="text-sm text-muted-foreground">Загрузка отчёта…</p>}
      ...
      {data !== undefined && figures !== null && (
        <>
          <section className="mb-4 rounded-xl border bg-card p-4" aria-label="Условия отчёта">
          ...
          >
            {[
              { label: 'С рассчитанным агрегатом', value: figures.ratedParticipants },
              {
              ...
              { label: 'Исправленных оценок', value: figures.correctedEvaluations },
            ].map((item) => (
              ...
              <tbody>
                {figures.groups.map((group) => (
                  <tr key={group.groupCode} className="border-t align-top">
                    <td className="p-3 text-sm font-medium">{group.safeLabel}</td>
                    <td className="p-3 text-sm tabular-nums">{groupValue(group)}</td>
          ...
          <section className="rounded-xl border bg-card p-4">
            <h2 className="mb-2 text-sm font-semibold">Что в отчёт не входит</h2>
