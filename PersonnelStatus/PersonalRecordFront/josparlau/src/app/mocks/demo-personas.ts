...
export const DEMO_PERSONAS: readonly DemoPersona[] = [
  {
    ...
    permissions: [
      ...
      'ops.feedback.create',
      'ops.feedback.view',
      // §19.22 «выставление оценки» — отдельный пункт списка прав, и теперь за
      // ним стоит операция (§19.7-19.14). Права смотреть агрегат
      // (`ops.rating.view_aggregate`) организатору НЕ выдано: оценщик обязан
      // оценить работу, не видя, как оценка сложится в рейтинг человека, —
      // иначе оценивание превращается в подгонку под число. На этом же
      // разделении держится демонстрация §19.14 «Сводка мероприятия
  ...
  },
  {
    ...
    permissions: [
      ...
      // §19.22: «просмотр агрегированного рейтинга» — свой пункт списка прав.
      // Аналитику выдан только он: sensitive-просмотр отдельных оценок и
      // оценщиков в этой сборке не реализован вовсе, а МЕТОДИКУ расчёта
      // (`ops.settings.manage_rating_policy`) ему не подчиняют — правка порогов
      // меняет значение рейтинга конкретных людей, а не способ его читать.
      'ops.rating.view_aggregate',
      // §19.22 «просмотр rating audit» — свой пункт. Аналитик здесь снова в
      // роли КОНТРОЛЁРА: видит действия с записями оценивания, но ни оценивать
      // (`ops.rating.evaluate`), ни исправлять (`ops.rating.correct`) не
      // вправе. Организатору ОМ журнал НЕ выдан — контроль над собственными
      // действиями не является их частью.
      'ops.rating.view_audit',
      // §19.29 «только при наличии соответствующего permission» — выгрузка
      // выдана аналитику: он читает агрегаты, и файл с той же сводкой не
      // расширяет его доступ. Организатору ОМ (`ops.rating.evaluate`) право НЕ
      // выдано — без persona БЕЗ него отказ на маршруте выгрузки был бы
      // недостижим: у администратора wildcard.
      'ops.rating.export',
      'ops.settings.view',
      'ops.settings.manage',
