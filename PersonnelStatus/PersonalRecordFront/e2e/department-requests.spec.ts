...
test.describe('заявки департаменту', () => {
  ...
  test('карточка заявки открывается на месте таблицы и несёт состав эталона', async ({
     ...
     */
    // 🔴 ПОЛЯ КВОТЫ ЖДУТ ОТДЕЛЬНОГО ЗАПРОСА (Plane №929).
    //
    // Карточка объясняла падение иначе: «проба берёт первую заявку стенда, а
    // у первых трёх `directorates` пуст — полей квоты и не могло быть».
    // ПРОВЕРЕНО, И ДИАГНОЗ НЕ ПОДТВЕРДИЛСЯ: строки управлений собираются не
    // из заявки, а из ДЕРЕВА ОРГСТРУКТУРЫ департамента
    // (`useDepartmentDirectorates` в `DepartmentRequestCard`), и поле квоты
    // рисуется у каждого управления департамента независимо от того,
    // разложена заявка или нет. Замер стенда: у всех десяти заявок
    // `directorates` пуст, а поля есть.
    //
    // Настоящая причина — ГОНКА: дерево приходит ОТДЕЛЬНЫМ запросом, а
    // проверка считала поля мгновенным `count()` сразу после открытия
    // карточки, то есть до ответа. Отсюда и «стабильно два прогона подряд»:
    // запрос дерева стабильно медленнее клика.
    //
    // Своя фикстура здесь НЕ заводится намеренно, хотя правило №822 требует
    // её для проб, ПРАВЯЩИХ состояние: эта только открывает карточку и
    // ничего не меняет, а лишнее ОМ на общем стенде стало бы чужой «первой
    // заявкой» — ровно тем, на что жалуется соседняя карточка.
    await signIn(page)
    await page.goto(`${APP}/employees?view=forces`)
    const tab = page.getByRole('tab', { name: 'Заявки', exact: true })
    await expect(tab).toBeVisible({ timeout: 30_000 })
    ...
    await expect(open, 'на стенде нет ни одной заявки — открывать нечего').toBeVisible({
      timeout: 20_000,
    })
    await open.click()
    ...
    // 🔴 ЖДЁМ, А НЕ СЧИТАЕМ СРАЗУ (Plane №929). Строки управлений приходят
    // ОТДЕЛЬНЫМ запросом — дерево оргструктуры департамента
    // (`useDepartmentDirectorates`), а не полем заявки. Мгновенный
    // `count()` читал пустой список ещё до ответа и давал ту же ошибку
    // «нет ни одного поля квоты», что и невезение со стендом, — то есть
    // два разных дефекта выглядели одинаково.
    const quotas = page.locator('input[id^="quota-"]')
    await expect(quotas.first(), 'нет ни одного поля квоты управления').toBeVisible({
      timeout: 20_000,
    })
