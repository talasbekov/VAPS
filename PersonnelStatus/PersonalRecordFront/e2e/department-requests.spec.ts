...
test.describe('заявки департаменту', () => {
  ...
  test('без права департамента вкладки «Заявки» нет вовсе', async ({ page }) => {
     ...
     *
     * 🔴 УЧЁТКА ВЫБРАНА НЕ ЛЮБАЯ. Первая версия ходила под `observer`, и
     * проба была ВАКУУМНОЙ: этому пользователю закрыт весь экран, вкладок у
     * него ноль вообще, и «нет вкладки Заявки» выполнялось само собой —
     * мутация «показывать вкладку без проверки права» её не краснила.
     * Нужен тот, кто экран ВИДИТ, но права департамента не имеет: `erda`.
     * Поэтому проба сначала убеждается, что соседние вкладки на месте.
     *
