...
test.describe(LIVE ? 'статусы: действия над действующим' : 'статусы (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('действующий статус продлевается и завершается досрочно, правки у него нет', async ({
    ...
    // 🔴 ПИН ПОДПИСИ ПОДНЯТ ОСОЗНАННО (Plane №366): «Командировка» → «В
    // командировке». Подпись статуса больше не берётся из таблицы тринадцати
    // кодов в коде клиента — она приходит из справочника, того самого, из
    // которого собран выпадающий список в этом же окне. До правки окно
    // ПРЕДЛАГАЛО «В командировке», а показывало сохранённое как
    // «Командировка»: два слова об одном статусе в одном диалоге.
    await expect(dialog.getByText('В командировке').first()).toBeVisible()
