...
test.describe(LIVE ? 'расстановка' : 'расстановка (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('дерево постов и назначение идут от живого расчёта', async ({ page, request }) => {
    ...
    await expect(
      page.getByText(/Выделено \d+ из потребности мероприятия \d+/)
    ).toBeVisible()
  ...
  test('отмена обоснования возвращает человека на прежний пост, а не теряет его', async ({
    ...
    try {
      ...
      const dialog = page.getByRole('dialog')
      await expect(dialog).toBeVisible({ timeout: 15_000 })
      // 🔴 ПОРОГ ВЫРОВНЕН С СОСЕДНЕЙ ПРОБОЙ — И ЭТО ВСЁ, ЧТО ОН ДАЁТ (Plane
      // №798, уточнено ревью 06.09.2026). Здесь стояло объяснение «окно
      // рисуется ПОСЛЕ ответа сервера, текст успевает прийти позже» — оно
      // НЕВЕРНО, и врущий комментарий хуже отсутствующего:
      //   • `ConflictDialog` возвращает `null`, пока конфликта нет
      //     (`features/ops-conflict-override/ui/ConflictDialog.tsx`), а
      //     заголовок собирается из `conflict.message` — окно ПОЯВЛЯЕТСЯ уже
      //     с текстом, состояния «видно, а текста нет» не бывает вовсе;
      //   • ожидание строкой выше (`toBeVisible`, 15 с) и так вбирает всю
      //     задержку ответа сервера, ради которой порог якобы ставился.
      //   • а главное — соседняя проба (`:1499`) 05.09.2026 упала на этом же
      //     тексте, УЖЕ ИМЕЯ пятнадцатисекундный порог. То есть «свой
      //     таймаут» её не спас, и общая причина не в пороге.
      // ЧТО ЗАМЕРЕНО: падение не воспроизводится — поодиночке с пятью
      // повторами 10 из 10, пачкой из карточки 39 из 39, и после правки снова
      // 39 из 39. Порог оставлен для единообразия двух одинаковых ассертов, а
      // причина мигания НЕ НАЙДЕНА; кандидаты вынесены карточкой.
  ...
  test('отклонённая правка роли возвращает человека с ПРЕЖНЕЙ ролью, а не с новой', async ({
    ...
    /**
     * ОТКЛОНЁННАЯ ПРАВКА НЕ ПРИМЕНЯЕТСЯ ВОВСЕ (Plane №703, с №762 — на сервере).
     *
     * Окно «Роль и секция…» меняет ПОСТ заодно с ролью. Пока перенос
     * выражался парой «снять + назначить», отклонённая правка применялась
     * НАПОЛОВИНУ: клиентский возврат из №744 собирался из ЦЕЛЕВЫХ значений и
     * ставил человека на прежний пост с НОВОЙ ролью. №703 научил возврат
     * помнить исток; №762 убрал сам возврат — перенос стал одной транзакцией,
     * и отказ не меняет на сервере ничего.
     *
     * Проба от этого не изменилась и проверяет ровно то же самое, что и
     * прежде: меняем РАЗОМ роль и пост, получаем отказ на набранном посту,
     * жмём «Отмена» — человек обязан стоять на ПРЕЖНЕМ посту с ПРЕЖНЕЙ ролью.
     * Второе условие здесь не украшение: без него проба свелась бы к №744.
     *
     * Красная проверка теперь СЕРВЕРНАЯ — снять `@transaction.atomic` с
     * `move_placement` и снимать назначение до проверки конфликтов: человек
     * останется нигде, и `rows.length === 1` не выполнится.
     */
    const token = await apiToken(STAND_USERNAME, STAND_PASSWORD)
    const auth = { Authorization: `Bearer ${token}`, 'content-type': 'application/json' }
    const roles = (await (
    ...
    try {
      ...
      // 🔴 ОКНО КОНФЛИКТА, А НЕ «ЛЮБОЕ ОКНО» (найдено ревью, Plane №798).
      //    `saveEdit` в `PlacementStage.tsx` зовёт `movePerson` через
      //    `mutateAsync`, и на 409 тот ОТКЛОНЯЕТСЯ — значит `setEditing(null)`
      //    не выполняется и окно «Роль и секция» остаётся на экране. Пока
      //    запрос в полёте, `getByRole('dialog')` указывает на НЕГО, а в
      //    момент прихода конфликта на экране два `role="dialog"` разом.
      //    Проба держалась на повторах Playwright и на порядке, в котором
      //    Radix убирает предыдущее окно из дерева доступности. Теперь окно
      //    выбирается по своему заголовку — «Конфликт: …».
      const conflict = page.getByRole('dialog', { name: /^Конфликт: / })
