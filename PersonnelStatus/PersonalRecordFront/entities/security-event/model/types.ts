...
export interface ForceRosterMember {
  ...
  /**
   * Кому штаб отдал человека (`[СБС-13]`); `null`/нет поля — ещё не роздан.
   *
   * 🔴 ПОЛЕ ОБЪЯВЛЕНО, ПОТОМУ ЧТО ЕГО ЧИТАЮТ (Plane №579). Сервер писал его в
   * состав с самого шага раздачи, а ни клиент, ни серверный гард расстановки
   * его не спрашивали — и беда, которую тот шаг объявлял починенной («у ОМ с
   * двумя объектами люди одного предлагались на посты другого»),
   * воспроизводилась ровно как раньше. Тип поля не объявлял вовсе, поэтому
   * `tsc` о пропуске молчал.
   */
  visitObjectId?: string | null;
}
...
export interface PlacementAssignment {
  ...
  /**
   * Когда человек ОТКРЫЛ свои назначения (`[ОЗН-02]`, Plane №452); `null` —
   * не открывал, ключа нет — строка заведена до №452 (читать как `null`).
   *
   * Четвёртое состояние строки, без которого старший не отличал «не видел» от
   * «видел и молчит»: первому надо напомнить, второму — звонить. Ставится
   * сервером ОДИН раз, при первом чтении человеком СВОЕГО списка, и только
   * пока ответа нет: после ответа факт открытия им же и поглощён.
   */
  viewedAt?: string | null;
  /** Телефон заступающего для звонка (`[ОЗН-03]`, Plane №452): служебный,
   *  иначе личный; пусто — нет ни одного. Считается сервером на чтении, в
   *  строке назначения не хранится: телефон меняется мимо мероприятия. */
  phone?: string;
  ...
  /**
   * Кто и КАК вписал отказ (Plane №588) — теми же двумя полями, что у
   * подтверждения. Отказ читается как слова самого сотрудника, а вписать их
   * может и старший (человек может позвонить): без этих полей чужая
   * формулировка выдавалась за его собственную.
   *
   * Пусто — у строк, заведённых до №588. Читать как «неизвестно», а не как
   * «сказал сам».
   */
  declinedVia?: "self" | "personal" | null;
  declinedBy?: string;
   ...
   * если предупреждение `OVER_NEED` было. Ключ необязателен: строки,
   * заведённые до №414, его не несут — читать как null. Своё поле, а не общее
   * с рейтингом: сводка расстановки считает обходы рейтинга отдельно, и
   * смешать два факта в одном числе значило бы соврать обоим. */
  needOverrideReason?: string | null;
...
export interface ApprovalRemark {
   ...
   * (Plane №510). Пусто — привязки не теряли. Снятие поста обнуляет `postId`,
   * иначе замечание держало бы согласование, ссылаясь на несуществующий
   * пост; без этого поля оно стало бы неотличимо от изначально общего, и
   * согласующий потерял бы, о чём писал. */
  detachedPost?: string;
...
export interface AcknowledgementNotifyReport {
  ...
  /** Кому НЕ слали НАМЕРЕННО: сотрудник уволен (Plane №900). Своё поле, а не
   *  строка в `unlinkedEmployeeIds`: то поле зовёт кадровика заводить учётку,
   *  а здесь чинить нечего — человека в наряде уже нет. Не необязательное:
   *  обе ручки напоминания идут через один `_send` и шлют его всегда. */
  dismissedEmployeeIds: string[];
