...
export const EVENT_PARTICIPATION_STATUS_CODES: ReadonlySet<string> = new Set([
  ...
  // 🔴 `IN_EVENT` ДОБАВЛЕН (Plane №378, ручное тестирование №377).
  //
  // Это тип «Участие в ОМ» — тот самый, о котором заказчик писал в №365
  // («Участие на ОМ должно быть как статус На дежурстве, должны выбираться
  // группы и физнаряд»). Пока его тут не было, случалось ровно то, от чего
  // список и завели: окно простановки не показывало ему ни мероприятия, ни
  // группы, а разрезы сбора сил считали такого человека В СТРОЮ — то есть
  // свободным, и предлагали на новое привлечение. Ошибка та же, что ловили
  // в Ш-5 для боевых групп, только с третьим кодом.
  "IN_EVENT",
]);
