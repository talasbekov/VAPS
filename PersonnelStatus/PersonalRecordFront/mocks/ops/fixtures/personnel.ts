...
/** Телефон демо-сотрудников для ☎ в строке ознакомления (`[ОЗН-03]`, Plane
 * №452).
 *
 * На сервере номер берётся из кадровой записи (служебный, иначе личный); у
 * мока кадровой записи нет, поэтому он задан таблицей — тем же приёмом, что
 * статус дня и рейтинг выше. Заполнены НЕ все намеренно: в живой базе телефон
 * есть не у каждого, и строка без номера — обычное состояние, которое экран
 * обязан переживать (у неё просто нет ☎), а не редкость, о которую он
 * споткнётся на стенде.
 */
export const PERSONNEL_PHONE: Record<string, string> = {
  "emp-1": "+7 701 000-10-01",
  "emp-2": "+7 701 000-10-02",
  "emp-3": "+7 701 000-10-03",
  "emp-6": "+7 701 000-10-06",
  "emp-9": "+7 701 000-10-09",
};

export function personnelPhone(id: string): string {
  return PERSONNEL_PHONE[id] ?? "";
}
