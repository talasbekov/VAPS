...
export function EditBulletinDialog({
  ...
  //
  // 🔴 ЗАВИСИМОСТЬ — ЗНАЧЕНИЯ СЕРВЕРА, А НЕ ОБЪЕКТ МЕРОПРИЯТИЯ (Plane №925,
  // тем же приёмом, что №555). Эффект стоял на `[open, event, reset]`, то
  // есть на ИДЕНТИЧНОСТИ объекта, а `event` — строка выдачи реестра, объект
  // из кэша React Query: любой рефетч ключа реестра меняет ссылку и
  // сбрасывает набранное на серверное молча.
  //
  // Сегодня не стреляет по СТЕЧЕНИЮ ТРЁХ ОБСТОЯТЕЛЬСТВ, ни одно из которых
  // не заявлено как гарантия: окно модальное, фоновый рефетч по фокусу
  // выключен, WS-инвалидация сужена до рейтингов и уведомлений. Вернётся при
  // первой же немодальной кнопке рядом или при расширении инвалидации на
  // реестр. Форма бюллетеня шире и дороже, чем «Ответ департамента», где тот
  // же класс уже стоил потери набранного.
  //
  // Подпись собирается ИЗ ТЕХ ЖЕ значений, что кладутся в форму: сравнивать
  // весь объект строкой значило бы реагировать на поля, которых форма не
  // касается (готовность, стадия, счётчики) — то есть вернуть прежнюю
  // болезнь под другим именем.
  const formPrint = JSON.stringify([
    valuesOf(event),
    detailsOf(event.protectedPersons),
  ]);
  useEffect(() => {
