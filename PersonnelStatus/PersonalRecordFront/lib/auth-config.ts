...
export const authOptions: NextAuthOptions = {
  ...
  session: {
    ...
    //
    // ЧТО ЭТО НЕ ЛЕЧИТ. Refresh-токен умирает через семь суток ПОСЛЕ ВХОДА
    // (`ROTATE_REFRESH_TOKENS` выключен, значит он не обновляется), поэтому
    // человек, работающий не прерываясь, на седьмые сутки получит
    // принудительный выход посреди работы. Скользящая cookie этого не
    // отменяет. Предупреждать ли заранее — вопрос к заказчику, заведён
    // карточкой; здесь важно, что факт записан, а не выглядит недосмотром.
