...
check "слушающий сервер без метки — «владелец не назвался»" "1" \
  "$(printf '%s\n' "$REPORT" | grep -c 'слушает :3106 — владелец не назвался, сперва спросите')"
check "сироты помечены «её и гасите»" "2" \
  "$(printf '%s\n' "$REPORT" | grep -c 'ПОРТА НЕТ')"

# ── 3a. Метка принадлежности: кто поднял стенд (Plane №832) ─────────────
OWNER_DIR=$(mktemp -d)
STAND_OWNER_DIR="$OWNER_DIR"

check "метки нет — справка пуста" "" "$(stand_owner_note 3106)"

STAND_OWNER="сессия-проба" stand_owner_write 3106 dev $$
check "метка записана файлом по ПОРТУ" "1" \
  "$(ls "$OWNER_DIR" | grep -c '^next-stand-3106.owner$')"
check "живая метка называет владельца и вид" "1" \
  "$(stand_owner_note 3106 | grep -c '^поднял сессия-проба, dev, ')"
check "в списке соседей владелец назван" "1" \
  "$(neighbours_report | grep -c 'слушает :3106 — поднял сессия-проба, dev, ')"

# 🔴 Метка МЁРТВОГО стенда не выбрасывается молча: файл в /tmp переживает свой
# сервер, и «поднял такой-то» про несуществующий процесс увело бы разбор к
# живому человеку вместо сироты. pid 999999 заведомо свободен.
STAND_OWNER="сессия-мертвец" stand_owner_write 3199 prod 999999
check "протухшая метка названа протухшей" "1" \
  "$(stand_owner_note 3199 | grep -c '^метка протухла (в ней pid=999999')"

stand_owner_forget 3106
check "метка снимается" "" "$(stand_owner_note 3106)"
rm -rf "$OWNER_DIR"
STAND_OWNER_DIR=/tmp
