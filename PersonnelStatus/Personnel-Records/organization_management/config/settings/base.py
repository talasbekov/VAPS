...
# Dev-only fallback; production.py hard-fails if this default reaches prod.
SECRET_KEY = os.getenv(
    "DJANGO_SECRET_KEY",
    "django-insecure-^18w8^kyktt4q14w%c4tci%w(8po97jj2pd&3(#hv(dyn3hznv",
)
...
REST_FRAMEWORK = {
    ...
    # Ограничение частоты — ТОЧЕЧНОЕ, по именованным областям, а не общее по
    # API: общая ставка на весь бэкенд задела бы реестры и сводки, которые
    # экраны тянут пачками. Пока область одна — смена своего пароля: ручка
    # честно отвечает, подошёл ли текущий пароль, и без ограничения работает
    # оракулом для перебора из угнанной сессии (см. api/self_account.py).
    'DEFAULT_THROTTLE_RATES': {
        'change-password': '10/hour',
    },
...
SIMPLE_JWT = {
    ...
    # 🔴 РОТАЦИЯ ВКЛЮЧЕНА (Plane №787, решение заказчика 06.09.2026).
    #
    # Что было. Сессия портала скользящая — cookie перевыпускается при каждом
    # чтении, — а refresh-токен жил семь суток ОТ ВХОДА и не обновлялся
    # никогда. Человек, работающий не прерываясь дольше недели, на седьмые
    # сутки получал отказ продления ПОСРЕДИ работы: его уводило на форму
    # входа, несохранённый экран терялся. Симптом при этом неотличим от
    # поломки — бэкенд здоров, права на месте, а из системы выкинуло.
    #
    # Что стало. Каждое удачное продление выдаёт НОВЫЙ refresh с новым
...
# SECURITY: `X-Forwarded-For` присылает КЛИЕНТ, и верить ему можно только
# тогда, когда запрос пришёл от собственного прокси (Plane №699). Список
# пуст по умолчанию: без прокси перед Django единственный достоверный адрес —
# `REMOTE_ADDR`, а заголовок не значит ничего. Разворачиваете за nginx или
# балансировщиком — перечислите АДРЕСА ЭТИХ ПРОКСИ, а не сети клиентов:
# доверие к чужому адресу возвращает подделку, ради которой настройка и
# заведена. Значение попадает в реквизиты подписи согласующего и в
# неизменяемый аудит, поэтому цена ошибки здесь — недоказуемая подпись.
TRUSTED_PROXY_IPS = tuple(
    h.strip() for h in os.environ.get('TRUSTED_PROXY_IPS', '').split(',') if h.strip()
)
