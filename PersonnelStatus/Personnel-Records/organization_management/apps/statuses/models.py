...
class EmployeeStatus(models.Model):
    ...
    # 🔴 БЕЗ `choices` (Plane №354). Тринадцать значений `StatusType` остаются
    # как ИМЕНА для кода — на них ссылаются сервисы, сигналы и пины тестов, —
    # но списком допустимого они больше не являются: словарь типов живёт в
    # справочнике `ops_status_types`, который заказчик правит в админке.
    # Пока здесь стояли `choices`, заведённый им тип нельзя было СОХРАНИТЬ, и
    # показать его в окне планирования значило бы предложить выбор, ломающийся
    # при нажатии «Сохранить». Что именно сервер принимает — решает
    # `statuses.catalog.known_codes()`, и решает по данным, а не по коду.
    #
    # Длина 100, а не 20: коды справочника раздела длиннее старых
    # (`EVENT_ASSIGNMENT_GROUP` — 22 символа), и тип, заведённый в админке,
    # молча не влезал бы в поле.
    ...
    def get_status_type_display(self) -> str:
        """Подпись типа статуса — ИЗ СПРАВОЧНИКА (Plane №354).

        🔴 ЭТОТ МЕТОД ОБЯЗАН ЗДЕСЬ БЫТЬ. Раньше его порождал Django из
        `choices`; сняв их, мы убили метод — а зовут его ВОСЕМНАДЦАТЬ мест:
        админка, семь уведомлений в `tasks.py`, сигнал истории изменений,
        сообщение о конфликте статусов, сервис создания и две команды. Первая
        же проба на сохранение упала пятисоткой в сигнале истории: статус
        создавался и тут же ронял запрос.

        Определён на модели, а не в каждом читателе: восемнадцать правок ради
        одной подписи разошлись бы при первой же следующей.
        """
        # Импорт внутри метода: `catalog` ходит в приложение `operations`, и
        # импорт на уровне модуля замкнул бы кольцо приложений.
        from organization_management.apps.statuses import catalog

        return catalog.label_for(self.status_type)
    ...
    def clean(self):
        ...
        if self.employee_id and self.start_date:
            ...
            for other_status in overlapping:
                # ИСКЛЮЧЕНИЕ: «В строю» не конфликтует ни с чем — это фоновое
                # состояние, а не утверждение о занятости, и оно уступает
                # любому реальному статусу (при активации того «В строю»
                # закрывается автоматически).
                #
                # Исключение СИММЕТРИЧНО, и это не косметика. Раньше оно
                # действовало только когда «В строю» на стороне ДРУГОГО
                # статуса, а проверяемый — запланированный. Из-за этого пара
                # «активное В строю без даты конца» + «запланированный отпуск»
                # заводилась, но потом делала невозможным ЛЮБОЕ изменение: при
                # пересохранении самой строки «В строю» (её закрывает смена
                # статуса) она сравнивалась с плановым отпуском уже с другой
                # стороны, исключение не срабатывало, и открытый вправо период
                # «В строю» пересекался с чем угодно. Отказ при этом называл
                # отпуск — будто мешает он, хотя новый статус его не касался.
                #
                # Два «В строю» подряд — по-прежнему конфликт: фоновых
                # состояний не бывает два.
                self_in_service = self.status_type == self.StatusType.IN_SERVICE
                other_in_service = other_status.status_type == self.StatusType.IN_SERVICE
