...
def _winners_from_snapshot(snapshot, business_date, catalog):
    ...
    СПРАВОЧНИК БЕРЁТСЯ ИЗ СНИМКА, а `catalog` — только запасной для старых
    версий схемы. Победителя выбирает ПРИОРИТЕТ, и читать подписанный день
    сегодняшними приоритетами значит переписывать заявление задним числом.

    Пока здесь стоял живой справочник, светофор МОЛЧАЛ на настоящем
    расхождении: сдали «на дежурстве», потом администратор поднял учёбу выше
    дежурства, а дежурство отменили — снимок, пересчитанный новыми
    приоритетами, давал «учёба», живой день давал «учёба», и цвет выходил
    зелёный. Документ на руках при этом говорил «дежурство».

    Довод «одна мерка на обе стороны, иначе покажем разницу справочника вместо
    разницы обстановки» (срез 135) оказался неверен и снят: если подписанный
    день и сегодняшний экран называют человека по-разному, это и есть
    расхождение — тем более важное, что обнаружит его тот, кто положит их
    рядом.
    """
