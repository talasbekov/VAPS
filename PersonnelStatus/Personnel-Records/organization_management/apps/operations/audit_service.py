...
# Уборка участий, переживших своё мероприятие (Plane №356). Пишется по тому же
# основанию, что и удаление ОМ: строки исчезают ЦЕЛИКОМ, и журнал остаётся
# единственным следом того, что они были.
#
# 🔴 ПОЧЕМУ ЭТО ОТДЕЛЬНАЯ КАРТОЧКА, А НЕ «ЗАОДНО». 31.08.2026 со стенда
# пропали 1135 участий-сирот, и установить, какой прогон их снял, оказалось
# НЕЧЕМ: уборка не писала в журнал вовсе, а звалась из teardown смоука шеллом,
# а не через HTTP, поэтому следа не осталось и в логе Django. Две сессии
# восстанавливали картину сопоставлением своих выводов и точного числа назвать
# не смогли. Сегодня это стенд и мусор, цена нулевая; команда та же и на
# боевой базе, и второй раз вопрос «кто снёс тысячу строк» будет задан уже не
# про мусор.
STATUS_PARTICIPATIONS_PURGED = "STATUS_PARTICIPATIONS_PURGED"
...
def _build(
    *,
    ...
    🔴 ОБЪЕКТ УЧЁТКИ ПРИВОДИТСЯ К ЕЁ ИДЕНТИФИКАТОРУ (Plane №895). Поле
    называется `actor_user_id`, но писалось в него `str(actor)`, а для
    `actor=request.user` это USERNAME. Таких вызовов в `ops/api/views.py`
    десять (против 35 с `resolve_actor_id`), и журнал по одним и тем же
    действиям хранил то идентификатор, то логин. Ошибки при этом нет —
    поле заполнено, — поэтому расхождение не всплывало: выборка по
    `actor_user_id` человека молча находила половину его действий, а
    вторая половина выглядела чужой.

    Чинится ЗДЕСЬ, а не в десяти вызовах, — ровно тем же доводом, которым
    это сделано у соседа по той же беде (`security_events.actor_display_name`,
    Plane №484): одна ветка `isinstance` дешевле дюжины правок и не
    оставляет одиннадцатому вызову шанса разойтись снова. Правка вызовов
    закрыла бы сегодняшние десять и ничего не сказала бы про завтрашний.

    Приводится ТОЛЬКО объект учётки. Строковые акторы остаются как есть:
    `system:dismissal`, `seed_access_matrix` и прочие — законные значения
    этого поля, и «в журнале всегда цифры» было бы неправдой о системе.
    """
    from django.contrib.auth import get_user_model

    # `get_user_model()`, а не прямой импорт: от этого имени зависит опознание
    # типа, и при переопределённом `AUTH_USER_MODEL` прямой импорт тихо
    # перестал бы срабатывать — снова записывая логины (тот же довод стоит у
    # `actor_display_name`).
