...
class DefaultPagination(LimitOffsetPagination):
    ...
    default_limit = 50
    # ПОТОЛОК СТРАНИЦЫ. Без него `?limit=1000000` отдаёт таблицу целиком одним
    # ответом: у LimitOffsetPagination верхней границы нет по умолчанию, и
    # размер страницы назначает спросивший.
    #
    # Опаснее всего это на журнале: он append-only и растёт без предела, то
    # есть «отдай всё» дорожает с каждым днём и сорвётся уже на живой
    # эксплуатации, когда строк накопится достаточно. Списки статусов и
    # уведомлений растут медленнее, но потолок один на раздел — второй,
    # «особенный» для журнала, разъехался бы с первым при первой же правке.
    #
    # 1000 — та же мерка, что у MAX_BULK_ROWS: сколько строк раздел готов
    # обработать в ОДНОМ обращении. Меньше ставить нельзя не подумав: выгрузки
    # и сверки берут страницы крупнее экранных.
    max_limit = 1000
