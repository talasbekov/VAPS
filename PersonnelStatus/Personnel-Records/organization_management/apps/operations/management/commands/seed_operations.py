...
ROLE_PERMISSIONS = {
    ...
    "HEAD_OPS_UNIT": [
        ...
        "placement.manage",
        # 🔴 `forces.command` — ПО СПЕЦИФИКАЦИИ (Plane №944, задача заказчика
        # 07.09.2026 «привести сбор сил к документации»). Раздел 7.1
        # `RAW/README.md` называет штабом ОБЕ персоны второго департамента
        # (`acc_dir_head_d2`, `acc_dept_head_d2`) и отдаёт им список заявок,
        # деление потребности, довыделение и распределение по объектам.
        # Матрица №348 («Сбор сил недоступен») этому противоречила; вопрос
        # №421 три дня ждал ответа, и ответом стала карточка №944: заказчик
        # проверил цепочку под своими учётками и назвал документацию
        # источником правды. Право область не спрашивает (штаб делит по всем
        # департаментам), поэтому лежит в профиле, а не в роли-добавке.
        "forces.command",
