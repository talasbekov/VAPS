...
СТАДИИ НАБИРАЮТСЯ ШТАТНЫМИ СЕРВИСАМИ, а не записью `stage="FORCES"` в базу.
...
DEMAND_NEEDS = (4, 7)
EXPECTED_REQUESTED = sorted(DEMAND_NEEDS)


class Command(BaseCommand):
    help = (
        "Завести на стенде фикстуры под сторожей смоука: привлечённых на ОМ на "
        "сегодня и мероприятие на стадии «Запрос сил». Идемпотентна."
    )
    ...
    # ── Мероприятие на стадии «Запрос сил» ──────────────────────────────────
    ...
    def _forces_event(self, day):
        ...
        # реестре нельзя: две строки на «Запросе сил» дают на экране два
    ...
    def _build(self, day):
        """Штатная цепочка: заведение → расчёт постов → потребность."""
        ...
        # Две группы, чтобы заявок было две: проба недобора называет строку
        # департамента поимённо, и на одной группе «поимённо» неотличимо от
        # «суммой».
        event = event_service.approve_demand(
            event.id,
            rows=[
                {
                    "id": "seed-demand-1",
