        ...
        assert build_division_snapshot(division.id, TODAY)["rows"] == [
            {
                ...
                "source": "USER",
                # Пин правлен ОСОЗНАННО вместе с версией 8 (Plane №751): вид
                # участия заморожен в снимке, потому что после слияния №486
                # разбивка «На ОМ (гр./нар.)» выводится только из него. У
                # статуса без участий список пуст — и кладётся всегда, чтобы
                # читатель отличал новый снимок от старого по наличию ключа.
                "participations": [],
            }
        ]
