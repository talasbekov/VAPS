...
def catalog():
    """Справочник нужен КАЖДОМУ тесту файла, хотя проверяет его не каждый.

    Со схемы снимка 3 билдер отказывается собирать день, справочник которого
    непригоден: замороженный каталог не чинится задним числом. Мир без единого
    типа статуса — не «минимальная фикстура», а состояние, в котором прод не
    бывает: seed_status_types заводит их до всего остального. Точечные вызовы
    seed_types() в отдельных тестах остались — get_or_create идемпотентен, и
    они показывают, где справочник существен ПО СМЫСЛУ.
    """
    seed_types()
