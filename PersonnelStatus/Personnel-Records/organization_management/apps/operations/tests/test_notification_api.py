...
def test_schema_describes_a_page_and_only_get():
    ...
    Владелец страничной обёртки здесь ОДИН — `pagination_class`: из него
    spectacular выводит и компонент Paginated…List, и параметры limit/offset.
    Ручной `paginated_response_schema()` соседних списков был бы вторым
    владельцем того же самого (проба: снятие любого из двух схему не меняет —
    значит ни один не удержан), поэтому у ленты его нет.
