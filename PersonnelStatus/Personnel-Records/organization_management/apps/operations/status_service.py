...
def restricted_employee_ids(employee_ids):
    """Из данных сотрудников — те, кто закрыт для правки статусов.

    ЕДИНЫЙ предикат гварда откомандированного для ОБОИХ путей: одиночного
    (assert_employee_status_editable ниже) и массового (bulk_status_service).
    Держать его в одном месте обязательно — раздвоение уже давало обход: пачка
    знала код DETACHED наизусть и не исключала ногу возврата, пока одиночный
    путь читал справочник и снимал ограничение по подтверждённому возврату.

    Блокирует ЖИВОЙ статус ОГРАНИЧИВАЮЩЕГО типа — справочник (restricts_editing),
    а не литерал: пометь администратор ограничивающим ещё один тип, экран
    показал бы замок, и бэк обязан его исполнить обоими путями. Прямой запрос
    существования (не «победитель дня»: статус с большим приоритетом замаскировал
    бы живой ограничивающий). Пустой список ограничивающих — законное состояние
    (никто не закрыт), и запрос с пустым `__in` его и означает.

    ЗА ВЫЧЕТОМ ноги с подтверждённым возвратом: ограничение снимает РЕШЕНИЕ, а
    не календарь — возврат действует со следующего дня, но правку разрешает уже
    сегодня, иначе спор двух подразделений считался бы неоконченным до полуночи.
    """
...
def assert_employee_status_editable(employee_id):
    ...
    Тонкая обёртка над общим предикатом restricted_employee_ids: пока у
    сотрудника есть живой ограничивающий статус (без подтверждённого возврата),
    любая правка его статусов — 403. Ограничение следует за СОТРУДНИКОМ, а не за
    областью правящего, поэтому действует и в штатном, и в принимающем
    подразделении. Пути возврата из прикомандирования и увольнения этот гард
    звать НЕ должны — они законно закрывают ограничивающий статус.
    """
    if restricted_employee_ids([employee_id]):
        raise DomainError(
            "PERMISSION_DENIED",
            403,
...
def cancel_status(status, *, actor, reason, amend=True):
    """Отменить не начавшийся статус или начавшийся СЕГОДНЯ — факты отмены
    append-once.

    Отменяется PLANNED и строка, начатая сегодня (Plane №322): последняя ещё не
    продержалась ни одного дня и фактом стать не успела, а закрыть её нечем —
    досрочное завершение требует дату позже начала и не в будущем. Идущий с
    прошлых суток статус — факт, который случился (его закрывают раньше срока,
    а не отменяют). Отменённую
    ...
    today = Clock.today_local()
    state = locked.state_on(today)
    # Отменяется НЕ НАЧАВШИЙСЯ статус — и начавшийся СЕГОДНЯ (Plane №322).
    #
    # Второе не послабление доктрины, а её же следствие. Доктрина говорит:
    # идущий статус — факт, который случился, его закрывают раньше срока, а не
    # отменяют. Но статус, заведённый сегодня, ещё НЕ ПРОДЕРЖАЛСЯ НИ ОДНОГО
    # ДНЯ, и фактом стать не успел; закрыть его нечем — `complete` требует дату
    # позже начала («Дата завершения должна быть позже даты начала») и при этом
    # не в будущем, а вместе это невозможное условие в день заведения.
    #
    # До этой правки оператор, поставивший статус по ошибке, не мог убрать
    # СВОЮ ЖЕ запись до следующих суток: ни отмены, ни завершения. Ошибочная
    # строка при этом успевала попасть в расход дня и в сводки.
    #
    # Правило взято не с потолка: кадровый каталог решает ровно этот случай
    # так же и объясняет теми же словами — «Такой статус не продержался ни
    # одного дня — он отменён, а не завершён» (`_close_active_statuses`), и то
    # же поведение закреплено пробой возврата из прикомандирования.
    #
    # Сданный день это НЕ ломает: ниже отмена по-прежнему проходит через
    # `enforce_amendment_on_retro_edit`, и правка сданного дня остаётся
    # поправкой с причиной, а не тихим переписыванием.
    started_today = (
        state == OpsEmployeeStatus.LifecycleState.ACTIVE
        and locked.date_start == today
    )
    if state != OpsEmployeeStatus.LifecycleState.PLANNED and not started_today:
        raise DomainError(
            "INVALID_LIFECYCLE_TRANSITION",
            422,
            detail={"state": str(state)},
            message=(
                "Отменить можно не начавшийся статус или начавшийся сегодня; "
                "идущий с прошлых суток закрывают досрочным завершением."
            ),
        )
    before = audit_service.status_snapshot(locked)
