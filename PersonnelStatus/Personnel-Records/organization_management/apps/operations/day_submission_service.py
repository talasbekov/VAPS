...
def amend_day(
    *,
    ...
    # ЗАМОК ДНЯ, А НЕ ГОЛОВЫ ЦЕПОЧКИ. Раньше здесь блокировалась голова, и это
    # не давало взаимного исключения: голова ДВИЖЕТСЯ, поправка добавляет строку
    # выше, и тот, кто залочил версию 2, не мешает соседу, залочившему версию 3.
    # Хуже того, номер считался от строки, прочитанной ДО ожидания на замке:
    # дождавшись своей очереди, вторая поправка по-прежнему видела старую голову
    # и просила уже занятый номер — то есть получала отказ, ничего неправильного
    # не сделав. Найдено пробой на настоящих потоках (срез 122).
    #
    # Замком служит строка ПЕРВОЙ версии: она есть у всякого поправимого дня и
    # не движется.
    if DailySubmissionSelector.lock_day(division_id, business_date) is None:
        raise DomainError(
    ...
    # Голова читается СВЕЖИМ запросом уже ПОД замком: свой снимок этого
    # оператора видит всё, что закоммитил предшественник, — в том числе версию,
    # которой не было, когда мы становились в очередь.
    latest = DailySubmissionSelector.latest_for(division_id, business_date)
    if latest is None:
        raise DomainError(
            "NO_SUBMISSION_TO_AMEND",
            422,
