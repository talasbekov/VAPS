...
UNAVAILABLE_FORMATS = [
    ...
    },
    {
        ...
        "label": "DOCX",
        "reason": (
            "Генератор DOCX в проекте ЕСТЬ и работает — им собираются "
            "документы по мероприятию в секции ниже (Plane №156). Но служебный "
            "отчёт за период это другой артефакт: у него нет бланка-образца, с "
            "которого снята бы вёрстка, а собирать его «как-нибудь» значило бы "
            "выдать за документ заказчика то, чего он не присылал."
        ),
    },
]
...
def read_source_rows():
    ...
    for shift in OpsDutyShift.objects.all():
        ...
        rows.append({
            ...
            # 🔴 ЭТО НЕ ЧЕТВЁРТАЯ КОПИЯ `post_label`, И РАЗЛИЧИЕ НАЗВАНО
            # ВСЛУХ (Plane №875). Склейка похожа, но правило другое, и свести
            # их значило бы получить неверный ответ:
            #   • данные другие — здесь `sectorName`/`postName` из ПРИВЯЗКИ
            #     смены к паспорту, а не поля поста расчёта; идентификатора
            #     поста в этой структуре нет вовсе;
            #   • пустота значит другое — «смена не привязана к посту», и
            #     ответ на это `None`, который читатель отчёта печатает как
            #     отсутствие привязки. `post_label` в том же случае вернул бы
            #     идентификатор (для поста расчёта это честнее пустоты), и
            #     здесь он был бы выдумкой: подставлять нечего.
            "postLabel": (
