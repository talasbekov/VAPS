...
def test_the_named_blank_slots_really_stay_empty(staff):
    """Названные пустыми места бланка действительно пусты (Plane №906).

    🔴 ЗАЧЕМ. Из 101 места шаблона два не производит ни одна ветка сборки:
    вторая строка под «Вооружения» и вторая строка третьей графы первого дня.
    Данные не теряются — значение целиком уходит в соседнее место `_1`, а `_2`
    это вторая строка бланка под переполнение длинного текста. Но пустое место
    в документе неотличимо от потерянного, и следующий читатель кода идёт
    искать пропажу. Теперь список назван вслух (`INTENTIONALLY_BLANK_SLOTS`),
    а проба держит его честным.

    🔴 ГРАНИЦА ЭТОЙ ПРОБЫ, НАЗВАННАЯ ЧЕСТНО. Она проверяет ОДНО направление:
    названные места пусты и содержательное значение стоит в соседнем `_1`.
    Обратное — «не появилось ли ТРЕТЬЕ незаполняемое место» — здесь не
    проверяется: чтобы отличить «код не заполняет никогда» от «данных не
    хватило», нужна сводка, заполненная целиком по всем разделам, дням, лицам
    и группам. Первая версия пробы пыталась вывести это разбором исходника, и
    отбор оказался ЛОЖНО ШИРОКИМ — цикл `values[f"{prefix}_{slot}"]` покрывал
    собой любое имя. Оставлять такую проверку хуже, чем не иметь её: она
    зеленела бы на том самом случае, ради которого заведена.
    """
    from organization_management.apps.ops import documents_summary

    event = make_event("ОМ-Т-59")
    staff.patch(
        f"{GVO_URL}ОМ-Т-59/",
        {
            "section": "org",
            "values": {"weapons": "Пистолет Макарова, 2 ед."},
        },
        format="json",
    )

    values = documents_summary.fill_all_keys(
        documents_summary.template_keys(documents_summary.summary_template_path()),
        documents_summary.document_values(event),
    )

    for slot in documents_summary.INTENTIONALLY_BLANK_SLOTS:
        assert slot in values, f"место {slot} исчезло из шаблона — список устарел"
        assert values[slot] == "", (
            f"место {slot} названо намеренно пустым, но заполнено: {values[slot]!r}"
        )
    # И содержательное значение стоит там, где ему положено: в первой строке.
    assert values["armament_1"] == "Пистолет Макарова, 2 ед.", (
        "значение «Вооружения» не дошло до первой строки бланка"
    )
