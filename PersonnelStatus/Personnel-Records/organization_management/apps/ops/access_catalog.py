...
def _rows():
    ...
    for path, entry in _walk(get_resolver().url_patterns):
        ...
        # 🔴 ТРЕТИЙ СПОСОБ ГЕЙТА — В СЕРВИСЕ (Plane №901). Часть вьюсетов
        # НАМЕРЕННО живёт без `RequirePermissionMixin`: §19.27 требует писать
        # в журнал оценивания и ЗАПРЕЩЁННЫЕ попытки, а гейт миксина ответил бы
        # раньше, чем сервис успел записать отказ. Право там проверяет сервис
        # первым действием — и каталог этих ручек не видел вовсе: экран «Права»
        # отвечал «право не стоит ни на одной ручке» про `rating.evaluate`,
        # которым закрыта вся работа оценщика.
        #
        # Карта ОПИСАТЕЛЬНАЯ, а не гейтящая, и названа так, чтобы это было
        # видно: `permission_map` у класса без миксина создала бы ложное
        # впечатление, что гейт есть.
        service_map = getattr(view_class, "permission_service_map", None) or {}
        ...
        # (Plane №108).
        if not permission_map and not inline_map and not bypass_map and not service_map:
            continue
        ...
        for method, action in actions.items():
            ...
            if code is None:
                # И наконец — право, которое проверяет СЕРВИС (Plane №901).
                # Последним, потому что это самый непрямой способ: гейта у
                # ручки нет, и запись каталога держится объявлением автора.
