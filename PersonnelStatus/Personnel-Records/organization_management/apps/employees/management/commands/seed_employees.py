...
слоты. Человек, у которого появилась ЧУЖАЯ РАБОТА, не удаляется, а называется
вслух.

🔴 «ЧУЖАЯ РАБОТА» — НЕ «ЕСТЬ СТАТУС» (Plane №233). Первая версия берегла всех,
у кого есть хоть один статус, — и не удаляла НИКОГО: статус «в строю» ставит
сигнал `give_new_employee_a_status` каждому заведённому человеку, автор при
этом пуст (завела система). На стенде это стоило дорого: снос 5112 человек не
тронул ни одного, повторный сид пересадил на слоты 426, и 4686 остались
сиротами без штатной единицы — реестр показывал 426, а список людей тысячи.

Признак чужой работы теперь конкретен: учётная запись, статус С АВТОРОМ
(`created_by` заполнен — значит его завёл человек или ручка от его имени) или
приложенный к статусу документ. Всё остальное сид создал сам и вправе убрать.
"""
...
class Command(BaseCommand):
    ...
    def _wipe(self) -> None:
        ...
        busy = [
            ...
            or employee.statuses.filter(created_by__isnull=False).exists()
            or employee.statuses.filter(documents__isnull=False).exists()
        ]
        with transaction.atomic():
