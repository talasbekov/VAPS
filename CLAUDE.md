...
#    🔴 ТЯЖЁЛЫЙ ПРОГОН (apps/ops, apps/ops+operations) ПРИ МАЛОЙ СВОБОДНОЙ
#    ПАМЯТИ — В ПЕРЕДНЕМ ПЛАНЕ, НЕ ФОНОМ (Plane №919, замер 06.09.2026).
#    Машина держала свободными ~900 МБ (доступно ~5,8 ГБ с учётом чужих
#    процессов): тот же прогон, снятый фоновой задачей дважды подряд молча
#    (ни ассерта, ни трассы — только пометка «снята из-за нехватки памяти»),
#    в переднем плане прошёл целиком (1169 passed за 2:22, следом
#    apps/operations — 3604 passed за 3:59) на ТОЙ ЖЕ свободной памяти —
#    дело не в нехватке памяти самой по себе, а в том, что фоновые задачи
#    снимаются первыми. Молчаливо снятый гейт неотличим от гейта, которого
#    не было: пустой вывод фонового прогона — это «прогон снят», а не
#    «прогон прошёл».
...
| `Предложено Claude` | всё, что придумал не заказчик: находка, техдолг, рефакторинг, «заодно бы починить», следствие чужой правки, шаг плана или ревью, ошибка из прогона (одна ошибка — одна карточка). В описании: откуда взялась и что будет, если не делать. Находки уровня P0/P1 (ломает бой, теряет данные, дыра в безопасности) — заводить с `--priority urgent`/`--priority high` (Plane №1009): поле есть в Plane, но без явного флага `add` всегда ставит `none`, и срочная находка тонет в общем потоке | — |
...
- 🔴 **Задача заказчика из Plane — приоритетна и ЗАПИСЫВАЕТСЯ в `<vault>/RAW/README.md`, раздел 17 «Задачи заказчика из Plane — журнал поручений»** (решение заказчика 07.09.2026: «мои поставленные задачи через Plane тоже приоритетные, их нужно добавлять в RAW/README.md, потому что я тестирую и подсказываю, как должно быть»). Одна карточка — одна строка таблицы, текст поручения как есть, в момент, когда карточка замечена в `Backlog`. README — источник требований высшего приоритета, поэтому поручение, не попавшее туда, для следующих сессий не существует. Карточки из `Предложено Claude` туда не пишутся.
...
Состав прогона:
1. **Фикстуры стенда**: `manage.py seed_smoke_fixtures` (без него три пробы падают сторожем «проверять нечего»). ⚠️ `seed_expense_chain` по умолчанию СДАЁТ ДЕНЬ и этим краснит две пробы `day-submission` (им нужен несданный день). Перед смоуком звать **`seed_expense_chain --no-submit`** — именно её, а не `seed_smoke_fixtures`: у второй такого флага НЕТ вовсе, и вызов с ним отвечает `unrecognized arguments: --no-submit`. Прежняя редакция говорила «звать его», и «его» прочли как ближайшую команду — полный прогон 06.09.2026 начался с ошибки (Plane №826). С флагом люди и статусы заводятся, день остаётся несданным, пробы проходят путь целиком (Plane №72).
...
- 🔴 **Занятость файла проверяется `git diff HEAD -- <файл>`, а не `git status`** (Plane №876, 06.09.2026). Индекс общий на дерево: чужой `git add` до чужого коммита оставляет в индексе версию СТАРЕЕ HEAD, и `git status` рисует такой файл `MM` («в индексе правка, в дереве правка») — хотя рабочее дерево на самом деле совпадает с HEAD и никакой незакоммиченной правки нет вовсе. Один такой случай (двенадцать файлов разом, устаревший индекс от коммита за несколько часов до текущего HEAD) стоил дерева переписки трижды — сессии спрашивали друг у друга владельца файлов без единой правки. `git status` отвечает на «что лежит в индексе», не на «кто держит файл»; на второй вопрос отвечает только `git diff HEAD -- <файл>` (пусто — не занято, что бы ни показывал `git status`).
...
- 🔴 **Django-стенд :8100 поднимается БЕЗ `--noreload`** (Plane №764, 05.09.2026). С этим флагом сервер не подхватывает правки Python с момента запуска ВОВСЕ, и признак читается ровно наоборот: код на диске верный, `pytest` зелёный, живая проба красная — и первым делом начинаешь чинить работающий код или переписывать пробу. Хуже того, ЗЕЛЁНАЯ живая проба на устаревшем сервере тоже ничего не значит, а выглядит подтверждением. **Признак, который надо помнить: «код верный, pytest зелёный, живая проба красная» → сначала проверь, не устарел ли сервер**, а не свой диафф. Проверять — `ps -eo pid,ppid,etime,args | grep runserver`.
  ...
  🔴 **Для стендов, поднятых через `setsid`, PPID не определяет владельца.**
  `prod-stand.sh` и `smoke-backend.sh` намеренно отсоединяют сервер, поэтому у
  разных сессий PPID становится одним (`systemd --user`); он не отличает свой
  стенд от чужого и не доказывает, что сирота принадлежит текущей сессии.
  Владельца такого стенда ищут по времени последней записи в
  `/tmp/next-prod-<порт>.log` или `/tmp/next-dev-<порт>.log`, наличию соединений
  через `ss -tn` и метке №832. Метка может быть старой: её pid нужно сверять с
  живым процессом; несуществующий pid означает «метка протухла», а не живого
  владельца. **Исполнитель: Кодекс Астра 6.**
