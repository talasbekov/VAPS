...
## Трекер задач — источник задач (ОБЯЗАТЕЛЬНО)

**Переезд 24.08.2026: ClickUp → самостоятельно поднятый Plane** (`/home/erda/plane`, http://localhost:8090). ClickUp с этой даты не используется как источник задач; его данные переносятся в Plane (перенос не завершён — см. `obsidian-vault/WIKI/Бэклог-ClickUp-Smart-Josparlau.md`). Историческая привязка: workspace ClickUp `90182995873` → Team Space → `🚀 Проекты` → список **Smart Josparlau** (`901820659486`).

Трекер — вход задач, vault — источник правды по их разбору и исполнению; расходиться они не имеют права.

Требования ниже обязательны к выполнению и не отменяются «мелкостью» правки:

1. **Подтягивать задачи из трекера.** В начале работы над разделом — прочитать задачи проекта в Plane. Не выдумывать себе задачи в обход трекера; если работа появилась не оттуда — сначала завести карточку, потом делать.
2. **Взял в работу → сразу `in progress`.** Перед первой правкой кода перевести карточку в работу. Не после, не «в конце заодно».
3. **Код готов → `review`.** Реализация завершена, идёт ревью (само-ревью/`/code-review`) — карточка в `review`.
4. **Ревью пройдено → `on test`.** Гоняются тесты/проверка на стенде. В `completed` из `review` напрямую не переводить.
5. **Тесты и проверка зелёные → `completed`.** Только по факту: гейт прогнан, вывод виден, дефектов нет. Красные тесты или найденный дефект → карточка возвращается в `in progress`, а не закрывается.
   Цепочка статусов ровно такая: `not started`/Backlog → `in progress` → `review` → `on test` → `completed`/Done.
6. **Каждый переход статуса зеркалить в vault.** Взял в работу — строка в `WIKI/Бэклог-ClickUp-Smart-Josparlau.md`; закрыл — перенести строку в «Закрытые» с коммитом, добавить запись в `<Модуль>/Changelog.md`, обновить `Status.md`, а отклонения и решения — в `Decisions.md`/`Known-Issues.md`. Задача не считается закрытой, пока vault не обновлён.
7. **Следить за актуальностью обоих.** Расхождение «в трекере одно, в vault другое» — дефект работы. Замеченное расхождение чинить сразу, не откладывая до конца сессии. Если трекер недоступен (лимит API, стек не поднят) — расхождение ЗАПИСАТЬ таблицей в бэклог-заметку vault и довести статусы, когда доступ вернётся.
