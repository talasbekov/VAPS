...
Пока задачи есть, по ходу каждой хватает **целевых проб её глубины** плюс `tsc`:
...
| Общий компонент или токен, который читают многие экраны | пробы всех экранов-читателей (найти грепом до кода) |
...
Состав прогона:
1. **Фикстуры стенда**: `manage.py seed_smoke_fixtures` (без него три пробы падают сторожем «проверять нечего»). ⚠️ `seed_expense_chain` СДАЁТ ДЕНЬ и этим краснит две пробы `day-submission` — звать его перед смоуком не надо.
...
3. **Фронтенд весь**: `tsc`; полный `playwright.smoke.config.ts` с `SMOKE_LIVE=1` (без переменной живые спеки молча скипаются, скип читается как зелень); консоль браузера — на ошибки.
...
- **Сторож `dev-guard.sh` не отменяет замера.** Стенд поднимается `npm run dev:guard`, и сторож сам перезапускает сервер: мягкий порог 2500 МБ (ждёт затишья — прогон e2e не рвёт) и жёсткий ≈40 % памяти машины (перезапускает немедленно). Мягкий порог ВЫШЕ правила выше — значит между 2 ГБ и 2500 МБ гашу по PID я, а не он.
