...
4. Закрытие задачи = гейт + vault + коммит + пуш + `On test` + замер RSS + пересборка Plane — и сразу следующая задача.
4а. 🔴 `Done` СТАВИТ ТОЛЬКО ЗАКАЗЧИК СВОИМИ СЛОВАМИ. Сделанная задача уходит в `On test` и ждёт там; переводить её в `Done` самому запрещено, сколько бы зелёных прогонов ни было (решение заказчика 31.08.2026).
...
| `On test` | ревью пройдено, гейт прогнан, вывод виден, дефектов нет. **Здесь задача и остаётся**, пока заказчик не проверит руками. Из `Review` в `Done` напрямую нельзя | см. «Закрытие задачи» |
...
4. Карточка → `On test` (НЕ `Done`); хэш коммита — последним шагом в строку `Changelog.md` (записанный до коммита устаревает при `--amend`).
...
4б. Ждать слова заказчика. `Done` ставится ТОЛЬКО после его «можно менять состояние» — своим решением карточку не закрывать.
...
Отказ пуша (сеть, права, разошедшиеся истории) — блокер: карточка не переводится в `On test`, причина пишется в vault, разбирается до взятия следующей задачи.
