...
Vault устроен как «второй мозг» в два слоя:

- **Слой знаний**: `RAW/` (сырые материалы — не переписывать, только ссылаться), `WIKI/` (структурированные заметки с `[[wiki-ссылками]]`, сюда же планы/эпики), `OUTPUT/` (готовые документы для внешнего использования), `LOG.md` (журнал обработки).
- **Слой разработки**: папки модулей (Personnel-Records / VisitX / Accreditation / Frontend / Infrastructure / BMAD-Process) с оперативным состоянием — Status / Changelog / Decisions / Known-Issues.

Правила для Claude Code:

- **Перед началом работы** над модулем — прочитать `obsidian-vault/<Модуль>/Status.md` и `obsidian-vault/<Модуль>/Known-Issues.md`.
...
- **Новый сырой материал** (транскрипт, статья, черновик) класть в `obsidian-vault/RAW/`, обработку вести по правилам из `00-Index.md` (RAW → WIKI → карта знаний в 00-Index → запись в LOG.md).
