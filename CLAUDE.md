...
SMOKE_LIVE=1 npx playwright test -c playwright.walk.config.ts -g "persona admin"  # обход портала — БЛОКАМИ по персонам (136 проб, три блока)
...
#        проб 136 = 3 × 45 + 1: у персоны 42 маршрута + 2 пробы каркаса
#          (`aside`, `header`) + выход из системы; сто тридцать шестая —
#          «карта маршрутов покрыта обходом», она вне персон и в `-g "persona …"`
#          НЕ ПОПАДАЕТ. Значит три блока дают 135, а не 136, и оставшуюся
#          пробу надо гонять отдельно — иначе покрытие карты не проверяется
#          вовсе.
...
#    сессией: `persona observer` — 45 passed за 6,0 мин.
