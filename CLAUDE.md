...
Сомневаешься — считай правку глубокой. Независимо от глубины полный прогон обязателен: перед отчётом о пустой очереди; после 5–7 мелких задач подряд без полной проверки (счёт по леджеру); после любого перезапуска или пересборки стенда; когда что-то повело себя странно без явной причины. Ориентир — примерно раз в шесть часов активной работы, но решает содержание, а не часы.
