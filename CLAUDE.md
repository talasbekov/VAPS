...
npm run stand:prod && npm run smoke:prod                                          # ВСЕ целевые пробы по ПРОД-СТЕНДУ (251 проба; 29.08.2026 — 245 passed, 4 skipped, 4,6 мин)
SMOKE_LIVE=1 npx playwright test -c playwright.walk.config.ts -g "persona admin"  # обход портала — БЛОКАМИ по персонам (142 пробы, три блока)
...
#        проб 142 = 3 × 47 + 1: у персоны 44 маршрута + 2 пробы каркаса
#          (`aside`, `header`) + выход из системы; сто сорок вторая —
#          «карта маршрутов покрыта обходом», она вне персон и в `-g "persona …"`
#          НЕ ПОПАДАЕТ. Значит три блока дают 141, а не 142, и оставшуюся
#          пробу надо гонять отдельно:
#              SMOKE_LIVE=1 npx playwright test -c playwright.walk.config.ts -g "карта маршрутов"
#          🔴 ЭТО НЕ ФОРМАЛЬНОСТЬ. Первый же отдельный запуск (29.08.2026,
#          Plane №319) оказался КРАСНЫМ: вне обхода жили два объявленных
#          маршрута — `/security-ops/vehicles` и справочники штата. То есть
#          «обход зелёный» не означало «портал обойдён», и узнать это из
#          прогона было нельзя: проба, стерегущая покрытие, выпадала ровно из
#          того способа, которым обход гоняют. Маршруты дописаны, числа выше —
#          уже с ними (было 136 и 45). Место этой пробы (обход или целевой
#          смоук) решается в №319 — пока она здесь, её запуск ОТДЕЛЬНОЙ
#          строкой обязателен.
...
#    сессией: `persona observer` — 45 passed за 6,0 мин. (Времена замерены на
#    45 пробах в блоке; после №319 их 47 — на два маршрута дольше.)
