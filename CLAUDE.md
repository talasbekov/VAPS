...
## Каждые два часа — полный прогон работоспособности (ОБЯЗАТЕЛЬНО)

По ходу длинной сессии, **не реже чем раз в два часа** (и обязательно перед тем, как отчитаться о пустой очереди), проект проверяется целиком — а не только то, что трогали последней задачей. Цель — ловить не свои регрессии, а состояние системы: чужие правки, протухшие фикстуры, отвалившиеся эндпоинты, раздувшийся стенд.
