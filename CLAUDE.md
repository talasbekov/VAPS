...
## ClickUp — источник задач (ОБЯЗАТЕЛЬНО)

Задачи берутся из ClickUp: workspace `90182995873` → Team Space → `🚀 Проекты` → список **Smart Josparlau** (`901820659486`). Зеркало с разбором — `obsidian-vault/WIKI/Бэклог-ClickUp-Smart-Josparlau.md`. ClickUp — вход задач, vault — источник правды по их разбору и исполнению; расходиться они не имеют права.

Требования ниже обязательны к выполнению и не отменяются «мелкостью» правки:

1. **Подтягивать задачи из ClickUp.** В начале работы над разделом — `clickup_filter_tasks` по списку `901820659486` (или `clickup_search`). Не выдумывать себе задачи в обход списка; если работа появилась не из ClickUp — сначала завести карточку (`clickup_create_task`), потом делать.
2. **Взял в работу → сразу `in progress`.** Перед первой правкой кода перевести карточку `clickup_update_task(status="in progress")`. Не после, не «в конце заодно».
...
5. **Тесты и проверка зелёные → `completed`.** Только по факту: гейт прогнан, вывод виден, дефектов нет. Красные тесты или найденный дефект → карточка возвращается в `in progress`, а не закрывается.
   Цепочка статусов списка ровно такая: `not started` → `in progress` → `review` → `on test` → `completed`.
...
7. **Следить за актуальностью обоих.** Расхождение «в ClickUp одно, в vault другое» — дефект работы. Замеченное расхождение чинить сразу, не откладывая до конца сессии.
