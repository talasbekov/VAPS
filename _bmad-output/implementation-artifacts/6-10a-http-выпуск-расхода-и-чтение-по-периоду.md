...
1. **POST-выпуск за дату.** Given `POST /api/operations/expense-reports/` с `{division_id, business_date}` и правом `daily_report.generate`, When вызываю, Then сервис `issue_expense_document(division_id, business_date, actor)` выполняется в одной транзакции и возвращает выпущенный `IssuedDocument` (номер, sha256, ссылка на attachment для скачивания через 6.7); scope-проверка «своё подразделение» — через сервис-гвард `ensure_division_scope` (PermissionService → 403 DomainError), вызываемый из view — КАНОН 5.8 (текст исправлен на ревью 2026-07-13, решение D3: изначальная формулировка «в СЕРВИСЕ, НЕ во view» противоречила канону — доменные сервисы вызываются хуками без HTTP-актора). Существующие guards пробрасываются как есть: нет сдачи→409 `REPORT_NOT_READY_FOR_DATE`, старая схема→422 `SNAPSHOT_SCHEMA_UNSUPPORTED`, несходимость→422 `REPORT_NOT_CONVERGENT`, повтор версии→409 `DOCUMENT_ALREADY_ISSUED`.

2. **GET расход за дату.** Given `GET /api/operations/expense-reports/?division_id&business_date`, When дата валидна и есть выпущенный документ, Then возвращается его метаданные (number/year/business_date/status/attachment_id) через `IssuedDocumentSelector` (read-only); авторизация — `ensure_division_scope` перед point-lookup (текст исправлен на ревью 2026-07-13, решение D3: для точечного чтения по явному division_id субтри-гвард функционально эквивалентен self-narrow по `visible_division_ids`; list-семантика с narrow-селектором появится при реальном списочном чтении, E10). Байт-файл отдаётся эндпоинтом скачивания 6.7 (X-Accel), 6.10a его НЕ дублирует.
...
<!-- Ревью 2026-07-13 (Fable 5, cross-model vs спека+dev Opus 4.8). Слои: Blind Hunter / Edge Case Hunter / Acceptance Auditor. Вердикт AC: 1-partial, 2-partial, 3-pass, 4-FAIL, 5-partial, 6-pass. -->

- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (`report_data_horizon()` = min(EmployeeStatusSelector.earliest_start(), HistoricalEmployeeSelector.earliest_history_start()); проба дата-чувствительна, вынесена из цикла; регрессионные пины: far-past 422 на НАСЕЛЁННОЙ БД для period и POST) (ex-Decision, **РЕШЕНО Bratan 2026-07-13: глобальный min-горизонт** = min(date_start по EmployeeStatus ∪ EmployeeDivisionHistory), дата раньше → 422; ветка «подразделение ещё не существовало» = 7.x, спека фиксирует) **ГЛАВНОЕ (AC-4 fail, все 3 слоя независимо):** проба date-before-data вакуумна вне тест-изоляции. `assert_report_date_has_data` — глобальная дивизион-агностичная truthiness-проба `roster_on`/`overlapping_on`, но `roster_on` имеет ДАТА-НЕЧУВСТВИТЕЛЬНЫЙ fallback (`core/selectors.py:314-316`: 0 history-строк на пилоте → ВСЕ WORKING через fallback на любую дату, хоть 1990) → на любой живой БД проба никогда не пуста → 422 не выстрелит НИКОГДА: period отдаст сфабрикованные ретро-страницы, POST упадёт в 409 «нет сдачи» (подмена семантики). Ветка AC-4 «подразделение ещё не существовало» не реализована в принципе. Тесты 422 проходят только на тотально пустой БД (dev-record это признаёт, но не эскалировал — «молчание=СТОП» нарушен). Решить семантику горизонта: (а) глобальный числовой горизонт = min(date_start по EmployeeStatus ∪ EmployeeDivisionHistory) — дата раньше → 422; (б) division-aware (требует достоверной даты создания подразделения — на пилоте её нет); (в) оставить и деферить в 7.x (AC-4 мёртв — не рекомендуется). [Backend/VAPS/apps/operations/submissions/services/expense_read_service.py:29-45]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (`date_to > Clock.today_local()` в period → 400; ⚠️ by-date GET намеренно НЕ блокирован — уточнение при имплементации: он читает только ISSUED-документы и не фабрикует, а легально выпущенный через override «на завтра»-документ 6.10b обязан читаться; зафиксировано комментарием в коде и описанием схемы) (ex-Decision, **РЕШЕНО Bratan 2026-07-13: блокировать** — `date_to > today` в period и `business_date > today` в by-date → 400) Будущие даты в GET не ограничены ничем: `GET /period/?date_from=2035-01-01&date_to=2035-03-03` отдаёт 62 официальные «страницы»-фантома (compute спокойно считает будущее из текущего списка). 6.10b вешает tomorrow-block ТОЛЬКО на ветку выпуска — period/by-date останутся открыты и после неё; FR-18 про чтение будущего молчит. Решить: (а) блокировать `date_to > Clock.today_local()` → 400 в period (и business_date > today в by-date), (б) разрешить осознанно и зафиксировать в спеке. [Backend/VAPS/apps/operations/submissions/services/expense_read_service.py:77-108]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (текст AC-1/AC-2 приведён к факту с пометкой ревью-решения D3) (ex-Decision, **РЕШЕНО Bratan 2026-07-13: принять канон 5.8**, текст AC-1/AC-2 исправить под факт, код не трогать) Scope во VIEW vs буква AC-1/AC-2: все три экшена зовут `ensure_division_scope` из view, actor-narrow селектора (`visible_division_ids`) не реализован, `IssuedDocumentSelector` не расширен — Task 2 отмечен `[x]` ложно. Смягчение: `ensure_division_scope` — сервис-слойная функция, это КАНОН 5.8 (docstring scope_gate прямо говорит «view calls this guard»), дыры в авторизации нет (403 до чтения, зондирование дат без прав невозможно). Решить: (а) принять канон и исправить текст AC-1/AC-2 в спеке под факт, (б) переносить scope внутрь сервисов + вводить actor-narrow метод селектора. [Backend/VAPS/apps/operations/submissions/api/views.py]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (`_ensure_division_exists` → 404 ENTITY_NOT_FOUND во всех трёх экшенах, ПОСЛЕ scope-гварда — existence-оракула для чужих нет; тест на POST/GET/period) Несуществующий `division_id` под глобальной ролью (scope NULL): period → 200 с нулевыми страницами (неотличимо от легально пустого), POST → мислидящий 409 «нет сдачи». `_scope_matches(None, id)` = True безусловно, `subtree_ids(unknown)` = {unknown}, existence нигде не проверяется. Fix: `CoreDivisionTreeSelector.exists(division_id)` (уже есть, selectors.py:129) → 404 ENTITY_NOT_FOUND в начале POST/GET/period [Backend/VAPS/apps/operations/submissions/api/views.py:187-246]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (+8 тестов: 409 нет-сдачи, 409 повторный выпуск, легально-пустое подразделение → 200 с нулями, far-past 422 на населённой БД ×2 [period+POST], граница cap 62→200/63→400, будущее→400, phantom-UUID→404 ×3 экшена; 18/18) Тест-дыры (Task 6 отмечен `[x]` ложно): нет HTTP-тестов проброса guards (409 REPORT_NOT_READY_FOR_DATE, 409 DOCUMENT_ALREADY_ISSUED); нет обязательного теста «легально пустое подразделение → валидно, НЕ 422» (единственная граница, отличающая новый код от старого — вскрыл бы вакуумную пробу); нет теста границы cap (62→200/63→400). Дописать после решений D1/D2 [Backend/VAPS/apps/operations/submissions/tests/test_expense_report_api.py]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (**ЗАМЕР: p95 ≈ 0.96с, median 0.94с, 15.2мс/дата** — derive_period 62 дня × 5000 сотрудников/25 подразделений, ~30% со статусами, 5 прогонов, dev-железо; порог 2с не превышен с запасом ×2 → SYNC подтверждён числом, ARCH-DEFERRED-048 остаётся deferred; двойная материализация убрана — горизонт-проба O(1) агрегатом, вынесена из цикла) AC-5 без числа (Task 5 `[x]` ложно): «Формальный бенчмарк не гонял» при требовании «решение зафиксировано ЧИСЛОМ». Плюс перф-дефект цикла: `roster_on` материализует ВСЕХ WORKING по БД дважды на дату (проба + compute, фильтрация subtree в Python) → ~124 полных материализации на 62-дневный запрос; оценка 62×38.5мс ≈ 2.4с > порога 2с. Fix: убрать двойную материализацию (проба переиспользует compute / лёгкий exists-запрос), замерить p95 на реалистичном объёме, зафиксировать число; > 2с → эскалация ARCH-DEFERRED-048 [Backend/VAPS/apps/operations/submissions/services/expense_read_service.py:39-45,105-107]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (`@extend_schema` на create/list/period: request/parameters/responses + 4xx в description; `make schema` (+161/−19) + `npm run generate:api` — frontend schema.d.ts синхронизирован [урок 9.2]; оба гейта зелёные) OpenAPI-схема новых эндпоинтов — пустышка (Task 1 `[x]` ложно, dev-record признаёт): POST без requestBody, GET без query-параметров, «No response body» при реальных телах, ни одного 4xx; потребители кодогена E8/E10 получат ложный контракт. Fix: `@extend_schema` на все три экшена + `make schema` (+ проверить дрейф frontend schema.d.ts — урок 9.2) [Backend/VAPS/apps/operations/submissions/api/views.py; Backend/VAPS/schema.yaml]
- [x] [Review][Patch] ✅ ЗАКРЫТО 2026-07-13 как ложная посылка: у DailySubmissionViewSet ТО ЖЕ ограничение с комментарием «HEAD stays 405 everywhere — 5.8a/b minimal surface» (Blind Hunter не видел контекста) — это канон проекта, не упущение; добавлен зеркальный комментарий Мелочь: `http_method_names = ["get", "post", "options"]` без `head` — HEAD к read-эндпоинтам получит 405 [Backend/VAPS/apps/operations/submissions/api/views.py:174]
- [x] [Review][Defer] `GET /expense-reports/` (list-роут) семантически point-lookup: возвращает одиночный объект/404, требует оба параметра, operationId `retrieve` на list-пути — клиент, ожидающий массив от коллекционного URL, получит объект — deferred, решить при подключении фронт-экрана расхода (10.5/E10)
- [x] [Review][Defer] Страницы периода без единого снапшота консистентности (READ COMMITTED, независимые запросы per-date): параллельная запись статуса во время цикла → соседние страницы отражают разные состояния БД — deferred, terpимо для read-only просмотра; задокументировать trade-off при 10.5
