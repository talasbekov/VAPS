...
- **Найдено ревью (Acceptance Auditor): «все назначения одного ОМ разделяют одно окно» — v1-АППРОКСИМАЦИЯ, не проверенный факт.** Пост, занятый лишь на суб-интервал внутри более длинного события, под этой моделью будет проверяться на Отдых/двойное назначение против ВСЕГО окна события — консервативно (никогда не занижает предупреждение), но может ложно-сработать на реальном суб-событийном тайминге. Не решается здесь (не изобретается построчный интервал без данных о реальной потребности в нём) — явно оставлено ОТКРЫТЫМ вопросом для 16.3b+, не унаследовано как решённое.
...
**3-агентное ревью (Blind Hunter / Edge Case Hunter / Acceptance Auditor) — реального бага не найдено, 1 находка закрыта документацией:**
- Blind Hunter: `clean()`-гард корректен (`>=`, короткое замыкание на `None`); миграция — чистый nullable `AddField`, ноль риска; `SecurityEventSerializer.Meta.fields` — явный список, НЕ `__all__` — новые поля НЕ попадают в API/schema.yaml (гипотетический риск дрейфа схемы не материализовался).
- Edge Case Hunter (живой прогон, включая временные пробники, все откачены): POST через реальный `ops-security-event-list`-маршрут — ключей `starts_at`/`ends_at` в ответе НЕТ; `make schema`-регенерация против закоммиченного `schema.yaml` — нулевой диф; `.objects.create(starts_at=now, ends_at=now)` УСПЕШНО создаёт строку (гард `clean()`-only, действительно обходится `create()`, задокументированная граница — подтверждена живым прогоном, не просто прочитана в коде); 948 тестов audit/rbac/route-поверхности — без изменений.
- **Acceptance Auditor: «все назначения одного ОМ разделяют одно окно» — v1-АППРОКСИМАЦИЯ, подана как решённый факт, не как флагованное допущение.** Не бага (консервативно — никогда не занижает предупреждение), но реальный design-trade-off, который 16.3b унаследует как открытый вопрос. Закрыто усилением докстринга модели + Scope Decision явной пометкой «v1 approximation, not settled — 16.3b+ должна пересмотреть», без изменения кода (нет данных для построчного интервала без реальной необходимости).

`make gate` (после документационной правки) — **3690 passed**, 0 regressions, no drift. Status → done.
...
| 2026-08-01 | 3-агентное ревью: реального бага не найдено. Событийная (не построчная) гранулярность окна усилена как явно флагованное v1-допущение (докстринг+Scope Decision), не изменение кода. `make gate` — 3690 passed. Status → done. |
