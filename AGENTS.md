...
# Smart Josparlau — правила работы Codex

## Обязательные правила заказчика — 05.09.2026, подтверждены 07.09.2026

1. **Обязательное поле каждой работы — второй мозг `/home/erda/Музыка/Obsidian_brain/`.** Это основное хранилище всей документации и истории работ. Документация Smart Josparlau ведётся в `/home/erda/Музыка/Obsidian_brain/smart-josparlau-vault/` (`<vault>`). Все планы, исследования, решения, выполненные действия, проверки, результаты и блокеры обязательно фиксируются там. Переписка, Plane и файлы в репозитории не заменяют записи во втором мозге; отдельную конкурирующую копию документации не создавать. Поле считается заполненным только после записи результата в проектный vault; без этой записи работа не завершена.
2. **Записывать по ходу работы, в том же заходе.** Для каждого действия указывать дату, карточку Plane, что сделано и почему, результат проверки; после коммита — его хэш. Не откладывать журнал на конец сессии. Не записано во втором мозге — не сделано.
3. **Обязательная подпись исполнителя: `Кодекс Астра 6`.** Во всех своих отчётах заказчику, комментариях Plane и записях второго мозга явно указывать `Исполнитель: Кодекс Астра 6`. Чужие и исторические записи себе не приписывать.
4. **Очередь запускается по команде заказчика.** После команды брать задачи из Plane на `http://localhost:8090`, workspace `vaps`, проект `Smart Josparlau`, и выполнять установленный ниже цикл. Одной команды на запуск очереди достаточно: повторно спрашивать разрешение на каждую следующую разрешённую задачу или обычный шаг её выполнения запрещено.
5. **После запуска очереди останавливаться, пока разрешённые задачи не иссякнут, нельзя.** Довести задачу до гейта, записи во втором мозге, коммита, пуша и `Review`, затем сразу брать следующую. Промежуточный отчёт и ожидание проверки карточки в `Review` не завершают работу над очередью. Не брать неразрешённые предложения и не переводить карточки самостоятельно в `On test` или `Done`. Исчерпание очереди и вынужденный останов подтверждаются по правилам раздела «Цикл работы».
...
3. Вся документация и всё сделанное записаны во втором мозге `/home/erda/Музыка/Obsidian_brain/`, в vault проекта, в том же заходе с подписью `Исполнитель: Кодекс Астра 6`. Не записано — не сделано.
4. После команды заказчика на запуск очереди: закрытие задачи = гейт + vault + коммит + пуш + `Review` + замер RSS + пересборка Plane — и сразу следующая задача без повторного разрешения, пока разрешённая очередь не иссякнет.
...
python3 /home/erda/plane/migration/plane_task.py add "Smart Josparlau" "Название" --desc "зачем; откуда взялась; что будет, если не делать" --state "Предложено Codex"
...
SMOKE_LIVE=1 npm run playwright -- -c playwright.walk.config.ts -g "persona admin"  # обход портала — БЛОКАМИ по персонам (141 проба = 3 × 47)
...
**`/home/erda/Музыка/Obsidian_brain/` — второй мозг заказчика.** Вся документация и все работы по этому проекту обязательно фиксируются в его проектном vault с подписью `Исполнитель: Кодекс Астра 6`; это обязательная часть выполнения любой задачи, включая правку правил и настройку инструментов.
...
VAPS-специфичное в auto-memory (`.Codex/memory`) и `docs/api-gaps.md` не пишется — только в vault. Auto-memory допустим лишь для общего, не относящегося к VAPS.
...
Состояния: `Backlog` → `In Progress` → `Review` → `On test` → `Done`; отдельно `Предложено Codex` и `Cancelled`. (`Review` и `On test` заведены руками в группе `started`; `Предложено Codex` — переименованная `Todo`.)

| Состояние | Когда | Зеркало в vault |
|---|---|---|
| `Backlog` | задача пришла от заказчика | — |
| `Предложено Codex` | всё, что придумал не заказчик: находка, техдолг, рефакторинг, «заодно бы починить», следствие чужой правки, шаг плана или ревью, ошибка из прогона (одна ошибка — одна карточка). В описании: откуда взялась и что будет, если не делать | — |
| `In Progress` | перед первой правкой кода. Из `Предложено Codex` — только после того, как заказчик перевёл карточку в `Backlog` или разрешил комментарием | строка в бэклог-заметку |
...
**Запуск очереди:** по команде заказчика. После запуска выполнять разрешённые задачи Plane непрерывно до исчерпания очереди; не завершать заход после одной карточки и не спрашивать «продолжать?» между задачами. Поручение изменить правила само по себе не запускает очередь разработки.

**Старт сессии / модуля**
0. **Новая сессия для каждой группы задач Plane — решение заказчика 10.09.2026, Plane №1107.** Каждую группу выполнять в отдельной новой сессии Codex; следующую группу в прежней сессии не начинать. Перед переходом записать в vault состав группы, выполненное, проверки, коммиты и блокеры; новой сессии передать номера карточек и ссылки на эти записи. Переход между сессиями не требует повторного разрешения на уже запущенную очередь и не отменяет её непрерывное выполнение. Само поручение изменить это правило очередь разработки не запускает. Исполнитель: Кодекс Астра 6.
1. Прочитать `<vault>/00-Index.md`, затем `<vault>/<Модуль>/Status.md` и `<vault>/<Модуль>/Known-Issues.md`.
2. `list "Smart Josparlau" --open`; прочитать новые комментарии заказчика в `Предложено Codex` и отработать их.
3. Ранжировать очередь; порядок с обоснованием — строкой в бэклог-заметку.

**Порядок выбора** — по смыслу, не сверху вниз: (1) блокирующее остальные — модель, контракт, от которых зависят экраны; (2) дефекты живого стенда; (3) задачи заказчика в логическом порядке сценария; (4) разрешённые заказчиком из `Предложено Codex`. Смежное (один файл/экран) — подряд: переключение контекста дороже правки.
...
4б. Ждать слова заказчика только по этой карточке, продолжая следующую разрешённую задачу. `On test` и `Done` ставятся ТОЛЬКО им — своим решением карточку дальше `Review` не двигать. Ожидание проверки не останавливает очередь.
...
**Обычное завершение запущенной очереди — только после исчерпания разрешённых задач.** Проверить всю очередь Plane с учётом пагинации API: одной страницы списка недостаточно. Готовые карточки в `Review` ждут заказчика, а предложения без его разрешения не становятся задачами в работу автоматически. «Очередь пуста» — вывод, требующий проверки: полный прогон → находки карточками в `Предложено Codex` → повторная сверка очереди → только потом итоговый отчёт.

**Вынужденный останов до исчерпания очереди** допустим только по прямой команде заказчика либо при блокере, который невозможно устранить в пределах имеющихся полномочий и который не позволяет продолжить разрешённую работу. Сначала исчерпать доступные способы устранения и проверить, можно ли выполнять независимые задачи. Блокер одной карточки не останавливает остальные доступные задачи. Невозможность продолжить, выполненные проверки и необходимые действия заказчика записать во второй мозг и сообщить с подписью `Исполнитель: Кодекс Астра 6`. Усталость, объём работы, завершение одной задачи или промежуточный отчёт не являются поводом остановиться.
...
- Каждый замер и перезапуск — строкой в `Frontend/Changelog.md` с цифрой RSS. Раздувание раньше срока повторяется — карточка в `Предложено Codex`, а не молчаливый рестарт.
...
## Codex / Shell Execution Decomposition

Split Codex, Codex, shell, SSH, and tmux work into separate stories:
