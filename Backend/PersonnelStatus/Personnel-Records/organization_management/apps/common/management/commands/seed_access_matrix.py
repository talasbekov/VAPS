"""Девять учёток под матрицу доступа заказчика (Plane №348, №382, №972).

ЗАЧЕМ. Заказчик проверяет права руками и назвал семь персон (№348), затем
восьмую (№382), затем отдельного Штаба второго департамента (№972); для
каждой — список НЕДОСТУПНЫХ модулей и уровень, на котором персона работает. Заводить
...
PERSONAS = [
    ...
    Persona(
        ...
        # «Сбор сил» снова ЗАКРЫТ (Plane №972, `[ШТБ-02]`, решение заказчика
        # 08.09.2026): №944 сняла его из закрытого, прочитав раздел 7.1 так,
        # будто штаб — обе руководящие персоны второго департамента. Штаб —
        # отдельная персона `ops_staff` ниже; этой «Сбор сил» закрыт по №939.
    ...
    Persona(
        ...
        "Реестр ОМ, Сбор сил, Командный центр, Транспорт ГОН, Система",
    ),
    Persona(
        "dept_head_d2",
        ...
        # «Сбор сил» снова ЗАКРЫТ (Plane №972, `[ШТБ-02]`): начальник второго
        # департамента — руководство, но не Штаб. Штабные обходы
        # `OPS_STAFF_COMMAND` при нём остаются: они про расстановку, этапы и
        # сводку ГВО (№601, №947), а не про сбор сил (`[ШТБ-05]`).
        "Сбор сил, Аналитика службы, Ежедневный отчёт, Система",
    ),
    Persona(
        "forces_officer",
    ...
    Persona(
        "ops_staff",
        "Штаб второго департамента",
        # Человек второго департамента (учётка привязывается к его
        # сотруднику), а грант — на ВСЮ организацию: Штаб делит потребность
        # по всем департаментам, и область у права нет смысла сужать.
        "dept_second",
        # 🔴 РОВНО ОДИН ГРАНТ (Plane №972, `[ШТБ-01]`, `[ШТБ-04]`, `[ШТБ-05]`,
        # решение заказчика 08.09.2026 «отдельный Штаб с ролью OPS_STAFF»).
        # До №972 Штабом считались обе руководящие персоны второго
        # департамента (№944 выдала их профилю `forces.command`), и «Сбор
        # сил» открылся тем, кому заказчик его закрыл (№939). Роль-добавка
        # `OPS_STAFF_COMMAND` сюда НЕ выдаётся: после передачи состава на
        # расстановку Штаб людей по постам не расставляет (`[ШТБ-06]`) — это
        # старший объекта.
        (("OPS_STAFF", "none"),),
        # Закрытое выведено из прав роли `OPS_STAFF` (профиль в
        # `seed_operations`): реестр ОМ, командный центр, «Обзор» и статусы
        # она читает (`OPS_READ`, `status.view`), аналитику — тоже; нет
        # `daily_report.generate`, `report.generate` и прав «Системы».
        "Создание и правка мероприятий, Ежедневный отчёт, Отчёты по ОМ, Система",
    ),
    Persona(
        "admin",
        "Администратор",
...
class Command(BaseCommand):
    help = "Заводит девять учёток под матрицу доступа заказчика (Plane №348, №382, №972)."
    ...
    def _scopes(self) -> dict[str, Division | None]:
        ...
        # При РАВНОМ штате — младший по id (Plane №1235, 12.09.2026): у двух
        # департаментов стенда штат сравнялся, и `max` выбирал разные при
        # разных прогонах — начальник управления (`n_meldebekov`, Первое
        # управление Первого департамента) и ответственный за сбор сил
        # (`a_raiymzhanov`) разъехались по разным департаментам, а сдача
        # управления до ответственного не доходила. Ключ с `-pk` держит выбор
        # одним и тем же, и обе учётки живут в одном департаменте.
        other = max(others, key=lambda d: (self._staffed_units(d), -d.pk))
