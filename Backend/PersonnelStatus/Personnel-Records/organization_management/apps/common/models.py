...
class Role(models.Model):
    ...
    # Исключение из области роли, и ровно одно (Plane №348). Область у
    # портальной роли ОДНА, а заказчик потребовал у начальника управления
    # «остальные модули на уровне своего управления ЗА ИСКЛЮЧЕНИЕМ Обзор».
    # Читатель у признака один — DivisionStatisticsViewSet; расширять его на
    # другие экраны значило бы завести вторую область окольным путём.
    overview_at_department = models.BooleanField(
        default=False,
        verbose_name='Обзор на уровне департамента',
        help_text=(
            'Экран «Обзор» показывает департамент целиком, даже если область '
            'роли — управление внутри него'
        )
    )
