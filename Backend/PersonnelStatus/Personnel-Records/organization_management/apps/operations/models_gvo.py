...
class OpsProtectedPerson(TimeStampedModel):
    ...
    # страна и строки «параметр = значение» (дата и место рождения, группа
    # крови, рост, размер обуви, ограничения и предпочтения в питании,
    # аллергии). Раньше всё это набиралось ТЕКСТОМ в карточке сводки при
    # каждом ОМ; теперь живёт у записи справочника и подставляется в сводку
    # при выборе лица. Страна лица — ещё и «Страна» сводки: заказчик просит,
    # чтобы она подтянулась сама, а не вписывалась второй раз.
    #
    # `facts` — список `{key, value}` в порядке образца, а не набор именованных
    # колонок: состав параметров у лиц разный (у супруги нет «предпочтений»),
    # и документ печатает их строками как есть.
