...
CODES = {
    ...
    # Технический диапазон e2e-фикстур конечен: исчерпание не означает
    # недопустимую нагрузку, повторить его без смены диапазона нельзя.
    "FIXTURE_DATE_RANGE_EXHAUSTED": frozenset({409}),
    # ── Доступ (Plane №36) ───────────────────────────────────────────────
    ...
    # 🔴 `PLACEMENT_EMPTY` ЗДЕСЬ БОЛЬШЕ НЕ ОБЪЯВЛЯЕТСЯ (Plane №793). Ключ стоял
    # ДВАЖДЫ — в блоке уведомлений о заступлении и здесь, в блоке
    # согласования. Значения совпадали, поэтому поведение не страдало: второе
    # объявление молча перекрывало первое. Но словарь — закрытый договор с
    # клиентом, и его читают глазами: два объявления одного кода означают, что
    # правка одного из них останется перекрытой вторым, беззвучно. Ровно этот
    # класс ошибки словарь и заведён предотвращать.
    #
    # Оставлено ОДНО объявление — выше, в блоке уведомлений о заступлении:
    # там код появился первым. Поднимают его оба места (`acknowledgement_
    # notify.py` и отправка на согласование в `security_events.py`), и это
    # нормально — код один, смысл один: «расстановка пуста, работать не с чем».
    ...
    # Отказавшемуся напоминать нечего (Plane №616) — СВОЙ код, а не
    # ALREADY_ACKNOWLEDGED: «подтвердил» и «отказался» — разные состояния, и
    # клиенту незачем читать про подтверждение там, где человек отказался.
    "ALREADY_DECLINED": frozenset({422}),
    ...
    # сектор берутся у назначения, и без поста определять их нечем.
    ...
    # Довыделение недобора (`[СБС-12]`, Plane №426).
    "ALLOCATION_NOT_SENT": frozenset({422}),
    ...
    # `ALREADY_ACKNOWLEDGED` объявлен выше, в блоке напоминаний этапа
    # «Ознакомление» (Plane №793): второе объявление здесь молча перекрывало
    # первое. Найдено не глазами, а сторожем, заведённым по той же карточке, —
    # карточка называла только `PLACEMENT_EMPTY`, а дублей оказалось два.
    ...
    # Строка документа не совпала с формой таблицы шаблона (Plane №156, «ПД-4»):
    # значений в строке больше или меньше, чем ячеек. Молча обрезанная строка
    # означала бы документ, в котором часть данных не показана, — а это хуже
    # отказа: читатель принял бы неполную таблицу за полную.
