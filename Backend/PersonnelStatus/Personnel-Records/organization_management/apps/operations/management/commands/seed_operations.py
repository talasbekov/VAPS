...
ROLE_PERMISSIONS = {
    ...
    # 🔴 ОТДЕЛЬНЫЙ АКТОР (Plane №972, `[ШТБ-01]`–`[ШТБ-05]`): Штаб второго
    # департамента — один человек или группа С ЭТОЙ РОЛЬЮ, а не начальники
    # второго департамента (`HEAD_OPS_UNIT`) и не ответственный за сбор сил
    # (`FORCES_GATHERING_OFFICER`). Для штабной цепочки — деление потребности,
    # приёмка/возврат и довыделение, распределение по объектам, передача на
    # расстановку — достаточно `forces.command`. Штабные обходы
    # `OPS_STAFF_COMMAND` (расстановка на чужом объекте, перевод этапов,
    # сводка ГВО) этой роли автоматически НЕ выдаются: после передачи состава
    # по постам расставляет старший объекта (`[ШТБ-06]`).
    "OPS_STAFF": [
    ...
    "HEAD_OPS_UNIT": [
        ...
        "placement.manage",
        # 🔴 `forces.command` ЗДЕСЬ НЕТ — И ЭТО РЕШЕНИЕ ЗАКАЗЧИКА (Plane №972,
        # 08.09.2026: «отдельный Штаб с ролью OPS_STAFF», `[ШТБ-01]`–`[ШТБ-04]`
        # раздела 21 `RAW/README.md`). С 07.09.2026 (№944, миграция 0101)
        # право лежало в этом профиле: раздел 7.1 тогда читался так, будто
        # штаб — ОБЕ руководящие персоны второго департамента. Заказчик это
        # отменил: `acc_dir_head_d2` и `acc_dept_head_d2` — руководство, но не
        # Штаб, и «Сбор сил на ОМ» им закрыт (№939). Штаб — отдельный актор
        # `OPS_STAFF` (профиль выше). На уже засеянных базах право с профиля
        # снимает миграция 0108: сид лишнего не снимает.
        # Сводку ГВО правит и начальник УПРАВЛЕНИЯ второго департамента
        # (Plane №947, слово заказчика 07.09.2026; `[ГВО-09]` называет штабом
        # обе персоны). Право вернулось в профиль миграцией 0105 — через
        # день после того, как №601 увёз его в добавку начальника
        # департамента; для расстановки и переходов №601 остаётся в силе.
        "gvo.manage",
        # 🔴 `placement.command` и `event.stage_override` ОТСЮДА
    ...
    # 🔴 `forces.command` ОТСЮДА СНЯТ (Plane №944): спецификация `[СБС-20]`
    # даёт ответственному «входящие запросы СВОЕГО департамента; чужие не
