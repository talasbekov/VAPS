...
Отступление от дословности одно и помечено на месте: роль `OPS_READER`
(11.08.2026). В источнике чтение раздела ОМ не несёт ни одна роль, кроме
ADMIN через «*», — раздел был бы виден одному администратору.
"""
...
PERMISSIONS = [
    ...
    # Заведение карточки ОМ и заполнение бюллетеня — права ОТДЕЛЬНЫЕ от
    # `event.manage` (Plane №382, решение заказчика 02.09.2026). Заказчик
    # назвал персону, которой «всё что касается ОМ видно, но без возможности
    # редактирования или удаления, а бюллетень создавать можно». Пока и то и
    # другое охранялось одним `event.manage`, такая персона не выражалась
    # вовсе: дать ей создание значило дать правку расстановки, согласования и
    # всех переходов этапов.
    #
    # 🔴 ЭТО РАСШИРЕНИЕ, А НЕ ПОДМЕНА: оба права выданы EVENT_OFFICER рядом с
    # `event.manage`, иначе ведущий мероприятие потерял бы то, что умел вчера.
    ("event.create", "Заведение охранного мероприятия"),
    ("event.bulletin", "Заполнение бюллетеня мероприятия"),
    ...
    # роли раздела его не получают, иначе гварды стадий перестали бы
    # что-либо значить для всех, кто ведёт ОМ.
    ("event.stage_override", "Перевод ОМ на любой этап"),
    # Удаление ОМ — право ОТДЕЛЬНОЕ от event.manage по той же мерке, что
    # stage_override: ведущий мероприятие правит его, а стирает из реестра —
    # решение админа. Заведённый по ошибке бюллетень убрать было НЕЧЕМ, и
    # реестр копил мусор (188 пробных строк из 194 на 24.08.2026).
    ("event.delete", "Удаление охранных мероприятий"),
    # Правка сводных данных ГВО — право ОТДЕЛЬНОЕ от event.manage (Plane
    # «Реестр ОМ-35.6»): мероприятие ведёт один человек, а сводку визита
    # иностранного ОЛ заполняет старший ГВО, и наоборот — ведущий внутреннее
    # ОМ к сводке отношения не имеет. Роли его пока НЕ получают: раздача —
    # предмет задачи «Система. Роли и права»; у ADMIN проходит «*», а старший
    # ГВО СВОЕГО мероприятия правит сводку по роли В ДАННЫХ (см.
    # OpsGvoSummariesViewSet.permission_override).
    ("gvo.manage", "Правка сводных данных ГВО"),
    # Цепочка «Сбор сил на ОМ» (Plane №74). Права ПОРОЗНЬ, потому что порознь
    # их называл заказчик: деление потребности и приёмка списков — работа
    # штаба; оповещение управлений и отправка списка — ответственного за
    # выделение В СВОЁМ департаменте; выделение людей (оно же проставление
    # статуса «Участие в ОМ») — начальника управления по СВОЕМУ управлению;
    # расстановка по постам — старшего объекта. Одно право на всю цепочку
    # означало бы, что принявший список может выделить людей за чужой
    # департамент, — ровно то разграничение, ради которого задача и ставилась.
    #
    # ОБЛАСТЬ у права не хранится: её несёт НАЗНАЧЕНИЕ роли
    # (`UserRole.scope_division_id`), и проверка спрашивает права под
    # конкретное подразделение (`PermissionService.effective_permissions`).
    ...
    # Штаб расставляет на ЛЮБОМ объекте (`[РАС-08]`: «штаб — всё»), а не
    # только там, где он старший: `placement.manage` отвечает «умеет ли
    # расставлять», это право — «чьё угодно мероприятие». Отдельный код, а не
    # ослабление проверки старшего для всех держателей `placement.manage`.
    ("placement.command", "Расстановка на любом объекте: штаб"),
...
ROLES = [
    ...
    # ── Профили руководителей, названные заказчиком (Plane №348) ────────
    # Заказчик перечислил семь ролей ПО ВИДИМОСТИ МОДУЛЕЙ, и четыре из них в
    # каталоге не выражались ничем. Существующие роли под них не подходят и не
    # переписываются: `DIRECTORATE_HEAD` и `DEPARTMENT_EXPENSE_OFFICER` держат
    # цепочку суточного расхода и сбора сил, их права пинятся тестами ниже, и
    # снять с них `event.view` значило бы сломать рабочую модель ради стенда.
    ...
    # второго департамента. У него права обычного сотрудника и еще все что
    # касается ОМ тоже видны, но без возможности редактирования или удаление.
    # Но у него должна быть возможность создавать бюллетень.»
    ("EMPLOYEE_OPS_D2", "Сотрудник второго департамента"),
    # Роль-добавка, а не профиль: её выдают ВТОРЫМ грантом поверх профиля, с
    # областью «вся организация». Заказчик: «Категории ОМ на уровне
    # Организации, остальное на уровне своего управления» — область у гранта
    # одна, поэтому областей приходится две, и вторая живёт отдельной ролью.
    ("OM_CATEGORY_ORG", "Охранные мероприятия по всей организации"),
    # Вторая роль-добавка, и по той же причине, что и первая: у гранта область
    # ОДНА, а заказчик просит про начальника управления «остальные модули на
    # уровне своего управления ЗА ИСКЛЮЧЕНИЕМ Обзор — Обзор на уровне
    # департамента» (Plane №352, Ш-5).
    #
    # 🔴 В РОЛИ РОВНО ОДНО ПРАВО, и это не экономия. Любое второе право,
    # добавленное сюда, приедет вместе с ней НА ДЕПАРТАМЕНТ — то есть тихо
    # расширит область того, что заказчик оставил на управлении. Проба
    # `test_overview_at_department.py` держит вторую половину требования
    # (статусы остаются на управлении) и покраснеет числом, если это забыть.
    #
    # До №352 то же требование выражал признак `overview_at_department` у
    # ПОРТАЛЬНОЙ роли, и ручка статистики сама поднималась к департаменту.
    # Признак был костылём вокруг того, что у портальной роли область одна;
    # у роли раздела грантов сколько угодно, и требование выражается прямо.
    ("OVERVIEW_DEPARTMENT", "Обзор на уровне департамента"),
    # Третья роль-добавка, и заведена она НЕ ради области, а ради одного
    # держателя (Plane №601, решение заказчика 06.09.2026). Штабные права
    # («расстановка на любом объекте», «перевод ОМ на любой этап», «правка
    # сводки визита») область гранта не спрашивают вовсе: они снимают проверку
    # «своё ли это мероприятие» по бесконтекстному набору прав. Пока они жили
    # в профиле `HEAD_OPS_UNIT`, их получали ОБЕ персоны второго департамента
    # — и начальник департамента, и начальник его управления, потому что
    # набор прав у них намеренно один, а различает их только область. Итог:
    # начальник управления командовал расстановкой по ВСЕЙ организации, хотя
    # `[РАС-08]` отдаёт «всё» штабу.
    ("OPS_STAFF_COMMAND", "Штаб ОМ: команда по всей организации"),
]
...
#: же правкой.
#: Чтение раздела БЕЗ реестра мероприятий. Отдельная переменная нужна с
#: Plane №348: у половины профилей заказчика Реестр ОМ, Командный центр и
#: Транспорт ГОН недоступны все три сразу — а закрыты они одним правом
#: `event.view`. Без этого разделения «читать раздел» и «видеть мероприятия»
#: остаются одним словом, и снять второе, не сняв первое, нечем.
SECTION_READ = ("catalog.view", "personnel.view", "orgstructure.view",
                "document.view")
OPS_READ = ("event.view", *SECTION_READ)
...
ROLE_PERMISSIONS = {
    ...
    # `object.view` добавлен 31.08.2026 (Plane №348): заказчик перечислил
    # модули, недоступные сотруднику, и «Объекты и паспорта» среди них НЕ
    # назвал — а без этого права экран отвечал бы ему «Доступ закрыт». Список
    # заказчика читается как закрытый: что в нём не названо, то открыто.
    "EMPLOYEE": [
        "catalog.view", "status.view", "feedback.view", "feedback.create",
        "document.view", "object.view",
    ],
    ...
    "EVENT_OFFICER": [
        # `event.create` и `event.bulletin` стоят рядом с `event.manage`
        # намеренно (Plane №382): ведущий мероприятие заводил и заполнял
        # бюллетень одним этим правом, и разделение не должно у него ничего
        # отнять.
        "event.manage", "event.create", "event.bulletin",
    ...
    # ── Профили заказчика (Plane №348) ──────────────────────────────────
    # Ниже раскладки читаются как список ДОСТУПНЫХ модулей меню: заказчик
    # описывал профили именно так, и проверять он будет тоже так.

    # Обзор (портал, уровень департамента), Статусы своего управления,
    # Объекты, Охраняемые лица, Законы об ОМ, Аналитика ОМ, Обратная связь.
    # Нет `event.view` — значит нет Реестра ОМ, Командного центра и Транспорта
    ...
    # начальников его управлений набор ОДИН, различает их только область
    ...
    "HEAD_OPS_UNIT": [
        ...
        # роль, различаясь областью. До этого право было только у
        # `EVENT_APPROVER`, и «Согласовать» у `acc_dept_head_d2` отвечала 403.
        # Привязка строки маршрута к учётке («если в маршруте») — отдельный
        # шаг P2 (№429); пока маршрут — текст, право отвечает за роль.
        "assignment.approve", "assignment.return",
        # Штаб по всей цепочке одним сидом (Plane №421, Ш-5 плана P2): заводит
        # ОМ и бюллетень (`[БЛН-10]`), расставляет на любом объекте
        # (`[РАС-08]`), правит и утверждает сводку визита (`[ГВО-09]`).
        # `forces.command` здесь НЕТ намеренно: спецификация `[СБС-10]` отдаёт
        # заявки штабу, а матрица заказчика №348 назвала «Сбор сил»
        # недоступным начальнику второго департамента — два его решения
        # расходятся, вопрос задан в карточке №421, до ответа право не выдано.
        "event.create", "event.bulletin",
        "placement.manage",
        # 🔴 `placement.command`, `gvo.manage` и `event.stage_override` ОТСЮДА
        # УБРАНЫ (Plane №601, решение заказчика 06.09.2026) и живут в
        # роли-добавке `OPS_STAFF_COMMAND`, которую получает ТОЛЬКО начальник
        # департамента. Причина — в комментарии у этой роли: они не смотрят
        # область гранта, поэтому в общем профиле доставались и начальнику
        # управления. Что осталось здесь — права, которые область уважают:
        # `placement.manage` расставляет на СВОЁМ мероприятии, `event.create`
        # и `event.bulletin` заводят своё, `assignment.approve` /
        # `assignment.return` названы спецификацией `[СОГ-12]` у ОБЕИХ персон
        # поимённо — их разделение было бы уже другим решением.
    ],
    # Ведёт сбор сил по своему департаменту: делит потребность и оповещает
    # управления. Реестр ОМ ему при этом НЕ открыт — заказчик назвал его
    # недоступным, и звенья цепочки живут на экране «Сбор сил на ОМ».
    # Ровно категория меню «Охранные мероприятия» и ничего сверх неё: реестр
    # ОМ и Транспорт ГОН (`event.view`), Охраняемые лица и Законы об ОМ
    # (`catalog.view`), Аналитика ОМ, Отчёты по ОМ. Статусов и сбора сил здесь
    # нет намеренно — иначе второй грант расширил бы до организации и их.
    #
    # 🔴 `orgstructure.view` В РОЛИ НЕТ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ (Plane №372).
    # Роль выдаётся с областью «вся организация», а по области этого права
    # считается «Обзор». Пока право стояло здесь, третий грант перебивал
    # второй: начальник управления второго департамента видел в «Обзоре» всю
    # службу (442 штатные единицы) вместо своего департамента (197) — ровно то
    # разграничение, ради которого заказчик расписывал семь персон. Дерево
    # подразделений этой персоне открывает её ПРОФИЛЬ (`HEAD_OPS_UNIT`,
    ...
    # Ровно одно право — см. комментарий у роли в списке выше.
    "OVERVIEW_DEPARTMENT": ["orgstructure.view"],

    # Штабные права-обходы, и ТОЛЬКО они (Plane №601). Роль выдаётся вторым
    # (третьим) грантом с областью «вся организация» — честно, потому что
    # ровно это они и означают: `[РАС-08]` «штаб — всё», `[ГВО-09]` «штаб
    # правит и утверждает сводку визита», `[РЕК-03]` «„Перевести ОМ сюда“ —
    # только штаб и админ».
    #
    # 🔴 НИЧЕГО СВЕРХ ЭТИХ ТРЁХ СЮДА НЕ ДОПИСЫВАТЬ — по той же причине, что у
    # `OVERVIEW_DEPARTMENT`: роль приезжает с областью «вся организация», и
    # любое право, которое область УВАЖАЕТ, тихо расширится до организации у
    # того, кому профиль оставил департамент.
    "OPS_STAFF_COMMAND": [
        "placement.command", "gvo.manage", "event.stage_override",
    ],

    # Рядовой сотрудник второго департамента: набор `EMPLOYEE` плюс ВЕСЬ
    # раздел ОМ на чтение, плюс заведение и заполнение бюллетеня.
    #
    # 🔴 ЧЕГО ЗДЕСЬ НЕТ И ПОЧЕМУ. Нет `event.manage` — ни рекогносцировки, ни
    # расстановки, ни согласования, ни закрытия; нет `event.delete`; нет
    # `event.stage_override`. Ровно это заказчик и назвал: «без возможности
    # редактирования или удаление». Нет и `orgstructure.view` с
    # `personnel.view` — они открыли бы «Обзор» и кадровый реестр, а персона
    # остаётся рядовым сотрудником, у которого их нет (сверено с профилем
    # `EMPLOYEE`). Поэтому `OPS_READ` здесь не разворачивается: он тянет оба.
    "EMPLOYEE_OPS_D2": [
    ...
    "FORCES_GATHERING_OFFICER": [
        "status.view", "forces.command", "forces.allocate",
        "analytics.view", "analytics.drilldown", "analytics.operations",
