...
STATUS_TYPES = [
    ...
    # 🔴 УЧАСТИЕ В ОМ — ОДИН КОД (Plane №486, решение заказчика 04.09.2026:
    # «убери статусы Привлечён на мероприятие, обе»). Прежде их было два —
    # `EVENT_ASSIGNMENT` (наряд) и `EVENT_ASSIGNMENT_GROUP` (боевая группа), —
    # и деление «на группы и физически наряд» (сценарий заказчика, Plane №243)
    # жило В КОДЕ СТАТУСА. Теперь оно живёт в `participations[].kind_code`, а
    # код один; счётчик расхода «На ОМ (гр./нар.)» читает вид оттуда.
    #
    # Старые два кода из сида СНЯТЫ намеренно: на живых стендах они остаются в
    # справочнике погашенными (миграция 0091 ставит `is_active=False`), и
    # вернуть их сюда значило бы завести их на новой базе СНОВА АКТИВНЫМИ —
    # `create_defaults` ставит `is_active=True` на пути создания.
    #
    # Колонка `IN_SERVICE`: человек на мероприятии остаётся в строю (Plane
    # №169), а справочный счётчик расхода живёт вне суммы колонок и потому
    # ничего в ней не сдвигает. Приоритет 75 — тот, с которым код уже заведён
    # на стенде; менять его сидом значило бы переписать живой справочник.
    ("IN_EVENT", "Участие в ОМ", 75, "IN_SERVICE"),
