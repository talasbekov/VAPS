...
class StatusTypeSelector:
    ...
    def catalog_rows():
        ...
        `name` расходу не нужен, а в проекцию входит потому, что ЭТА проекция
        замерзает в снимке сдачи (схема 4). Без имени личная копия печатала бы
        подпись статуса из СЕГОДНЯШНЕГО словаря, и переименование типа меняло
        бы уже выданный файл. StatusCatalog лишний ключ не читает.
        """
        return list(
...
class EmployeeStatusSelector:
    ...
    def snapshot_facts_on(cls, on_date, employee_ids=None):
        ...
        `participations` — ТЕМ ЖЕ ЗАПРОСОМ, ЧТО И У РАСХОДА (Plane №751).
        Снимок самодостаточен: расход по сданному дню считается только из
        него, живых данных не читая. Значит вид занятости («наряд» или
        «боевая группа»), который после слияния №486 выводится ТОЛЬКО из
        строки участия, обязан быть заморожен вместе с фактом — иначе
        печатный документ за сданный день показывает «2 (0/0)» вместо
        «2 (1/1)». Расход это поле уже отдавал (`overlapping_on`), читатель
        документа уже переносил (`expense_document._parsed_facts`) — не хватало
        ровно этой середины.
        """
        rows = list(
            cls._live_overlapping(on_date, employee_ids).values(
...
class DailySubmissionSelector:
    ...
    @staticmethod
    def lock_day(division_id, business_date):
        """Замок ДНЯ: строка его ПЕРВОЙ версии. Возвращает её либо None.

        Замок нужен взаимному исключению поправок, и брать его на ГОЛОВЕ цепочки
        нельзя — голова движется. Тот, кто залочил версию 2, не мешает соседу,
        залочившему версию 3: строки разные, и оба спокойно посчитают один и тот
        же следующий номер.

        Первая версия для этого годится: она есть у всякого дня, который вообще
        можно поправить (сдача всегда пишет версию 1), и она НЕ ДВИЖЕТСЯ —
        поправки только добавляют строки выше. Один и тот же ряд на весь день =
        настоящее взаимное исключение.

        Строку саму по себе читать незачем: она здесь работает замком, а не
        данными. Номер следующей версии обязан читаться ОТДЕЛЬНЫМ запросом уже
        под этим замком — своим свежим снимком, иначе он посчитается от того,
        что было видно ДО ожидания.
        """
        return (
            OpsDailySubmission.objects.select_for_update()
            .filter(
                division_id=division_id, business_date=business_date, version=1
            )
            .first()
        )
...
class OpsAuditLogSelector:
    ...
    def list(
        *,
        ...
        ИДЕНТИФИКАТОР БЕЗ ТИПА НЕ ПРИНИМАЕТСЯ. entity_id — число, и нумерация
        у семи сущностей журнала своя: статус №5, вложение №5 и выпуск №5
        существуют одновременно. Отфильтруй мы по одному числу — в «ленту
        объекта» слились бы истории семи РАЗНЫХ объектов, и отличить их в
        списке нечем: события настоящие, время настоящее, актор настоящий.
        Такую ленту невозможно ни опровергнуть, ни заметить — поэтому здесь
        отказ, а не выборка.

        Правило живёт в селекторе, а не в маршруте: селектор объявлен
        единственным каналом чтения журнала, и второй читатель (команда,
        выгрузка) обязан получить тот же запрет, не переписывая его у себя.
        """
