...
Порядок — КАК В СНИМКЕ (он уже детерминирован сборкой). Своей сортировки нет
намеренно: две копии одной версии обязаны совпасть побайтно, а второй порядок
...
def _pair_rows(roster, rows):
    """[(человек | {}, статус | None)] в порядке снимка.

    Человек без статусов даёт ровно одну пару; строки на неизвестного
    сотрудника уходят в хвост с пустой записью списка.
    """
    grouped = {}
    for row in rows:
