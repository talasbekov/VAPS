...
def test_the_policy_sections_carry_the_codes_the_settings_screen_shows(seeded):
    """🔴 «ТАБЛИЦА НЕПУСТА» НЕ СТЕРЕЖЁТ РАЗДЕЛ (Plane №670, найдено ревью
    №825).

    Общий сторож проверяет только `objects.exists()`, а версий разделов
    несколько. Мутация, которая и есть предмет карточки, — убрать из
    `seed_operations` строку `("APPROVAL_POLICY", …)` — оставляла таблицу
    непустой за счёт двух других разделов, и сторож молчал. Ровно то же
    различие уже проведено у типов статусов и ролей наряда двумя пробами
    выше: пустота не единственный способ соврать, важен СОСТАВ.

    Коды берутся те же, что перечисляет экран настроек (`sections` в
    `app/security-ops/settings/page.tsx`): раздел без своей версии показывает
    администратору карточку без версии и без единой настройки — задать порог
    автосрочности ему нечем.
    """
    codes = set(OpsPolicySectionVersion.objects.values_list("section_code", flat=True))
    assert {"CONFLICT_RULES", "PASSPORT_FRESHNESS", "APPROVAL_POLICY"} <= codes, sorted(
        codes
    )
