...
def test_schema_describes_a_page_and_only_get():
    ...
    Две ловушки разом. Первая: у plain ViewSet spectacular выводит страничную
    обёртку только у GenericAPIView, поэтому без объявленной руками схемы
    ответ был бы описан голым массивом, которого клиент никогда не получит.
    Вторая: у действия с именем `list` эвристика заворачивает объект-страницу
    ЕЩЁ и в массив, если many=False не стоит на КЛАССЕ сериализатора.
