...
class Clock:
    ...
    def to_local(value: datetime) -> datetime:
        """ЧУЖОЙ момент в локальной зоне раздела (Plane №696).

        `local_now()` отвечает про «сейчас», а печатать надо и хранимые
        отметки: время подтверждения ознакомления, дату замечания, момент
        записи в журнал. Голый `value.astimezone()` для этого НЕ ГОДИТСЯ — он
        берёт зону операционной системы, а не `TIME_ZONE` проекта, и на
        сервере в UTC каждая отметка в документе смещалась бы на пять часов.
        Локально дефект невидим: машина разработки уже +05.

        Наивное значение возвращается как есть: приписать ему зону значило бы
        сдвинуть неизвестный момент на неизвестную величину. Такие отметки
        приходят из JSON-полей мероприятия, где зоны не было и на записи.
        """
