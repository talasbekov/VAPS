...
def generate_expense_pdf(data):
    ...
    # splitInRow — не украшение, а условие того, что документ вообще соберётся.
    # Ячейка статуса несёт до двадцати имён с хвостом, и одна такая СТРОКА
    # оказывается выше листа. reportlab по умолчанию строку между страницами не
    # разрывает и в этом случае не «печатает как получится», а ПАДАЕТ
    # (LayoutError) — то есть расход с людным нарядом не выпустился бы вовсе.
    # Найдено не рассуждением: врезка канона порядка сменила состав видимых
    # двадцати на чуть более длинные имена, ячейка перенеслась на строку выше и
    # сборка упала.
    table = Table(table_rows, repeatRows=1, splitByRow=1, splitInRow=1)
    table.setStyle(
        TableStyle(
            [
