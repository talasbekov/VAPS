...
_DICTIONARIES = (
    ...
    # Роли наряда в расстановке (Plane №237). Состав значений взят ИЗ БЛАНКА
    ...
    "EVENT_GROUP_ROLES",
    # Секции бланка «Общая расстановка» — вторая координата места (Plane №242,
    # решение заказчика 30.08.2026). Роли одной не хватает: «Көшпелі күзетінің
    # жауаптысы» есть у восьми выездных охран подряд, и система знала роль, но
    # не знала, чьего кортежа. Состав НЕ придуман и не ведётся руками — он
    # читается из шаблона (`documents_placement_full.template_sections`) и
    # обновляется командой `sync_placement_sections`.
    "PLACEMENT_SECTIONS",
)
