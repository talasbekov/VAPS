...
class _AppendOnly(TimeStampedModel):
    ...
    #: Админка показывает такие строки ТОЛЬКО НА ЧТЕНИЕ (Plane №671).
    #:
    #: Флаг объявлен здесь, а не проверкой `issubclass(_AppendOnly)` в
    #: `admin_auto`: тот модуль общий для всех приложений и знать про реестр
    #: сбора сил не обязан. Модель же про себя знает — и говорит об этом одним
    #: словом, которое читается там же, где написано правило `save`.
    #:
    #: Почему только чтение, а не «починить сохранение»: строка реестра не
    #: правится ПО ОПРЕДЕЛЕНИЮ — довыделение, новый срок, новый ответ
    #: департамента пишутся НОВОЙ строкой. Форма, которая «сохраняет» такую
    #: строку, обязана была бы завести новую, то есть делать не то, что
    #: написано на кнопке. Единственное изменяемое поле (`removed_at` у
    #: состава) меняет сервис, и смысл у него «исключён из состава», а не
    #: «поправлен».
    admin_readonly = True
