...
ЧТО ПОЧИНЕНО В ОБРАТНОМ ХОДЕ 05.09.2026 (Plane №753): синтетическое участие
«мероприятие неизвестно», заведённое прямым ходом, теперь СНИМАЕТСЯ. Оставляя
его, откат записывал вид дважды — и в коде статуса, и в строке участия, — а
повторный прямой ход спотыкался об ограничение уникальности пары (статус,
мероприятие).

ОСТАВШАЯСЯ ДЫРА, названная вслух: `backwards` переписывает в снятые коды ВСЕ
строки `IN_EVENT` с известным видом, включая заведённые цепочкой уже ПОСЛЕ
слияния. Отличить их нечем — метки «эту строку перевело слияние» в данных
нет; заводить её ради отката отдельное решение (карточка заведена).
"""
...
# «Мероприятие неизвестно». Значение продублировано числом, а не взято из
# `models_status`: миграция обязана читаться и работать так же, когда код
# уедет вперёд.
UNKNOWN_EVENT_ID = 0
...
def backwards(apps, schema_editor):
    ...
    for status in Status.objects.filter(status_type_code=TARGET).iterator():
        ...
        status.save(update_fields=["status_type_code"])
        # 🔴 СИНТЕТИЧЕСКУЮ СТРОКУ ПРЯМОЙ ХОД ЗАВЁЛ — ОБРАТНЫЙ ОБЯЗАН СНЯТЬ
        # (Plane №753). Без этого откат оставлял участие «мероприятие
        # неизвестно» у статуса, который снова несёт вид В СВОЁМ КОДЕ: тот же
        # факт записан дважды, а повторный прямой ход спотыкается об
        # ограничение уникальности (статус, мероприятие).
        status.participations.filter(
            event_id=UNKNOWN_EVENT_ID, kind_code__in=LEGACY_CODE_BY_KIND
        ).delete()
