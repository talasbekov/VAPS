...
разошлась: потребитель слушал `user_<id>`, а `signals.py` (снят по №866) слал в
...
def _without_comments(text: str) -> str:
    ...
    был выписан комментарием в `signals.py` (файл снят по №866) и цитировал
...
def test_no_module_builds_the_group_name_by_hand():
    ...
    # 🔴 ГРАНИЦА СУЖЕНА (ревью №825 по №917, 08.09.2026): голый `["']user_`
    # ловил ЛЮБУЮ строку с этим префиксом — `"user_id"`, `"user_role"`,
    # `'user_type'` в обычном поле или ключе словаря, никак не связанную с
    # адресацией группы. Предмет остаётся «литерал, а не форма склейки», но
    # сузился до самих guilty-форм: голый `"user_"` сразу перед закрывающей
    # кавычкой (склейка плюсом/`.join`) либо `user_` с форматированием внутри
    # той же строки (`%s`, f-строка `{`) до закрывающей кавычки. Полное слово
    # вида `user_id` ни тем, ни другим не является — оно проходит чисто.
    handmade = re.compile(r"""["']user_(?:["']|[A-Za-z0-9_]*[%{])""")
    offenders = [
