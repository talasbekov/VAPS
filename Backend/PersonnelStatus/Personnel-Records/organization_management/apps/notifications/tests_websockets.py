...
async def test_notification_is_broadcast_to_user():
    ...
    адрес стоял в `signals.py`, откуда уходили уведомления о смене статуса.
    Разобрано это отдельной карточкой (№866): обработчики не подключались ни
    разу и сняты по решению заказчика 06.09.2026, а `signals.py` вместе с
    ними. Здесь проверяется ПОТРЕБИТЕЛЬ, и брать он обязан свой собственный
    адрес, а не тот, по которому ошибочно писал издатель.
