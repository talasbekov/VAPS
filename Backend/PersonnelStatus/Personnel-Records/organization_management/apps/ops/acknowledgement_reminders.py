...
def _unconfirmed(event):
    """Неподтвердившие: без отметки И без отказа (Plane №884).

    🔴 ОТКАЗАВШИЙСЯ — НЕ НЕПОДТВЕРДИВШИЙ. Здесь стояло одно условие
    (`acknowledgedAt is None`), и в список «кому звонить за час до
    заступления» попадал человек, уже сказавший «не могу заступить».
    Начальник тратил бы час на того, кого положено ЗАМЕНИТЬ, а не
    уговаривать.

    Правило модуля не новое и не выведено здесь: соседний
    `acknowledgement_stage._pending` держит его прямым текстом с №616
    («отказавшийся — не „ожидает“, ему напоминать нечего, его заменяют»),
    и одиночная ручка напоминания отбивает отказ ошибкой `ALREADY_DECLINED`
