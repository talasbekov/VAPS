...
class RegistryReadContext:
    ...
    def __init__(self):
        ...
        #: Справочник {id подразделения: имя} целиком — одна загрузка на
        #: ответ, как `_division_children` (Plane №1031).
    ...
    def division_names(self, division_ids):
        """{id подразделения: имя} — ОДИН запрос на весь справочник, а не на
        строку (Plane №1031), тем же правилом, что и `division_children_map`.

        `_merge_status_members` спрашивает имя подразделения добавленного
        статусом человека НА КАЖДОЙ строке реестра, где такой человек
        нашёлся. Кеш ПО ID, наполняемый по ходу, здесь не годится: строки
        реестра сериализуются последовательно, и подразделение первой строки
        ещё не известно, когда пришла вторая, — прирост остаётся линейным,
        только реже. `Division` — как и весь справочник структуры — читается
        сюда ВЕСЬ и один раз, как `DivisionTreeSelector.children_map()`.
        """
        if self._division_names is None:
            from organization_management.apps.divisions.models import Division

            self._division_names = dict(Division.objects.values_list("pk", "name"))
        return {
            int(value): self._division_names.get(int(value), "")
            for value in division_ids
    ...
    def prime_employees(self, employee_ids):
        ...
        if not missing:
            return
        # `rank` и `staff_unit__position` присоединены тем же запросом ради
        # `denorm_for` ниже (Plane №1031): она читает те же карточки, что уже
        # тянет `divisions_of`, и вторым отдельным select_related платить не
        # за что — джойн лишних колонок дешевле второго запроса.
        for employee in Employee.objects.filter(pk__in=missing).select_related(
            "staff_unit__division", "staff_unit__position", "rank"
        ):
    ...
    def divisions_of(self, employee_ids):
        """{employee_id: division_id} по штатным единицам — порт
        `StaffUnitSelector.divisions_of` на уже собранные карточки
        (Plane №1031).

        Сотрудник без штатной единицы отсутствует в ответе — тот же
        fail-closed выбор, что и у селектора: молчаливо приписывать область
        значило бы решить за вызывающего. `staff_unit` — обратный OneToOne
        (`Employee.staff_unit`), его отсутствие бросает исключение, а не
        отдаёт `None`, — тем же правилом, что уже держит `_employee_division`.
        """
        from organization_management.apps.employees.models import Employee

        result = {}
        for key, employee in self.employees(employee_ids).items():
            try:
                staff_unit = employee.staff_unit
            except Employee.staff_unit.RelatedObjectDoesNotExist:
                continue
            if staff_unit is not None and staff_unit.division_id is not None:
                result[int(key)] = staff_unit.division_id
        return result

    def denorm_for(self, employee_ids):
        """{employee_id: {"full_name", "rank", "position_level"}} — порт
        `EmployeeSelector.denorm_for` на уже собранные карточки (Plane №1031).

        Читает РОВНО те поля, что и селектор (`rank__name`,
        `staff_unit__position__level`), только из карточки, которую страница
        реестра уже подняла для `divisions_of`/справочника — вторым запросом
        на строку не платит.
        """
        from organization_management.apps.employees.models import Employee
        ...
        for key, employee in self.employees(employee_ids).items():
            ...
            try:
                ...
                staff_unit = None
            position_level = (
                staff_unit.position.level
                if staff_unit is not None and staff_unit.position_id is not None
                else None
            )
            result[int(key)] = {
                "full_name": " ".join(part for part in parts if part).strip(),
                "rank": employee.rank.name if employee.rank_id else "",
...
def _merge_status_members(event, rows, *, read_context=None):
    ...
    # Подразделение — через контекст страницы, если он собран (Plane №1031):
    # `read_context` уже держит карточку сотрудника (`prime_employees`), и
    # второй запрос за тем же фактом на каждую строку реестра не нужен.
    division_of = (
        read_context.divisions_of(extra_ids)
        if read_context is not None
        else StaffUnitSelector.divisions_of(extra_ids)
    )
    # Имя подразделения — ОДНИМ запросом (или из кеша страницы, Plane №1031).
    # Первая версия оставляла его пустым, и в карточке заявки у всех, кто
    # попал в список статусом, в колонке «Подразделение» стоял прочерк: экран
    # знал id и не знал названия.
    if read_context is not None:
        division_names = read_context.division_names(division_of.values())
    else:
        from organization_management.apps.divisions.models import Division
    ...
    # зависящее от числа людей, — ровно того раздел избегает везде. Через
    # контекст страницы (Plane №1031) — та же карточка, что уже подняли для
    # `division_of` выше, второго запроса за тем же именем не нужно.
    names = {
        employee_id: row.get("full_name", "")
        for employee_id, row in (
            read_context.denorm_for(extra_ids)
            if read_context is not None
        ...
        view.append(
            {
                **row,
                ...
                # 🔴 «ОТКРЫЛ И НЕ НАЖАЛ» (`[ОЗН-02]`, Plane №452). Ставится в
                # `my_assignments.mark_viewed` при первом чтении человеком
                # СВОЕГО списка. Отдаётся ЯВНО, а не самотёком через `**row`:
                # у строк, заведённых до №452, ключа нет вовсе, и старший
                # должен получать `null` («не открывал»), а не отсутствие поля,
                # по которому клиент угадывал бы разницу.
                "viewedAt": row.get("viewedAt"),
                # ☎ В СТРОКЕ (`[ОЗН-03]`, Plane №452). Считается НА ЧТЕНИИ по
                # той же причине, что подразделение и статус выше: номер
                # меняется мимо мероприятия, и копия, записанная в момент
...
def actor_display_name(actor):
    ...
    обещала идентификатор, а часть вьюх передаёт `actor=request.user`.
    `str(User)` даёт username, `.isdigit()` ложь, и функция возвращала
    username как есть: в поле «кем создана версия» документа «Расстановка
    сил» — того самого, который подписывают и рассылают, — стояло `admin`
    вместо фамилии. Ровно та болезнь, от которой докстрока и защищает, только
    с другого конца.

    Чинится ЗДЕСЬ, а не в вызовах: с конвенцией функция разошлась сама. Одна
    ветка `isinstance` дешевле дюжины правок и не оставляет следующему вызову
    шанса разойтись снова.

    🔴 `actor=request.user` — МЕНЬШИНСТВО, А НЕ КОНВЕНЦИЯ (уточнено ревью
    №825). Здесь стояло «это ОБЩАЯ конвенция вьюх, больше десятка мест», и
    следующий читатель принял бы это за разрешение писать так дальше. Счёт по
    `api/views.py`: `actor=resolve_actor_id(request)` — 35 мест,
    `actor=request.user` — 10. Общая конвенция — ИДЕНТИФИКАТОР; принимать
    объект функция обязана потому, что десять мест уже так зовут, а не
    потому, что так правильно. Аудиту от `request.user` достаётся username в
    поле `actor_user_id` — это отдельный дефект, заведён карточкой.

    🔴 УВОЛЕННЫЙ ПОДПИСЫВАЕТСЯ ФАМИЛИЕЙ, А НЕ ЛОГИНОМ. Фильтра `is_active` у
    поиска записи нет НАМЕРЕННО — в отличие от `_find_personnel`, который
    отвечает на другой вопрос («кто вправе действовать»). Здесь вопрос
    «как звали того, кто это сделал», и увольнение не должно превращать
    подпись под уже подписанным документом в `admin`. Прежняя докстрока
    обещала «живую» запись и расходилась с кодом — обещание снято.
    """
    if actor is None:
        return ""
    ...
    # `get_user_model()`, а не прямой импорт `auth.models.User`: от этого
    # имени зависит теперь не запрос, а ОПОЗНАНИЕ ТИПА — переопредели проект
    # `AUTH_USER_MODEL`, и `isinstance` тихо перестал бы срабатывать, вернув
    # логины вместо фамилий. Ошибка импорта была бы заметнее, но её не будет.
    User = get_user_model()
...
def remove_placement_post(event_id, post_id, *, deputy=None):
    ...
    # 🔴 ЗАМЕЧАНИЯ СНЯТОГО ПОСТА НЕ ИСЧЕЗАЮТ И НЕ ЗАПИРАЮТ ЭТАП (Plane №510).
    # Раньше пост уходил, а замечание к нему оставалось «Открыто» со ссылкой
    # на несуществующий пост: метка «!N» пропадала вместе с постом, на экране
    # замечание не показывалось — а `_approval_ready` держал согласование
    # ровно по нему. Этап не завершался НИКОГДА, и причина была невидима.
    #
    # Замечание становится ОБЩИМ по объекту (`postId = None` — штатная форма,
    # `[МД-07]`), а имя снятого поста уезжает в `detachedPost`: слова
    # согласующего не переписываются, но и контекст не теряется. Закрывать
    # чужое замечание за согласующего система не имеет права — это его
    # суждение; она обязана лишь оставить его ВИДИМЫМ и отвечаемым.
    _detach_remarks_of_post(event, post_id, post)
...
def complete_placement(
    ...
    # 🔴 ЭТАП ОБЪЕКТА, А НЕ ТОЛЬКО МЕРОПРИЯТИЯ (Plane №508). Завершение
    # расстановки МЕНЯЕТ этап объекта, а сторожило только этап ОМ — а он с
    # №412 держится МИНИМУМОМ по объектам. У ОМ, где объект А уже согласован и
    # ушёл на «Ознакомление», а объект Б ещё расставляется, `event.stage`
    # остаётся «Расстановкой», и повторное «Завершить расстановку» по объекту А
    # проходило гвард: `advance_visits` писал ему «Согласование» БЕЗУСЛОВНО.
    #
    # Чем это било: согласованный (а тем же путём и ЗАКРЫТЫЙ) объект
    # откатывался назад, а его `approval_status`/`approval_snapshot`
    # оставались «Согласовано» — состояние, из которого система себя не
    # выведет. Плюс на каждый повтор заново писался аудит «завершено с
    # недобором». Достижимо не только по API: выборщик объекта перечисляет ВСЕ
    # объекты независимо от их этапа, и экран шлёт выбранный.
    #
    # Из всех операций расстановки эта была ЕДИНСТВЕННОЙ без проверки объекта:
    # соседи (назначение, перенос, снятие, старший сектора) идут через
    # `_require_placement_editable`. Здесь нужен не он, а прямой гвард: тот
    # отвечает на вопрос «заморожена ли расстановка ПОСТА», а вопрос здесь —
    # «на своём ли этапе объект».
    _require_stage(
        event,
        "PLACEMENT",
    ...
    _require_visit_stage(
        visit,
        "PLACEMENT",
        "Расстановка этого объекта уже завершена — этап у него дальше.",
    )
...
#
# ЗАМОРОЗКА — по СТАТУСУ ДОКУМЕНТА объекта: назначение, снятие и смена
# старшего сектора отбиваются, пока документ отправлен или согласован. Так
# покрываются и «на согласовании» (`[СОГ-07]`: после отправки форма только для
# чтения), и «согласовано» (`[СОГ-04]`); возврат и отзыв возвращают документ в
# черновик и тем же правилом размораживают. Гвард ищет объект ПО ПОСТУ — у
# назначения адреса объекта нет, а пост свой объект знает.
#
# 🔴 ЗДЕСЬ СТОЯЛО «ПО СТАДИИ ОБЪЕКТА», И ЭТО ПЕРЕЖИЛО СВОЙ ПРЕДМЕТ (найдено
# ревью №825). Абзац объяснял, почему заморозка начинается «с „Согласования“ и
# дальше», — а `placement_frozen` тридцатью строками ниже начинается со слов
# «🔴 НЕ ПО ЭТАПУ ОБЪЕКТА» (Plane №533). Читатель раздела получал два
# взаимоисключающих правила, и первым шло снятое.
#
# 🔴 Довод, ради которого ключ и меняли, остался верным и переехал сюда:
# запирать РАННИЕ стадии нельзя. Первая версия правила ключилась на стадию и
# запирала «Рекогносцировку» с «Бюллетенем» — полный прогон дал 27 красных:
# старые пробы и сокращённые фикстуры расставляют людей до завершения осмотра.
# Спецификация замораживает ДОКУМЕНТ, а до отправки документа нет. Ключ на
# статус документа это выражает прямо, а ключ на стадию — только приблизительно
# (и ошибался: завершение расстановки уводило объект на «Согласование», где
# документ ещё черновик, — оператор запирал собственную работу, Plane №533).
# Decisions за 03.09.2026, №398.
...
def placement_frozen(visit):
    ...
    🔴 ЗАКРЫТЫЙ ОБЪЕКТ ЗАМОРОЖЕН НЕЗАВИСИМО ОТ ДОКУМЕНТА (найдено ревью №825).
    Ключ на статус документа слабее прежнего ключа на стадию в одну сторону:
    объект, доведённый до «Проведения» или ЗАКРЫТЫЙ с документом-черновиком,
    снова становился правимым — `placement/assign|unassign|senior` пропускали
    его, потому что других гвардов у них нет. Достижимо это админским обходом
    этапов (`override_stage` двигает стадии, документов не трогая), но
    `[ЗАК-05]` не оговаривает способ: «после закрытия изменения невозможны».
    Стадия здесь не заменяет статус документа, а дополняет его вторым, более
    поздним основанием.
    """
    if visit.stage == "CLOSED" or visit.closed_at is not None:
        return True
...
def _require_placement_editable(event, post_id):
    ...
    visit = _visit_of_post(event, post_id)
    if visit is not None and placement_frozen(visit):
        _refuse_frozen_placement(visit, what="Расстановка объекта")
...
def document_version_diff(previous, current):
    ...
    def posts_of(snapshot):
        # 🔴 ПОДПИСЬ — ОБЩАЯ (Plane №875, найдено ревью №865). Здесь стояла
        # ТРЕТЬЯ копия склейки «сектор · пост», и она расходилась с общей на
        # трёх входах из четырёх: у поста без номера давала «Периметр · », без
        # сектора — « · Пост 3», а без обоих — « · », то есть разделитель без
        # сторон. Общая `post_label` в этих случаях печатает то, что осталось,
        # а в последнем — идентификатор: бесполезно человеку, но честнее
        # пустоты. Читает эту строку человек — в истории версий документа
        # (`[ВОЗ-06]`) и в деле согласования.
        #
        # На СРАВНЕНИЕ версий подпись не влияет: ключ здесь — идентификатор
        # поста, подпись только печатается. Проверено чтением обоих
        # использований ниже, а не предположением.
        return {
            str(p.get("id")): post_label(p)
...
# Версия, по которой согласующие УЖЕ высказались: её нельзя переписать
# отправкой — под ней подписались (или её вернули с причиной), и оба факта
# принадлежат истории. Отправка поверх такой версии открывает СЛЕДУЮЩУЮ.
# Черновик и «на согласовании» решения не несут: первая отправка и повтор
# того же состава остаются в своём номере.
_DECIDED_DOCUMENT_STATUSES = ("APPROVED", "RETURNED")


def _submit_document_version(event, visit, *, actor=None):
    """Отправка: черновик → «на согласовании» тем же номером; поверх решённой
    версии (согласована или возвращена) — новая версия N+1, прежняя помечена
    отменённой.

    🔴 СОГЛАСОВАННАЯ ВЕРСИЯ ПЕРЕКРЫВАЕТСЯ, А НЕ ПРАВИТСЯ (Plane №534). Ветка
    ловила только `RETURNED`, и повторная отправка УЖЕ СОГЛАСОВАННОГО объекта
    правила его строку НА МЕСТЕ: статус «Согласовано» затирался на «На
    согласовании», а `decided_at` оставался от прежнего решения — момент
    согласования, которого больше нет. Запись о согласовании уничтожалась —
    ровно противоположное тому, что обещает «Все версии хранятся, отменённые
    помечены». Дойти до этого можно на ОМ с двумя объектами: этап мероприятия
    держится на «Согласовании» вторым объектом, и `_require_stage` отправку
    первого пропускает.

    Правило спецификации `[СОГ-04]` для обоих решённых статусов ОДНО: «После
    согласования версия замораживается: правка невозможна; любое изменение =
    новая версия → повторное согласование». Возврат — частный случай этого
    правила, а не единственный.
    """
    now = Clock.now()
    current = _ensure_document_version(event, visit, actor=actor)
    if current.status in _DECIDED_DOCUMENT_STATUSES:
...
def _is_urgent(event, explicit, *, urgent_days=None):
    ...
    🔴 ОКНО ОГРАНИЧЕНО С ОБЕИХ СТОРОН (Plane №681). Условие было
    односторонним — «осталось не больше порога», — и потому истинным для ВСЕХ
    ПРОШЕДШИХ дат: у вчерашней разница −1, у прошлогодней −365, и обе «не
    больше суток». Написано это было для замечаний согласования, где прошедшая
    дата возникнуть не может: замечание ставят на живом мероприятии. Доска
    сбора сил (`force_collection_board.is_urgent`) зовёт ту же функцию по
    КАЖДОЙ строке листинга, а листинг исключает только закрытые — и каждый
    незакрытый сбор прошлого месяца получал красный бейдж «Срочно» и по
    `sort_key` вставал ВЫШЕ сегодняшних действительно срочных.
...
def new_remark(
    ...
    return {
        ...
        # Пост, к которому замечание было привязано, ПОКА он был в расчёте
        # (Plane №510). Пусто — привязки не теряли. Заполняется при снятии
        # поста: `postId` тогда обнуляется, и без этого поля замечание
...
def _returned_order(visit):
    """Ключ сортировки «когда объект вернули последний раз».

    🔴 ВРЕМЯ БЕРЁТСЯ У ВЕРСИИ ДОКУМЕНТА, А НЕ У МАРШРУТА (Plane №491). В
    маршруте отметка `decidedAt` есть только у решения согласующего, а возврат
    приходит ДВУМЯ путями — решением в маршруте и ручкой `approval/return/`, —
    и у второго строки `RETURNED` в маршруте не появляется вовсе. Общее у
    обоих одно: `_return_visit` помечает текущую версию документа статусом
    `RETURNED` и ставит ей `decided_at`. Это и есть «когда вернули».

    Не `updated_at` объекта: его двигает любая правка, включая соседнюю, и на
    вопрос «когда вернули» он не отвечает.

    Объекты без отметки (строки, заведённые до появления таблицы версий —
    бэкфилла у неё нет намеренно) сортируются РАНЬШЕ отмеченных: известное
    время честнее неизвестного.

    🔴 СТРОКИ БЕЗ ОТМЕТКИ ОТБРАСЫВАЮТСЯ, А НЕ СОРТИРУЮТСЯ ПЕРВЫМИ (найдено
    ревью №825). `order_by("-decided_at")` в PostgreSQL кладёт NULL ВПЕРЁД:
    у объекта, возвращённого дважды, старая строка `RETURNED` без отметки
    (её заводит `_ensure_document_version` у данных до №0073) побеждала
    свежую датированную, функция возвращала «время неизвестно» — и объект
    уезжал в самый низ сортировки. То есть в ветке старых строк, которую этот
    же докстринг обещает обслуживать, выбор переворачивался, и мероприятие
    снова несло чужую причину: дефект №491, выживший внутри своей починки.

    Отбор `decided_at__isnull=False` честнее любого `nulls_last`: строка без
    отметки на вопрос «когда вернули» не отвечает вовсе, и участвовать в
    сравнении времён ей нечем.
    """
    row = (
        visit.document_versions.filter(status="RETURNED", decided_at__isnull=False)
        .order_by("-decided_at", "-number")
        .first()
    )
    if row is None or row.decided_at is None:
        return (0,)
    return (1, row.decided_at.timestamp())
...
def _sync_event_approval(event):
    ...
    согласованы все; иначе ожидание.

    🔴 «ПОСЛЕДНИЙ ВОЗВРАЩЁННЫЙ» — ПО ВРЕМЕНИ РЕШЕНИЯ, А НЕ ПО ПОРЯДКУ ОБЪЕКТОВ
    (Plane №491). Здесь стояло `returned[-1]` над списком, отсортированным по
    `position`, — то есть «последний» означало «нижний в списке», вопреки этой
    же докстроке. У ОМ с двумя возвращёнными объектами поле мероприятия несло
    причину того из них, кто стоит ниже, и она могла быть СТАРШЕ той, что
    человек только что получил.

    Момент берётся у версии документа, помеченной возвратом
    (`_returned_order`): это единственная отметка, которую ставят ОБА пути
    возврата — и решение согласующего в маршруте, и ручка `approval/return/`.
    """
    visits = list(event.visit_objects.order_by("position", "pk"))
    if not visits:
        return
    ...
    if returned:
        ...
        comment = max(
            returned,
            key=lambda v: (_returned_order(v), v.position, v.pk),
        ).approval_comment
...
def _require_own_posts(event, visit, rows, *, single=None):
    ...
    неверный тип `remarks` (№668).

    🔴 ПОЛЕ НАЗЫВАЕТСЯ СО СТРОКОЙ, А ID В ТЕКСТ НЕ ПОПАДАЕТ (найдено ревью
    №825). Ошибка клалась в поле `remarks` — имя КОЛЛЕКЦИИ без номера, — и
    окно возврата не могло подсветить строку: `humanizeFieldPath` подменяет
    имя коллекции подписью, только если следом идёт цифра, а голое `remarks`
    отдаётся как есть. Человек читал «remarks: Пост не принадлежит объекту:
    post-4f2a9c1b8e30» — то есть машинный ключ и СЫРОЙ ID ПОСТА, ровно тот, из
    за которого карточка №506 и заведена. Теперь адрес — `remarks.<номер>`
    (так же делает сосед по файлу `_incoming_remarks`), а текст называет
    правило, а не идентификатор.
    """
    own = {
        str(p.get("id"))
        for p in visit_object_posts(event, visit, single=single)
    }
    field_errors = {}
    for index, row in enumerate(rows):
        ...
        if post_id in (None, ""):
            continue
        if str(post_id) not in own:
            field_errors[f"remarks.{index}"] = [
                "Пост не принадлежит согласуемому объекту."
            ]
    if field_errors:
        raise _validation(
            field_errors,
...
def decide_approver(
    ...
    event = lock_event(event_id)
    visit = _approval_target(event, visit_object_id)
    # 🔴 РЕШАЮТ, ПОКА ОБЪЕКТ НА СВОЁМ ЭТАПЕ (Plane №568). Гварда здесь не было
    # вовсе, и это была АСИММЕТРИЯ: соседи (`return_placement`,
    # `approve_placement`, `withdraw_from_approval`) этап спрашивают, а
    # решение согласующего — нет. Ветка `APPROVED` защищена случайно
    # (автозавершение выходит само), а `RETURNED` — ничем.
    #
    # Чем это било: повторное «Вернуть» ПОСЛЕ завершения этапа откатывало
    # объект с «Ознакомления» обратно на «Расстановку» — и мероприятие вместе
    # с ним, потому что его стадия наименьшая. Люди уже получили уведомления о
    # заступлении, документ согласован, а карточка внезапно снова просит
    # расставлять. Повтор — не редкость: двойной клик, ретрай сети, вторая
    # вкладка согласующего.
    _require_visit_stage(
        visit,
        "APPROVAL",
        "Решение согласующего принимается только на этапе «Согласование».",
    )
    ...
    if decision == "APPROVED":
        ...
        # 🔴 ТРЕТИЙ ВХОД, ГДЕ ПОДПИСЬ ОСТАВАЛАСЬ (доводка №513/№583 по ревью
        # №825). Решение по УЖЕ РЕШЁННОЙ строке здесь не запрещено —
        # проверяется только `NOT_SENT`, а очередь смотрит лишь предыдущие
        # строки. Значит согласующий, который подписал, может нажать
        # «Вернуть»: статус станет `RETURNED`, а реквизиты подписи останутся —
        # и `_return_visit` их не снимет, он чистит только `APPROVED`/`PENDING`
        # и до собственной строки возвращающего не доходит. Получалась строка
        # «Возвращено» с надписью «Согласовано … версия N · хэш» — тот же
        # `[СОГ-10]`-дефект, из третьей двери. Повтор решения здесь не
        # экзотика: соседний комментарий сам называет двойной клик, ретрай
        # сети и вторую вкладку.
...
def _unattributed_posts(event):
    ...
    return [
        p for p in (event.recon_sector_posts or [])
        if str(p.get("visitObjectId") or "") == ""
    ]


def _refuse_empty_placement(event, visit):
    """Отказ отправке, когда подпись расстановки пуста.

    🔴 ДВЕ РАЗНЫЕ БЕДЫ, И ЧИНЯТСЯ ОНИ ПО-РАЗНОМУ (Plane №477). «Расстановка
    пуста» верно, когда посты у объекта есть, а людей на них нет. Но у ОМ с
    НЕСКОЛЬКИМИ объектами, посты которых не размечены `visitObjectId`,
    `visit_object_posts` честно отдаёт пусто — неразмеченная строка не
    принадлежит никому, приписать её объекту значило бы выдумать факт, — и
    подпись пуста при полностью укомплектованной расстановке.

    Абсурд, который из этого выходил: на «Согласование» такое ОМ провёл САМ
    сервер (`complete_placement` проверил укомплектованность каждого поста),
    а дальше сам же отказывался работать — со словами «согласовывать нечего»,
    которые уводили разбор в расстановку, где всё на месте. Разметка делается
    на рекогносцировке, двумя этапами назад, и человек, читая этот отказ, туда
    не пойдёт: ему сказали не про то.

    Такие ОМ не выдумка: неразмеченные посты остаются после миграции 0069 —
    единственному объекту она посты приписала, а у нескольких приписывать
    было не к чему.
    """
    orphans = _unattributed_posts(event)
    if orphans and event.visit_objects.count() > 1:
        raise DomainError(
            "RECON_POSTS_UNASSIGNED",
            422,
            detail={
                "visitObjectId": str(visit.pk),
                "unattributedPosts": len(orphans),
            },
            message=(
                "Посты расчёта не отнесены к объектам посещения: у "
                f"мероприятия несколько объектов, а у {len(orphans)} "
                "постов объект не указан — согласовывать нечего, потому что "
                "неизвестно, чьи это посты. Разметьте посты по объектам на "
                "рекогносцировке (администратор возвращает этап обходом), "
...
def send_for_approval(event_id, *, visit_object_id=None, object_lead=None, actor=None):
    ...
    if scoped_assignments == "":
        _refuse_empty_placement(event, visit)
    for item in route:
    ...
    #
    # 🔴 ВЕРСИЯ ЗАВОДИТСЯ ДО СМЕНЫ `approval_status` (доводка №584 по ревью
    # №825). Присвоение `PENDING` стояло ВЫШЕ, и у объекта БЕЗ строки истории
    # версий это ломало нумерацию: `_ensure_document_version` выводит статус
    # первой строки из `visit.approval_status`, и вместо `RETURNED`/`APPROVED`
    # он видел уже переписанный `PENDING` при непустом снимке → заводил строку
    # `SUBMITTED`, а `_submit_document_version` переписывал её на месте вместо
    # N+1 с отметкой «отменена». Объекты без строки версии — не теория:
    # бэкфилла у этой таблицы нет НАМЕРЕННО (см. `_ensure_document_version`),
    # то есть это ровно те старые данные, ради которых заведён весь заход.
    ...
    # 🔴 ОБЪЕКТ ВОЗВРАЩАЕТСЯ НА «СОГЛАСОВАНИЕ» (Plane №534 + №568; тупик найден
    #    ревью, задача №825). Отправка принимает ДВА этапа — согласованный
    #    объект стоит на «Ознакомлении», а `[СОГ-04]` требует, чтобы новая
    #    версия ушла на повторное согласование. Но сам этап объекта при этом
    #    не менялся, и после того как №568 закрыла решение согласующего
    #    гвардом «только на этапе „Согласование“», повторно отправленный объект
    #    становился ТУПИКОМ: версия `SUBMITTED`, весь маршрут `PENDING`, а
    #    решить, вернуть, отозвать и завершить расстановку — все четыре ручки
    #    отвечают 422 по этапу; расстановку вдобавок держит заморозка №533.
    #    Выйти можно было только админским `override_stage`.
    #
    #    Лечение — здесь, а не расширением гварда: этап обязан означать то, что
    #    он означает. Объект ЖДЁТ решения — значит он на «Согласовании», и все
    #    четыре ручки снова согласны между собой.
    advance_visits(event, "APPROVAL", visits=[visit])
    record_object_lead_action(event, visit, object_lead, "approval_send")
    return event


def _unsend_document_version(visit):
    """Вернуть текущую версию документа в черновик (Plane №536).

    Номер НЕ меняется: отзыв возможен только пока НИКТО НЕ ПОДПИСАЛ, значит
    отозванная версия ничьим решением не закреплена и «двум составам один
    номер» здесь не выдаётся — выдавался бы, если бы отзыв был возможен после
    подписи, а он запрещён отдельно (`APPROVAL_WITHDRAW_AFTER_SIGN`). Меняется
    только то, что отзыв и означает: статус `DRAFT`, отметка отправки снята.
    Решённую версию (согласована или возвращена) не трогаем: отзыв после
    решения запрещён выше, а история решений принадлежит тем, кто их принял.

    Подпись и снимок строки НЕ чистятся, и это осознанно (уточнено ревью
    №825): их безусловно перезаписывает следующая отправка
    (`_submit_document_version`, ветка недорешённой версии), а в промежутке
    они — единственное, с чем `document_version_diff` может сравнить
    следующую версию. Пустая строка сделала бы разницу «первой версией», то
    есть скрыла бы правку, ради которой отзыв и делали.
    """
    current = _current_document_version(visit)
    ...
    current.save(update_fields=["status", "sent_at", "updated_at"])
...
def withdraw_from_approval(event_id, *, visit_object_id=None, object_lead=None):
    ...
    Номер версии тоже остаётся — почему именно, разобрано ниже.

    🔴 ДОКУМЕНТ ВОЗВРАЩАЕТСЯ В ЧЕРНОВИК, И БЕЗ ЭТОГО ОТЗЫВ БЫЛ МЁРТВ (Plane
    №536). Раньше отзыв сбрасывал ТОЛЬКО статусы маршрута: этап оставался
    «Согласованием», документ — «На согласовании», и расстановка оставалась
    замороженной. Править нельзя, а вернуть документ некому — весь маршрут уже
    в «Не отправлено». Отозвавший упирался в стену, которую сам же и поставил;
    `test_ops_withdraw_rule` этого не видел, потому что стерёг одни статусы
    маршрута.

    Отзыв — это «не отправляли»: документ снова черновик, отметка отправки
    снимается, снимок состава стирается. Прежний разбор говорил «снимок
    остаётся: отзыв не меняет расстановку» — снимок и не про расстановку, а
    про то, ЧТО ОТПРАВИЛИ; после отзыва не отправлено ничего, и сравнивать с
    ним «устарела ли расстановка» значит сравнивать с призраком.

    🔴 И ОБЪЕКТ ВОЗВРАЩАЕТСЯ НА «РАССТАНОВКУ» (найдено ревью №825). Без этого
    отзыв размораживал документ и оставлял человека там, где править нечем:
    карточка ОМ рисует панель ПО ЭТАПУ объекта, на «Согласовании» это
    `ApprovalStage`, а в нём нет ни одной операции расстановки; уйти на чужой
    шаг по `?step=` можно только с правом `event.stage_override`. То есть
    «расстановка разморожена» было правдой для API и неправдой для человека:
    он видел, как гаснет кнопка «Отозвать», и упирался в ту же стену, только
    молча. Движение то же, что у возврата согласующим (`_return_visit`), и
    довод тот же: наименьшая стадия объектов сама возвращает мероприятие.

    🔴 НОМЕР ВЕРСИИ ПРИ ПОВТОРНОЙ ОТПРАВКЕ ОСТАЁТСЯ ТЕМ ЖЕ — И ПОСЛЕ ПРАВКИ
    СОСТАВА ТОЖЕ. До ревью здесь стояло обоснование «версия уже уходила
    людям», и оно спорило с абзацем выше («после отзыва не отправлено
    ничего»): из первой посылки следует беречь отправленное, из второй —
    стирать. Верна вторая, и она же согласуется с `[СОГ-04]`: замораживает
    версию СОГЛАСОВАНИЕ, а отзыв возможен только пока никто не подписал.
    Значит отозванная версия ничьим решением не закреплена, N+1 ей не нужен,
    и повторная отправка перезаписывает подпись и снимок в той же строке.
    Закреплено пробой: отзыв → другой состав → тот же номер и НОВАЯ подпись.
    """
    event = lock_event(event_id)
    visit = _approval_target(event, visit_object_id)
    _require_visit_stage(
    ...
    # Объект возвращается туда, где расстановку правят руками (см. докстроку).
    old_stage = event.stage
...
def override_stage(event_id, *, stage, actor):
    ...
    #
    # 🔴 «УЖЕ ТАМ» СЧИТАЕТСЯ ПО ОБЪЕКТАМ, А НЕ ПО МЕРОПРИЯТИЮ (Plane №478).
    # Шорткат написан во времена, когда этап был один на мероприятие; теперь
    # `event.stage` — МИНИМУМ по объектам, и равенство ему перестало значить
    # «все на месте». Объект А на «Согласовании», объект Б на «Расстановке» —
    # `event.stage` «Расстановка», и обход на «Расстановку» отвечал успехом,
    # не делая НИЧЕГО: А оставался согласуемым.
    #
    # Чем это било: обход этапа — последнее средство, к нему идут, когда всё
    # остальное встало (см. №475, где карточка запиралась целиком). Именно в
    # этот момент он молча не срабатывал, а ответ говорил «сделано».
    if old_stage == stage and all(visit.stage == stage for visit in visits):
        return event
    ...
    for visit in visits:
        ...
        #
        # 🔴 СМОТРИМ НА ЦЕЛЕВОЙ ЭТАП, А НЕ НА ПРЕЖНИЙ ЭТАП МЕРОПРИЯТИЯ (Plane
        # №527). Условие стояло `old_stage == "CLOSED"`, то есть штамп
        # снимался, только если было закрыто ВСЁ мероприятие. Но
        # `close_visit_object` умеет закрыть ОТДЕЛЬНЫЙ объект при мероприятии
        # на «Проведении» — и после обхода на живой этап получался объект со
        # стадией `PLACEMENT` и непустым `closed_at`: «закрыт и одновременно в
        # работе». Экраны и отчёты читают эти два поля НЕЗАВИСИМО, и каждый
        # верит своему.
        #
        # Правило простое: живой этап и штамп закрытия несовместимы. А все
        # цели обхода — живые: `CLOSED` в `STAGE_OVERRIDE_TARGETS` нет вовсе,
        # и закрыть карточку обходом нельзя (закрытие — решение со сводкой и
        # комментарием, его принимает человек, а не админский перевод).
        # Поэтому штамп снимается БЕЗУСЛОВНО: условие `stage != "CLOSED"`
        # было бы всегда истинным, то есть враньём о наличии выбора.
        visit.closed_at = None
        # 🔴 И ИТОГ ЗАКРЫТИЯ — ТОЖЕ (доводка №527 по ревью №825). Штамп
        # снимался, а `closing_comment`, поставленный `close_visit_object`,
        # оставался: `closingComment` отдаётся полем API БЕЗУСЛОВНО, и у
        # живого объекта в нём стоял итог закрытия. Сегодня экраны печатают
        # его только при `stage == "CLOSED"`, а дело согласования — только при
        # непустом `closed_at`, то есть наружу не вылезало; но поле врало, и
        # следующий читатель напечатал бы «итог закрытия» на открытом объекте.
        #
        # У МЕРОПРИЯТИЯ решено ИНАЧЕ и осознанно (ниже: «Итоги направлений при
        # этом ОСТАЮТСЯ»): там это собранный факт — сводка по направлениям,
        # которую заполняли люди. Здесь другое: одна строка, которую написал
        # закрывающий В МОМЕНТ закрытия. Закрытия больше нет — нет и его
        # комментария; сохранять его значило бы приписывать живому объекту
        # решение, которое отменено.
        visit.closing_comment = ""
        visit.save(
            update_fields=["stage", "closed_at", "closing_comment", "updated_at"]
        )
    event.stage = stage
    event.readiness_percent = STAGE_READINESS[stage]
    ...
    # `kind` определяется сравнением индексов, и равенство даёт FORWARD).
    # Аудит при этом пишется ВСЕГДА: обход условий — решение человека, и по
    # нему разбираются поимённо, состоялось движение этапа или нет.
    if old_stage != stage:
