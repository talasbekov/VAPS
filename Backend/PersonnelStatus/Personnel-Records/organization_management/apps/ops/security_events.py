...
def _unsend_document_version(visit):
    ...
    Номер НЕ меняется: версия уже уходила людям. Меняется только то, что
    отзыв и означает, — она больше не на согласовании: статус `DRAFT`, отметка
    отправки снята. Решённую версию (согласована или возвращена) не трогаем:
    отзыв после решения запрещён выше, а история решений принадлежит тем, кто
    их принял.
    """
    current = _current_document_version(visit)
...
def withdraw_from_approval(event_id, *, visit_object_id=None, object_lead=None):
    ...
    Номер версии тоже остаётся: она уже уходила людям, и «откатить» его значило
    бы выдать двум разным составам один номер.
