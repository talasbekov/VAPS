...
class SecurityEventViewSet(RequirePermissionMixin, viewsets.ViewSet):
    ...
    permission_map = {
        ...
        # «Мои назначения» (Plane №403): свои — любой вошедший с ЖИВОЙ
        # кадровой привязкой (уволенному наряд закрыт, №596), подчинённого —
        # по области `status.manage`; `event.view` открывает всех (штаб,
        # админ).
    ...
    def _my_assignments_override(self, request):
        ...
        Чтение: без параметра `employee` — свои строки (нужна кадровая
        привязка И `is_active`: с №596 уволенный получает пустой ответ со
        СВОЕЙ причиной, а не сведения о наряде, к которому больше не имеет
        отношения; отсутствие привязки — тоже не 403, а пустой ответ с
        причиной); с параметром — чужие, по области `status.manage` актора.
