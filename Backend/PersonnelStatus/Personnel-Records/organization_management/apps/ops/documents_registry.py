...
def list_formats():
    ...
    заказчика 26-27.08.2026). DOCX-шаблоны и конвертер LibreOffice остаются
    внутри `render()` (нужны для сборки PDF) — из каталога снят только выбор,
    предлагаемый ЧЕЛОВЕКУ; было (до №986) — DOCX первым и PDF, потому что
    образцы заказчика — рабочие бланки Word."""
    return [
