...
def test_personnel_snapshot_shape(manager):
    ...
    # 🔴 `callsign` ДОПИСАН В ПИН ОСОЗНАННО (Plane №456), а не подогнан под
    # новый вывод: позывной приезжает снимком с этой задачи, и его отсутствие
    # в форме означало бы, что клиент о нём не узнает. Пустая строка — «не
    # вписан»; у сотрудника пробы позывного нет, и это ровно тот случай, что
    # у всех записей, заведённых до миграции 0003.
    assert data["results"] == [
        {
            "id": str(employee.pk),
            "name": "Абенов С.",
            # Полное имя и путь подразделения — Plane №1247 (пин поправлен
            # осознанно): однофамильцев по инициалам и отделу не различить.
            "fullName": "Абенов Серик",
            "unitPath": "Отдел охраны объектов",
            "rankLabel": "Майор",
            "callsign": "",
...
def test_the_closure_summary_says_unknown_where_the_placement_does(manager):
    ...
    for visit in visits:
        ...
        # Замены и инциденты считаются по ЗАПИСЯМ ЖУРНАЛА этого объекта
        # (Plane №727): ссылка на пост прямая, и размеченные посты известны
        # даже тогда, когда про остальное молчим. У этой фикстуры журнала нет
        # вовсе — ноль здесь означает «записей нет», а не «неизвестно».
