...
def test_every_seeded_permission_has_at_least_one_catalog_row():
    """У каждого сеяного права есть строка каталога — или именная причина
    (Plane №901).

    🔴 СТОРОЖ КЛАССА, А НЕ ОДНОГО СЛУЧАЯ. №602 закрыл один экземпляр беды
    «гейт мимо `require_permission` каталогу не виден», и в том же файле
    осталось ещё несколько: четыре вьюсета рейтинга без миксина (право
    проверяет сервис), расширяющие права аналитики, отзывов и отчётов,
    заниженный гейт заведения значения справочника. Каждый раз экран «Права»
    отвечал администратору одно и то же — «право не стоит ни на одной
    ручке» — про право, которым закрыта настоящая работа.

    Пока проверялись отдельные случаи, следующий экземпляр появлялся тем же
    способом. Здесь проверяется ВЕСЬ список сеяных прав разом, и новое право
    без строки каталога краснит пробу в тот же день, когда его завели.

    Исключения — ИМЕННЫЕ и с причиной: «список того, чего нет» без объяснения
    через полгода читается как «эти права сломаны, но чинить не стали».
    """
    from organization_management.apps.ops import access_catalog
    from organization_management.apps.operations.management.commands import (
        seed_operations,
    )

    #: Права, которых в каталоге НЕТ ПО УСТРОЙСТВУ.
    allowed = {
        # Wildcard: не право ручки, а «все права разом» у ADMIN. Строки
        # каталога у него быть не может — он не стоит нигде и открывает всё.
        "*": "wildcard администратора, не гейт конкретной ручки",
        # Старый RBAC штатного расписания (`common/rbac.py`): карта
        # «действие → право» вне DRF-вьюсетов, каталог обходит URL-резолвер и
        # до неё не достаёт. Переезд — отдельная работа, здесь названо, чтобы
        # отсутствие не читалось как дефект.
        "orgstructure.manage": "старый RBAC штатного расписания вне DRF-вьюсетов",
    }

    seeded = {code for code, _label in seed_operations.PERMISSIONS}
    in_catalog = set(access_catalog.catalog().keys())
    missing = seeded - in_catalog - set(allowed)

    assert not missing, (
        "права сеются, но каталог их не показывает — экран «Права» скажет "
        f"администратору «не стоит ни на одной ручке»: {sorted(missing)}"
    )
    # И обратная сторона: исключение, которое ПЕРЕСТАЛО быть нужным, тоже
    # дефект — список исключений врёт про систему.
    stale = set(allowed) & in_catalog
    assert not stale, (
        f"право попало в каталог, но всё ещё числится исключением: {sorted(stale)}"
    )
