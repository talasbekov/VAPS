...
// Динамика оперативного рейтинга (§19.20). Экран НЕ считает ни одного
// значения: точки, версии методики и границы приходят готовыми; единственная
// геометрия здесь — перевод присланного числа в координату. Рядом с графиком —
// таблица тех же точек: значение только по наведению недоступно с клавиатуры,
// а tooltip обязателен как носитель версии методики.
...
import type { RatingDynamicsPoint } from "@/entities/operational-rating";
...
/** Подпись точки — она же tooltip (§19.20 «Tooltip с версией policy»). */
...
export function RatingDynamicsSection() {
  ...
  return (
    <section
      ...
      aria-labelledby="rating-dynamics-title"
    >
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Только закрытые периоды и только серверные агрегаты. Отдельные
            оценки на графике не показываются.
          </p>
        </div>
        {data !== undefined && (
          <div className="flex items-center gap-2">
            <label
              htmlFor="rating-dynamics-employee"
      ...
      {data !== undefined && !data.capabilities.operationalRatings && (
        <p className="text-sm">
          Оперативный рейтинг выключен сервером — ряда точек нет. Это не «ноль
          по всем периодам»: агрегаты не рассчитывались.
        </p>
      )}
      ...
      {data !== undefined &&
        ...
        data.points.length === 0 && (
          <p className="text-sm">
            Закрытых периодов ещё нет — строить график не по чему.
          </p>
        )}
          ...
          <>
            ...
            >
              ...
              {data.boundaries.map((boundary) => {
                ...
                const x =
                  (xAt(index - 1, data.points.length) +
                    xAt(index, data.points.length)) /
                  2;
                return (
