    ...
    <Card data-slot="evaluation-panel">
      ...
      <CardContent className="space-y-4">
        ...
        {!rights.canManage ? (
          ...
          //
          // 🔴 ТЕКСТ — КАНОНИЧЕСКИЙ, А НЕ СВОЙ (доводка №913 по ревью №825).
          // Панель стояла ДО общего блока `AccessHints` (строка 101) и
          // объясняла то же самое право СВОИМИ словами — тот самый «частокол
          // формулировок» под одну причину, который №913 обещал закрыть, но
          // не увидел здесь. Дедуп текстом не достаёт: панель далеко от
          // общего блока, и своя строка ей всё равно нужна (тот же довод,
          // которым сам блок объясняет, почему он не наверху этапа) — но
          // СЛОВА теперь одни и те же везде на экране.
          <p
            id={lockedHintId}
