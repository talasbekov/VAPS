        ...
        >
          ...
          <div className="flex gap-2">
            ...
            {/* 🔴 ПРИЧИНА — ВИДИМОЙ СТРОКОЙ, А НЕ `title` (правило №801,
                ...
                `AcknowledgementStage`) переведены на `RightGate` раньше; этот
                остался, хотя №635 переписала сам текст причины.
                🔴 `ConductStage` здесь СТОЯЛ В СПИСКЕ И СТОЯЛ ЗРЯ (снято ревью
                №825): его кнопки закрытия чинились по №777 вручную и ни одного
                `RightGate` не содержат. Перевод на общую обёртку — отдельная
                карточка. */}
