...
export function BulletinPanel({
  ...
  //
  // 🔴 Ветка «раскрыть, если не заполнен» снята (Plane №748/№468): именно она
  // отжимала работу вниз на «Расстановке» и стала поводом убрать панель с
  // этапов совсем — а вместе с ней исчез единственный редактор описания и
  // задач. Оба решения совместимы: панель доступна везде, но сама лезет на
  // глаза только там, где её заполняют. Незаполненный бюллетень при этом не
  // теряется: раскрыватель называет его состояние строкой заголовка.
  const [open, setOpen] = useState(awaitingRecon);
      ...
      <CardContent className="p-0">
        ...
        <div className="flex items-center gap-2 pr-4">
          ...
          {/* Ссылка на карточку визита стоит НА БЮЛЛЕТЕНЕ (Plane №193 —
              место выбрал заказчик) и ТОЛЬКО ссылкой (`[ГВО-03]`, Plane
              №441): сводка на этапах не разворачивается, у визита своя
              страница (`[ГВО-01]`, №436). Внутреннему ОМ ссылка не нужна —
              визита у него нет; `kind === null` (ОМ до появления типа) —
              ссылка остаётся: скрывать по незнанию нельзя. */}
          {event.kind !== "INTERNAL" && (
            <Link
...
function EventFacts({ event }: { event: SecurityEvent }) {
  ...
  return (
    <section className="rounded-md border bg-muted/30 p-3">
      ...
      {/* Сводка живёт на СТРАНИЦЕ ВИЗИТА (`[ГВО-01]`/`[ГВО-03]`, Plane №436 и
          №441): ссылка ведёт туда, панели в карточке ОМ больше нет. */}
