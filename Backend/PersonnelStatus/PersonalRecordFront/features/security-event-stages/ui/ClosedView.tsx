            ...
            {event.visitObjects.map((visit) => {
              ...
              return (
                <li key={visit.id}>
                  ...
                  {/* ПРИЛОЖЕНИЕ ОБЕЩАЕТСЯ ТОЛЬКО К ОТПРАВЛЕННОМУ ДОКУМЕНТУ
                      (Plane №732). Вторая половина строки печаталась
                      безусловно, и у объекта, чья расстановка не
                      отправлялась, выходило «… · не отправлялась · лист
                      ознакомления в приложении — в деле (PDF выше)»: архив
                      утверждал про приложение к документу, которого нет.
                      Лист ознакомления — приложение К РАССТАНОВКЕ, и без неё
                      его негде приложить.

                      🔴 «СУЩЕСТВУЕТ» ≠ «ОТПРАВЛЕН» (доводка №732 по ревью
                      №825). `_ensure_document_version` заводит версию 1
                      ЧЕРНОВИКОМ (`sentAt: null`) ещё ДО первой отправки —
                      объект, снятый на «Проведение»/«Закрыто» админским
                      обходом этапа (`override_stage`) без единой настоящей
                      отправки, имел бы `current` truthy и ту же ложь про
                      приложение, которую эта карточка чинит. `sentAt`
                      отвечает на настоящий вопрос — отправляли или нет. */}
                  {current?.sentAt != null
                    ? ` · версия ${current.number} · лист ознакомления в приложении — в деле (PDF выше)`
                    : " · не отправлялась"}
                </li>
              );
            })}
