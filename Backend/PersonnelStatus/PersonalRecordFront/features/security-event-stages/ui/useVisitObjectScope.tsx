...
/**
 * Есть ли на самом деле, что показывать под «Не отнесены к объекту»
 * (Plane №488).
 *
 * 🔴 ЗАЧЕМ ОТДЕЛЬНОЕ ПРАВИЛО. `__unassigned__` — законное значение `?visit=`:
 * его пишет сам выборщик объекта и его пересылают ссылкой. Но законно оно
 * ровно в одном случае: неразмеченные строки ЕСТЬ и объектов БОЛЬШЕ ОДНОГО.
 * У единственного объекта неразмеченные строки — его (так считает и сервер), а
 * когда неразмеченных строк не осталось, показывать нечего вовсе.
 *
 * Чем это било, пока правила не было. Значение проходило все проверки «жив ли
 * выбор» и не чинилось никогда, а потребители писались под другой случай — «у
 * ОМ нет объектов вовсе» — и получали «объекта нет» на мероприятии, где
 * объекты есть. Отсюда три разных симптома одной причины: на этапе проведения
 * молча исчезали панели оценки и закрытия объекта; на согласовании показывался
 * маршрут первого объекта, а кнопки уходили на сервер без адреса и получали
 * отказ; шапка на ОМ с одним объектом уверяла «показаны строки, не отнесённые
 * ни к одному объекту», рисуя при этом ВСЕ строки. Выйти можно было только
 * правкой адресной строки руками.
 *
 * Правило названо ОДИН раз и читается и хуком, и страницей: два ответа на
 * вопрос «показывать ли ничейные» разошлись бы при первой же правке — именно
 * так этот дефект и появился.
 */
export function unassignedIsMeaningful(
  visitObjects: { id: string }[],
  rows: { visitObjectId?: string | null }[]
): boolean {
  if (visitObjects.length < 2) return false;
  return rows.some((row) => (row.visitObjectId ?? null) === null);
}
...
export function useVisitObjectScope(
  ...
  //
  // 🔴 «НЕ ОТНЕСЕНЫ» ТОЖЕ МОЖЕТ УКАЗЫВАТЬ В ПУСТОТУ (Plane №488). Раньше это
  // значение считалось живым ВСЕГДА, поэтому не чинилось никогда: сняли
  // последнюю неразмеченную строку — пункта в списке уже нет, а выбор остался,
  // и этап показывал «объекта нет» на мероприятии, где объекты есть.
  const alive =
    wanted === UNASSIGNED_VISIT
      ? unassignedIsMeaningful(event.visitObjects, rows)
