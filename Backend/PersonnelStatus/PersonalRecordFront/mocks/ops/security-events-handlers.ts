...
/** Старший объекта СИДА — из кадрового списка мока, а не литералом (Plane
 *  №633). Пара `id + имя` берётся из одной строки ростера: две строки рядом
 *  уже расходились однажды, и разойтись им нечему только тогда, когда
 *  источник один. */
const SEED_CHIEF = PERSONNEL_ROSTER.find((person) => person.id === "emp-9");
    ...
    visitObjects: [
      {
        ...
        // старшего» сверяет `visit.chiefEmployeeId` со строками кадрового
        // списка и не подсвечивало НИКОГО как текущего — «уже старший» не
        // появлялось, и его можно было «назначить» повторно.
        //
        // Комментарий тут раньше стоял в двух взаимоисключающих строках
        // («не назначен» и «назначен» подряд). Верна вторая: старший у
        // объекта СИДА есть, без него рекогносцировка закрыта (№424).
        //
        // 🔴 У ЗАВЕДЁННОГО ОМ СТАРШИЙ НАСЛЕДУЕТСЯ ОТ ФОРМЫ, А НЕ БЕРЁТСЯ
        // ОТСЮДА (Plane №633, найдено ревью №825). `emptyEvent` — фабрика и
        // сида, и всякого нового мероприятия, а сервер по №190 ставит объекту
        // старшего, ВЫБРАННОГО В ФОРМЕ, или `null`. Пока константа стояла
        // здесь безусловно, мок-стенд отвечал двумя неправдами: заведи ОМ с
        // другим старшим — объект возвращался с Ахметовой, которую никто не
        // выбирал; заведи БЕЗ старшего — объект получал его всё равно, и оба
        // мок-гварда «назначьте старшего» на первом объекте не срабатывали
        // никогда. Наследование дописывает обработчик создания.
        //
        // Пара `id + имя` берётся ИЗ РОСТЕРА, а не пишется литералами: две
        // строки рядом уже расходились однажды, и разойтись им нечему только
...
export const securityEventsHandlers = [
  ...
  http.post(`*${SECURITY_EVENTS_PATH}`, async ({ request }) => {
    ...
    // 🔴 СТАРШИЙ НАСЛЕДУЕТСЯ ОБЪЕКТОМ ПОСЕЩЕНИЯ (Plane №190, доведено ревью
    // №825). Сервер ставит первому объекту старшего мероприятия либо `null`;
    // мок оставлял объекту константу сида, и заведённый ОМ возвращался со
    // старшим, которого человек не выбирал, а «заведён без старшего» в моке
    // не воспроизводился вовсе.
    if (created.visitObjects[0] !== undefined) {
      created.visitObjects[0].chiefEmployeeId = chief === null ? null : chief.id;
      created.visitObjects[0].chiefName = chief === null ? "" : chief.name;
    }
  ...
  http.post(
    ...
    async ({ params, request }) => {
      ...
      const assignment: PlacementAssignment = {
        ...
        // Старший ПОСТА назначается отдельной ручкой; поставленный на пост
  ...
  // Старший ПОСТА (Plane №65 «Р-4» → `[РАС-03]`, №445): один на ПОСТ, как у
  // сервера. Шапка говорила «один на сектор» и расходилась с кодом прямо под
  // собой — он снимает признак у назначений того же ПОСТА (Plane №780).
  http.post(
