...
# 🔴 СВОЙ КАТАЛОГ МЕТОК — С ПЕРВОЙ ПРОВЕРКИ, А НЕ С РАЗДЕЛА 3a (Plane №945).
# Подставной сервер 111 «слушает» :3106 — тот же порт, что у живого dev-стенда,
# и пока каталог меток был настоящим (`/tmp`), проба «владелец не назвался»
# находила метку ЖИВОГО стенда и краснела ровно тогда, когда стенд поднят, —
# то есть всегда во время работы. `gate:front` на этом останавливался и не
# доходил до прод-сборки; сессии либо гоняли сборку руками, либо читали
# красный гейт как свою поломку. Каталог — временный на весь прогон, и в
# `/tmp` проба больше не возвращается: живой машины она касаться не должна
# нигде.
# Боевой дефолт каталога меток пинится ДО подмены (ревью №825 по №945):
# пока проба ходила в настоящий `/tmp`, опечатка в дефолте сторожа ломала бы
# её сама; теперь это единственное место, где дефолт вообще проверяется.
echo "neighbours.sh:"
check "боевой каталог меток по умолчанию — /tmp" "/tmp" "$STAND_OWNER_DIR"
OWNER_DIR=$(mktemp -d) || { echo "  ✘ mktemp -d не удался — пробе негде держать метки"; exit 1; }
STAND_OWNER_DIR="$OWNER_DIR"
trap 'rm -rf "$OWNER_DIR"' EXIT
...
# Каталог меток — временный, заведён в шапке пробы (№945).
