...
/* 🔴 КОНСТАНТА КОДА УЧАСТИЯ СНЯТА ОТСЮДА (Plane №754). Она называлась
   `EVENT_ASSIGNMENT_CODE`, после слияния №486 держала значение `IN_EVENT` и не
   имела НИ ОДНОГО потребителя: разрез считает занятость по общему набору
   `EVENT_PARTICIPATION_STATUS_CODES`, который импортирован выше. Оставленная,
   она обещала третий источник правды о коде участия — рядом с общим набором и
   серверным `PARTICIPATION_STATUS_CODES`. */
