...
function SecurityEventScreen() {
  ...
  return (
    <DashboardLayout>
      ...
      <Card className="mb-4">
        <CardContent className="p-4">
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-1.5 text-xs">
            ...
            {/* «Карточка визита →» ВЕРНУЛАСЬ СЮДА (Plane №468). Она жила в
                шапке панели бюллетеня (№193, `[ГВО-03]`/№441), а из этой
                строки была снята намеренно («Реестр ОМ-35.8»: две ссылки на
                одно место рядом сбивают). С №468 панель на этапах не
                рисуется — и вместе с ней пропал ЕДИНСТВЕННЫЙ вход в визит из
                карточки ОМ: в приложении на него ссылались только две строки,
                обе внутри панели. Показывается ровно там, где панели нет, —
                иначе на «Бюллетене» ссылок снова стало бы две.
                Внутреннему ОМ визита нет; `kind === null` (ОМ до появления
                типа) ссылку сохраняет: скрывать по незнанию нельзя. */}
            {event.kind !== "INTERNAL" && event.stage !== "BULLETIN" && (
              <Link
      ...
      {/* Бюллетень — ТОЛЬКО на стадии «Бюллетень» (Plane №468, решение
          заказчика 04.09.2026). Прежде панель стояла над этапами и была видна
          на всех пяти: на «Расстановке» она к тому же САМА раскрывалась (пока
          бюллетень заполнен не полностью) и отжимала работу вниз — с этого и
          началась карточка.

          Отменяет часть решения от 24.08.2026 («Реестр ОМ-4»), по которому
          панель и переехала наверх. Сведения, ради которых её туда поднимали,
          на этапах остаются в шапке выше: код, стадия, готовность, название,
          даты, ответственный, объект, объект посещения со старшим и лицом,
          цепочка шагов.

          Что с экранов этапов УХОДИТ вместе с панелью и куда делось: тип
          мероприятия, локация/адрес, факты ГВО, краткое описание и первичные
