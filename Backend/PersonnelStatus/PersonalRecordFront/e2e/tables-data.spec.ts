...
test.describe(LIVE ? 'таблицы: правда в колонках' : 'таблицы (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('«Участие в ОМ» ведёт на разрез сбора сил и не выдаёт себя за адрес события', async ({
    ...
    // 🔴 ПИН ПОДПИСИ ИЗМЕНЁН ОСОЗНАННО (Plane №274, Ш-5). Старый текст —
    // «Статус не хранит связь с конкретным ОМ» — перестал быть правдой ещё в
    // Ш-3: `ops_status_participations` хранит мероприятия статуса, и по
    // несколько сразу. Проба пинила ЛОЖНОЕ утверждение и тем закрепляла его.
    // Новый текст ничего не утверждает о модели: карточка участия пока не
    // показывает (кадровая ручка отдаёт `current_status` без участий —
    // отдельная карточка), и подпись честно говорит, ГДЕ их видно.
    const CAPTION = 'Мероприятия участия видны в разрезе «Сбор сил»'
