...
//
// `placement-stage.spec.ts` — ДВА неproверенных `placement/assign/` в
// помощниках `assignTo` (строки ~1223 и ~1415, форма
// `const assignTo = async (...) => request.post(...)` без проверки ответа).
// Найдены усилением сторожа 06.09.2026 (задача №825). Файл в этот момент
// ДЕРЖИТ соседняя сессия — у неё в нём большая незакоммиченная правка по
// №822, — и заходить в чужой файл значило бы унести её работу своим коммитом.
// Отдано владельцу отдельной карточкой; строка снимается вместе с починкой.
const KNOWN_SILENT: readonly string[] = ['placement-stage.spec.ts']
...
function checkedByHelper(lines: string[], index: number): boolean {
  ...
  while (end < lines.length - 1) {
    ...
    if (line.trim() !== '' && indentOf(line) <= base && !/^\s*[)}\]]/.test(line)) break
    end += 1
  }
