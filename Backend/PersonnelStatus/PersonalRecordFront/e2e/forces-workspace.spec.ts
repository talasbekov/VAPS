...
test.describe('рабочие места сбора сил', () => {
  ...
  test('ответственный попадает на рабочий стол и выбирает отдельные процессы', async ({ page }) => {
    ...
    await expect(nav.getByRole('link', { name: 'Ежедневный расход', exact: true })).toHaveCount(1)
    await expect(nav.getByRole('link', { name: 'Сбор сил на ОМ', exact: true })).toHaveCount(1)
    ...
    await expect(nav.getByRole('link', { name: 'Сбор сил на ОМ', exact: true })).toHaveAttribute('aria-current', 'page')
    await page.reload()
  ...
  test('штаб попадает в распределения', async ({ page }) => {
    ...
    await expect(page.locator('aside').getByRole('link', { name: 'Распределения', exact: true })).toHaveCount(1)
    await expect(page.locator('aside').getByRole('link', { name: 'Сбор сил на ОМ', exact: true })).toHaveCount(0)
    await expect(page.getByRole('navigation', { name: 'Рабочее место' })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Ежедневный расход организации' })).toHaveCount(0)
  })

  test('старая ссылка свода открывает расход с сохранённой датой', async ({ page }) => {
    await signIn(page, 'acc_forces_officer')
    await page.goto(`${APP}/employees?view=department-summary&businessDate=2026-09-10`)
    await expect(page.getByRole('heading', { name: 'Ежедневный расход', exact: true })).toBeVisible({ timeout: 30_000 })
    await expect(page.locator('aside').getByRole('link', { name: 'Ежедневный расход', exact: true })).toHaveAttribute('aria-current', 'page')
    await page.reload()
    await expect(page).toHaveURL(/businessDate=2026-09-10/)
