...
test.describe('склонение по числу', () => {
  ...
  test('сводный запрос департаменту называет ОБА числа и состав (Plane №922)', () => {
    // 🔴 ЗАЧЕМ ОБА ЧИСЛА В ЗАГОЛОВКЕ. Ответственный за департамент получает
    // ОДНО письмо на все свои управления заявки — потому что ключ уведомления
    // «одно на человека в день» иначе оставил бы ему правду про ОДНО
    // управление из нескольких. Значит письмо обязано сказать и сколько
    // людей, и по скольким управлениям: «выделите 3» не говорит, что их надо
    // разложить на два управления, а «по 2 управлениям» не говорит сколько.
    const row = (need: number, count: number): OpsNotification => ({
      id: 1,
      recipient: '5',
      kind: 'FORCES_REQUEST_DEPARTMENT',
      business_date: '2026-09-05',
      payload: {
        need,
        directorateCount: count,
        directorates: [
          { divisionId: '11', name: 'Первое управление', need: 2 },
          { divisionId: '12', name: 'Второе управление', need: 1 },
        ],
        eventCode: 'ОМ-2026-1',
        eventTitle: 'Проба',
        businessDate: '2026-09-05',
    ...
    // Дательный падеж, а не именительный: «по 2 управления» — согласованное
    // число при несогласованном падеже, и читается как недописанная строка.
    expect(describeOpsNotification(row(3, 2)).title).toBe(
      'Выделите 3 сотрудников по 2 управлениям на ОМ-2026-1',
    )
    expect(describeOpsNotification(row(1, 1)).title).toBe(
      'Выделите 1 сотрудника по 1 управлению на ОМ-2026-1',
    )
    expect(describeOpsNotification(row(5, 5)).title).toBe(
      'Выделите 5 сотрудников по 5 управлениям на ОМ-2026-1',
    )
    // Состав — в подписи поимённо: ответственный решает, откуда брать людей,
    // прямо в ленте, не открывая карточку.
    expect(describeOpsNotification(row(3, 2)).message).toContain(
      'Первое управление — 2, Второе управление — 1',
    )
  })
