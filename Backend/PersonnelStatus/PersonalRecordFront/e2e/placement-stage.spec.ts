...
test.describe(LIVE ? 'расстановка' : 'расстановка (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('отмена обоснования возвращает человека на прежний пост, а не теряет его', async ({
    ...
    try {
      ...
      const dialog = page.getByRole('dialog')
      await expect(dialog).toBeVisible({ timeout: 15_000 })
      // 🔴 СВОЙ ТАЙМАУТ, А НЕ УМОЛЧАНИЕ (Plane №798). У `expect` в конфиге
      // порога нет, значит действует умолчание Playwright — пять секунд, — а
      // окно обоснования рисуется ПОСЛЕ ответа сервера на перенос: под
      // нагрузкой (прогон пачкой, стенд занят соседней сессией) текст успевает
      // прийти позже. Соседняя проба этого же файла давно ждёт двадцать пять
      // секунд появления строки и пятнадцать — этого же текста; здесь порог
      // остался умолчанием, и это была единственная несимметричность между
      // двумя пробами, падавшими в пачке 05.09.2026.
