          ...
          route.fulfill({
            json: {
              results: [
                ...
                // 🔴 АВТОР ЗДЕСЬ НЕПУСТ, И ЭТО ГЛАВНОЕ В ПРОБЕ (найдено ревью
                //    коммита e6b8fd98, задача №825). Сервер несимметричен:
                //    `acknowledgedBy` при собственном подтверждении обнуляется
                //    (`my_assignments.py`, ветка acknowledge), а `declinedBy`
                //    пишется ВСЕГДА — значит у собственного отказа стоит имя
                //    самого сотрудника. С пустым автором ассерт ниже проходил
                //    бы из-за пустоты, а НЕ из-за признака «своей рукой», и
                //    мутация «показывать автора по одному лишь непустому
                //    автору» осталась бы зелёной — при том что каждый, кто
                //    отказался сам, увидел бы под своими словами «записал:
                //    <собственное имя>».
                row('assignment-своей-рукой', 'self', 'Пробов П.'),
              ],
              employeeId: String(mine.employee!.id),
              unlinkedReason: null,
            },
