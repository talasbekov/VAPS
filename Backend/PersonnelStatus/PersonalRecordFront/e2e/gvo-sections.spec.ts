...
test.describe(LIVE ? 'сводные данные ГВО' : 'сводные данные ГВО (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('без event.view вкладка визитов закрыта вместе с реестром ОМ', async ({
    ...
    // `observer` получил `event.view` вместе с рабочим чтением ОМ; он больше
    // не является отрицательной персоной. `erda` — реальный оператор
    // подразделения без ОМ-прав, поэтому отказ ниже проверяет именно гейт.
