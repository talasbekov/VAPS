...
// 🔴 ИМЯ ПОДРАЗДЕЛЕНИЯ БОЛЬШЕ НЕ ПОПАДАЕТ В РЕГУЛЯРКУ (уточнено ревью №767).
// Здесь стояло объяснение, зачем экранируется имя: на живом стенде есть
// «Управление (стенд)», и голые скобки в `new RegExp(row.name)` читались бы
// как группа захвата. Экранирующий помощник снят уборкой №767 вместе с
// последним `new RegExp` в этом файле — отбор идёт по подписи «имя · путь»
// (`divisionLabels` ниже), то есть сравнением строк, а не поиском по шаблону.
// Комментарий оставлен как предупреждение: вернёте `new RegExp` от имени —
// вернёте и эту ловушку.
