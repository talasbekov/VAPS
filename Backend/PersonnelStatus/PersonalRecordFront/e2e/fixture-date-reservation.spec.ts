/**
 * Дата заранее выдаётся локальной Django-командой в global setup; обычная
 * персона только проходит реальный пользовательский путь создания и чтения ОМ.
 */
...
test.describe(LIVE ? 'бронь e2e-деловой даты' : 'бронь e2e-деловой даты (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('не-администратор создаёт и читает ОМ в браузере на выданной дате (Plane №890)', async ({ page }) => {
    ...
    const title = `(e2e) Бронь даты №890 ${Date.now()}`
    await signIn(page)
