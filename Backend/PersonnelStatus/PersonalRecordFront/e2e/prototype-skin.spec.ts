...
test.describe(LIVE ? 'слой прототипа' : 'слой прототипа (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('полотно отличается от карточки в обеих темах', async ({ page }) => {
    ...
    // Ожидание ЯВНОЕ и длинное (Plane №150). Проба падала в общем прогоне и
    // была зелёной в одиночку — так выглядит не дефект экрана, а гонка со
    // сборкой маршрута: `next dev` компилирует страницу на первый заход, а
    // «Командный центр» тяжёлый, и под конец длинного прогона, когда сервер
    // уже раздут, компиляция не укладывается в умолчание в 5 секунд. Соседние
    // пробы этого файла ждут по 15-20 с ровно поэтому.
    //
    // Ассерт при этом НЕ ослаблен: заголовок обязан появиться, просто ждём мы
    // его столько, сколько стоит первая сборка страницы.
    await expect(
      page.getByRole('heading', { name: 'Командный центр' }),
  ...
  test('категории меню нераскрываемые: все пункты видны сразу', async ({ page }) => {
    ...
    await expect(headings).toHaveText([
      ...
      // Категория переименована 25.08.2026 (Plane №50): её имя совпадало с
      // именем пункта внутри неё.
      'Система',
      'Обратная связь',
    ])
    ...
    // 22 = 21 + «Транспорт ГОН» в категории «Охранные мероприятия» (Plane
    // №215): реестр парка службы живёт независимо от мероприятий и читается
    // сам по себе, поэтому получил свой пункт, а не вкладку внутри объектов.
    // 21 было 18 + три экрана раздела доступа («Права», «Роли»,
    // «Пользователи»), вставших в «Систему» шагом «П-10» (Plane №106,
    // карточка №124); 18, а не 19, потому что пункт «Реестр ГВО» снят вместе
    // с модулем (Plane «Реестр ОМ-35.8»). Число правится ОСОЗНАННО и только
    // вместе с меню: оно стережёт, что пункты не расползаются мимо категорий
    // и не рисуются дважды.
  ...
  test('меню читается: кегль 14px, строка 36px, шаг 40px', async ({ page }) => {
    ...
    // 22 — см. выше: 21 прежний плюс «Транспорт ГОН» (Plane №215).
    expect(metrics.links, 'меню недосчиталось пунктов — замеры были бы не о том меню').toBe(22)
  ...
  test('активный пункт подводится в зону видимости сам', async ({ page }) => {
    ...
    // 🔴 ДВА ОЖИДАНИЯ, А НЕ ОДНО (Plane №341). Раньше здесь стоял только
    // первый опрос — «меню поехало», — и замер брался сразу за ним. Но
    // подводка отрабатывает ДВАЖДЫ: сначала на монтировании, потом ещё раз,
    // когда доезжает `user` и снизу появляется карточка роли (~57px), из-за
    // чего меню становится ниже. Между этими двумя срабатываниями пункт
    // законно стоит ниже края, и замер, попавший в промежуток, видел недокрут
    // ровно в высоту карточки — 54px в прогоне 30.08.2026. Под нагрузкой
    // полного прогона промежуток растягивается, поэтому проба краснела ТОЛЬКО
    // в нём, а поодиночке была зелёной.
    //
    // Предмет пробы при этом НЕ подменён, и это важно: первый опрос остался
    // на месте и по-прежнему стережёт вакуум — «меню вообще прокрутилось,
    // иначе подводку проверять не на чем». Добавлено второе ожидание — конца
    // движения, а не его начала.
    await expect
    ...
    await expect
      .poll(async () => {
        ...
        return now.aboveTop <= 0 && now.belowBottom <= 0
      }, {
        message: 'подводка не довела активный пункт до зоны видимости меню',
        timeout: 5_000,
      })
      .toBe(true)
  ...
  for (const [route, buttonName] of [
    // `?view=forces` (Plane №273): actions-слот реестра. Раньше здесь
    // проверялась «Добавить сотрудника» — кнопка снята вместе с окном по
    // слову заказчика (Plane №940); крайним справа осталось «Обновить».
    ['/employees/?view=forces', 'Обновить'],
    ['/statuses/', 'Обновить'],
  ] as const) {
  ...
  const HEADER_ROUTES: Array<{ path: string; title: string; eyebrow: string }> = [
    ...
    // Переименовано 25.08.2026 по требованию заказчика: «Настройки ОМ» →
    // «Администрирование» (Plane «Реестр ОМ-37»), а КАТЕГОРИЯ, чьё имя стоит
    // надзаголовком, стала «Система» (Plane №50) — до этого категория и пункт
    // назывались одинаково.
    { path: '/security-ops/settings/', title: 'Администрирование', eyebrow: 'Система' },
    { path: '/security-ops/feedback/', title: 'Обратная связь', eyebrow: 'Обратная связь' },
    { path: '/security-ops/audit/', title: 'Аудит', eyebrow: 'Система' },
    // Личный кабинет
