...
test.describe(LIVE ? 'слой прототипа' : 'слой прототипа (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('категории меню нераскрываемые: все пункты видны сразу', async ({ page }) => {
    ...
    // 21 = 18 + три экрана раздела доступа («Права», «Роли», «Пользователи»),
    // вставших в категорию «Система» шагом «П-10» (Plane №106, карточка №124).
    // Было 18, а не 19: пункт «Реестр ГВО» снят вместе с модулем (Plane
    // «Реестр ОМ-35.8») — сводка живёт панелью в карточке ОМ, сводный взгляд
    // вкладкой реестра. Число правится ОСОЗНАННО и только вместе с меню: оно
    // стережёт, что пункты не расползаются мимо категорий и не рисуются
    // дважды.
  ...
  test('меню читается: кегль 14px, строка 36px, шаг 40px', async ({ page }) => {
    ...
    // 21 — см. выше: 18 прежних плюс три экрана раздела доступа («П-10»).
    expect(metrics.links, 'меню недосчиталось пунктов — замеры были бы не о том меню').toBe(21)
