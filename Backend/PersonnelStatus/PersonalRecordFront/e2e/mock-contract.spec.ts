  ...
  () => {
    ...
    test('старший объекта в сиде — из кадрового списка мока (Plane №633)', async ({
      ...
      const seen = await page.evaluate(async () => {
        ...
        return {
          ids: roster.results.map((person) => person.id),
          // Имена нужны ВТОРЫМ ассертом: id из списка ещё не значит, что
          // рядом стоит имя этого же человека (см. ниже).
          names: Object.fromEntries(
            roster.results.map((person) => [person.id, person.name]),
      ...
      for (const chief of seen.chiefs) {
        ...
        // 🔴 ИМЯ ТОЖЕ СВЕРЯЕТСЯ (найдено ревью №825). Оно собиралось, но
        // уходило только в текст сообщения об ошибке: переименуй человека в
        // фикстуре — и проба останется зелёной, а окно скажет «Сейчас
        // назначен ‹старое имя›» под строкой с новым. Тот же класс
        // рассинхронизации, что и №633, только по второму полю той же строки.
        expect(
          chief.name,
          `имя старшего объекта ${chief.code} разошлось с кадровым списком (id ${chief.id})`,
        ).toBe(seen.names[chief.id])
      }
    })
