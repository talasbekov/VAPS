...
/**
 * МЕСТНАЯ дата, а не UTC (Plane №373). Уборка закрывает статусы сегодняшним
 * числом, и `toISOString()` в плюсовой зоне после семи вечера подставлял бы
 * ВЧЕРАШНЕЕ: сервер отбил бы «завершение раньше начала» у строки, заведённой
 * сегодня, и уборка молча перестала бы убирать.
 */
