...
test.describe(LIVE ? 'статусы: язык календаря' : 'статусы/календарь (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('календарь заведения статуса печатает месяц и дни недели по-русски', async ({ page }) => {
    ...
    // 🔴 ПОДПИСЬ ВЗЯТА ИЗ СПРАВОЧНИКА, А НЕ ИЗ ПАМЯТИ (Plane №366). Здесь
    // стояло «Командировка» — слово из тринадцати зашитых в код подписей.
    // Подписи переехали на справочник сервера, и `business_trip` называется
    // там «В командировке»: старый пин искал вариант, которого в списке нет
    // вовсе, и падал по таймауту выбора.
    await page.getByRole('option', { name: 'В командировке' }).click()
