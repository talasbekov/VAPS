 ...
 *
 * Проба отвечает на два вопроса: сводка стоит на РЕАЛЬНОМ расчёте, а пустую
 * причину возврата отбивает СЕРВЕР, а не экран.
 *
 * 🔴 ПРО «БЛОК ОБХОДОВ ПРЕДУПРЕЖДЕНИЙ» ЗДЕСЬ БОЛЬШЕ НЕ ГОВОРИТСЯ (Plane
 * №720): и блок, и плитку сводки снял №446 — обходы предмет аудита, не
 * согласования. Прежний докстринг обещал проверку того, чего на экране нет,
 * и переживал собственный предмет; сама проверка отсутствия (`not
 * .toContainText('обходов предупреждений')`) осталась ниже и работает.
 *
...
test.describe(LIVE ? 'согласование' : 'согласование (скип: нет SMOKE_LIVE=1)', () => {
  ...
  test('сводка и обходы идут от расчёта, пустой возврат отбивает сервер', async ({
    ...
    // №399) — его место в аудите. Плитки сводки с числом обходов тоже нет
    // (№446); упоминание о ней снято вместе с ней (№720). Назначение с
    // обходом при этом по-прежнему видно в расчёте на согласование.
  ...
  test.describe('подзаголовок без объектов посещения', () => {
    ...
    test('у ОМ без объектов посещения подзаголовок не печатает «(версия —)»', async ({
      ...
      const token = await apiToken()
      // СВОЁ ОМ, а не «первое с постами» из общего реестра (Plane №852).
      // Первым с постами в живом реестре стоит временное мероприятие соседней
      // спеки или сессии: оно удаляется в её `finally`, карточка отвечает 404,
      // и подзаголовок не появляется ни за 15 секунд, ни вовсе — ровно тот
      // признак, которым проба мигала (05.09.2026: в пачке ✘, поодиночке ✓;
      // замер 07.09.2026 на одном стенде: 3 пачки + 3 одиночных — ✓, то есть
      // мигание приходит извне, от соседей). Порог не поднят: он не лечит
      // удалённое ОМ. Перехват `page.route` ниже правит ответ СВОЕГО ОМ.
      const code = await prepareEvent(token, { businessDate: uniqueBusinessDate() })
      const target = (await events(token)).find((e) => e.code === code)
      expect(target, `не удалось подготовить фикстуру (${code})`).toBeDefined()
  ...
  test('«строки без объекта» не выдают чужое согласование за своё', async ({ page }) => {
    // 🔴 ЧТО ЭТО СТЕРЕЖЁТ (Plane №479 и №488 — одна причина, две карточки).
    // При `?visit=__unassigned__` разрез отдавал «объекта нет», и этап
    // показывал поля УРОВНЯ МЕРОПРИЯТИЯ — то есть маршрут и замечания ПЕРВОГО
    // объекта как свои. Человек читал чужое согласование, а каждая кнопка
    // уходила на сервер без адреса объекта и получала отказ. Выхода со
    // страницы, кроме правки адреса руками, не было.
    //
    // ЧТО ПРОВЕРЯЕТСЯ ЗДЕСЬ, А ЧТО РЯДОМ. №488 сделал само значение
    // ЗАКОННЫМ ТОЛЬКО ТОГДА, когда неразмеченные строки есть и объектов
    // больше одного; в остальных случаях адрес чинится на первый объект. У
    // этой фикстуры посты размечены по объектам, поэтому проверяется именно
    // починка: тупика больше нет. Само правило проверяется прямо и
    // исчерпывающе в `visit-scope-unassigned.spec.ts`, а пустое состояние
    // №479 остаётся для законного случая — ОМ с неразмеченными строками,
    ...
    // Адрес починен на первый объект, а не оставлен указывать в пустоту:
    // пересланная ссылка не разносит тупик дальше.
    await expect(page).toHaveURL(new RegExp(`visit=${first.id}`), { timeout: 15_000 })
    // И это НАСТОЯЩИЙ объект, а не «поля мероприятия»: маршрут на месте и
    // кнопки живые.
    await expect(card.getByText('Маршрут согласования').first()).toBeVisible({
      timeout: 15_000,
    })
    await expect(card.locator('[data-slot="approval-unassigned"]')).toHaveCount(0)
  })
})
