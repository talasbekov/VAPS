 ...
 *
 * `ROTATE_REFRESH_TOKENS` на сервере включён: ответ несёт новый `access` и
 * новый `refresh`, а прежний refresh после этого отзывается.
 *
 ...
 *
 * Ротация вместе с `BLACKLIST_AFTER_ROTATION=True` отзывает прежний refresh.
 * Поэтому параллельные обращения к сессии обязаны делить одно продление на
 * токен: второй запрос с уже отозванным значением не должен выкинуть человека
 * на ровном месте. Связь между двумя настройками и этим кодом зафиксирована
 * здесь, а продление сделано одним на токен.
 *
...
async function refreshed(token: JWT): Promise<JWT> {
  ...
  if (outcome.kind === "ok") {
    const next: JWT = {
      ...
      // экрана. При включённом списке отозванных (`BLACKLIST_AFTER_ROTATION`)
      // это также обязательно: старый токен уже нельзя отправлять повторно.
...
export const authOptions: NextAuthOptions = {
  ...
  session: {
    strategy: "jwt",
    // 🔴 ЭТО ОКНО ПРОСТОЯ, А НЕ СРОК ЖИЗНИ СЕССИИ (Plane №463). Прежний
    // комментарий здесь обещал «сессия не живёт дольше refresh-токена» — для
    // JWT-стратегии это неверно, и обещание вводило в заблуждение ровно там,
    // где комментарии служат защитой от повторения №383.
    //
    // Сессия СКОЛЬЗЯЩАЯ: маршрут `/api/auth/session` пересчитывает срок и
    // перевыпускает cookie при КАЖДОМ чтении. Замерено на стенде 05.09.2026 —
    // два чтения с разницей в две секунды дали `Expires` 12:23:54 и 12:23:56,
    // то есть срок уехал ровно на паузу между ними. У человека, который
    // работает, cookie обновляется бесконечно; `maxAge` ограничивает только
    // то, сколько сессия ждёт БЕЗ обращений.
    //
    // Тридцать дней по умолчанию всё равно не годятся: ушедший в отпуск
    // вернулся бы к живой сессии, продлить которую нечем, — та самая болезнь
    // №383. Семь суток означают «вернулся в пределах недели — работаешь
    // дальше», и совпадение с `REFRESH_TOKEN_LIFETIME` здесь по существу:
    // держать окно простоя дольше жизни refresh-токена бессмысленно.
    //
    // ЭТО БЫЛО НЕ ЛЕЧЕНО СЕМЬ СУТОК ОТ ВХОДА, ТЕПЕРЬ ЛЕЧЕНО РОТАЦИЕЙ (Plane
    // №787, следом за №463 этим же комментарием). До №787 refresh-токен умирал
    // через семь суток ПОСЛЕ ВХОДА безусловно — человек, работающий не
    // прерываясь, получал принудительный выход посреди работы, и скользящая
    // cookie этого не отменяла. Теперь `ROTATE_REFRESH_TOKENS` включён, каждое
    // продление выдаёт новый токен с новым семисуточным окном, и клиент его
    // сохраняет (`refreshed()` ниже) — непрерывная работа продлевает refresh
    // так же, как продлевает cookie.
