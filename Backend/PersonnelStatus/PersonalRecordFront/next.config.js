...
const nextConfig = {
  ...
  // зависеть от того, чего в проекте нет.
  //
  // Вместе с этим сняты `.eslintrc.json` и скрипт `lint`: конфиг описывал
  // правила, которые никто не применяет, а команда `next lint` не работала.
  // Файл, обещающий проверку, которой не существует, — тот же класс, что
  // «код, который врёт про себя»; из-за него класс «элемент списка без
  // ключа» пришлось закрывать РУЧНЫМ сторожем (`e2e/react-list-keys.spec.ts`)
  // вместо правила `react/jsx-key`.
  //
  // Внесение линтера — отдельное решение (объём правок на существующем коде
  // неизвестен), вынесено карточкой.
  eslint: {
