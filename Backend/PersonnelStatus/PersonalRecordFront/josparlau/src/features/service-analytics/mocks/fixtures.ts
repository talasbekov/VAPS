...
// Максимальная глубина ПРОИЗВОЛЬНОГО периода здесь больше не лежит: §22.5
// запрещает хардкодить её, а слайс потребителя — почти то же самое, что
// константа в его коде. Владелец — раздел «Настройки» (§29, `ANALYTICS_LIMITS`),
// читается узкой проекцией `readAnalyticsCustomPeriodLimit`.
