...
export const MODULE_PERMISSION = {
  ...
  // Отдельная страница структуры читает те же подразделения и штатные единицы,
  // что обзор, поэтому её маршрут и API обязаны открываться одним правом.
  // Неизвестный адрес трактуется картой как публичный для любого вошедшего —
  // пропуск этой строки пускал EMPLOYEE с одним status.view на страницу, где
  // сводка отвечала, а защищённое дерево штатки закономерно возвращало 403.
  "/organization": "orgstructure.view",
