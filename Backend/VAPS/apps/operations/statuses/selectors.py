...
class StatusTypeSelector:
    ...
    def catalog() -> list[dict]:
        ...
        пинится отдельным тестом). Tie-break ``code`` тоже проверяем тестом на
        равных priority — на живом сиде все приоритеты уникальны, и без такого
        теста половина ``order_by`` была бы вакуумной. Шесть полей — контракт
        combobox'а и подсветки; остальные колонки модели принадлежат владельцам
