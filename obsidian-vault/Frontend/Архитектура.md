...
Конспект `docs/frontend/FRONTEND_SOURCE_INDEX.md`, `FRONTEND_ROUTE_MAP.md`, `FRONTEND_TRACEABILITY_MATRIX.md` (слито 2026-08-20). Документы описывают **демонтированную 10.08.2026 SPA** (`frontend/`, Vite + React + MSW, «Smart Josparlau») — живой фронт сейчас PersonalRecordFront (Next.js :3106), куда прототип переносился срезами (`/security-ops/**`). Конспект сохраняет архитектурные принципы SPA — они переносимы; конкретные пути кода — история. См. также [[Дизайн-и-скин]], [[Тестирование]], [[../Продукт/Карта-модулей|Карту модулей]].
