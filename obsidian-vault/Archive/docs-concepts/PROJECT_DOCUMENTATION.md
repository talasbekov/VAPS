...
> **ℹ️ СТАТУС: AS-IS ДОКУМЕНТАЦИЯ ЛЕГАСИ-МОНОЛИТА.** Описывает существующий монолит Personnel Records (`Backend/PersonnelStatus/Personnel-Records/`) «как есть».
> Это **донор логики**, а не целевая архитектура VAPS. Целевая структура и решения — `docs/RECONCILIATION.md` (G1/G2) и `VAPS_7.8.2.md`.
> Описанные здесь WebSocket-уведомления, расписания Celery (09:05–09:15) и модель `AuditEntry` относятся к легаси и в целевой системе заменяются решениями R1, R8 и G2.
