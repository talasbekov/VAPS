...
_Code review 2026-06-16 (bmad-code-review; адверсариально, multi-layer: 3 Blind Hunter (только дифф) + 2 Edge Case Hunter + 7 Acceptance Auditor → 22 сырых → 10 подтверждённых после 3-линзовой адверсариальной верификации (code-reality · impact · spec-sanction); 12 отклонено как шум/санкционировано)._

**Decision resolved → Patch (1):** _Решение Bratan (2026-06-16): **Вариант 1** — Σ-conservation backstop на подразделение (`Σ сравнимых донор-колонок > Σ сравнимых VAPS-колонок` сверх поглощаемого fold/timing → эмитить `data/skipped_employee`-блокер) + консервативный дефолт (непарный донор-избыток в type-колонке без компенсирующего VAPS-избытка в подразделении → `unclassified`). Гейт никогда не зеленит при нетто-недосчёте VAPS. Закрывает C1 и C6._

- [x] [Review][Patch] DoD-гейт можно молча озеленить потерянной донор-строкой, попавшей в type-колонку. `_classify_cell` возвращает `model/single_winner` для ЛЮБОГО донор-избытка в type-колонке (delta<0), когда `vaps_in_service_higher=False`, без проверки, что VAPS реально держит «победителя» в другой колонке (определяющий признак single_winner по Dev Notes: «двойной счёт — попадает в обе колонки»). Чистая потеря строки при импорте (`data/skipped_employee`, «донор прав, VAPS потерял») неотличима от настоящего двойного счёта → помечается explained `model` → `has_unclassified=False` → гейт зелёный. Воспроизведено на реальном коде: VAPS SICK=0 / донор sick_leave=3 → `model/single_winner`, `has_unclassified==False`; то же для standalone DETACHED-потери. Прямое нарушение AC-5 + Решение №11 + Dev Notes:138 («потеря донор-строки молча зеленит гейт» — явно запрещённый failure). Ветка IN_SERVICE (стр. 281-282) лоss корректно роутит в `data/skipped_employee` — пропущен только type-column путь. Связано (C6): отсутствует per-division Σ-сверка headcount как backstop. Направление фикса однозначно (непарный донор-избыток в type-колонке НЕ должен авто-объясняться как `model`), но конкретный механизм дискриминации требует решения владельца (разный профиль ложных срабатываний на parallel-run). [donor_diff.py:294-297; гейт только из per-cell категорий — strength_report.py:79]

**Patch (4):**

- [x] [Review][Patch] VAPS-side дубликат `division_code` молча схлопывается (last-write-wins), асимметрично с donor-side STOP в `load_baseline`; противоречит докстрингу модуля «never a silent last-write-wins». `Division.code` уникален только в рамках `(organization, code)`, а `code_by_division_id` строится org-слепо (`Division.objects.values_list('id','code')`). Материальность: на E1 (один орг) не достижимо, но латентный correctness-баг + противоречие собственному докстрингу. Фикс: симметричный STOP (`ValueError`) на дубль VAPS-кода ИЛИ ключ по `(org, code)`. [donor_diff.py:309-313; strength_report.py:59]
- [x] [Review][Patch] `load_baseline` молча перетирает день при дубликате `date` между блоками `days[]` — нет STOP, в отличие от дубля `code` внутри дня (стр. 143-146 кидает `ValueError`). Склейка фриза, случайно дважды вклеившая один день, теряет первый блок без ошибки; downstream-проверка «нет дня» не ловит (ключ-дата присутствует). Фикс: `if day_date in result: raise ValueError`. [donor_diff.py:148]
- [x] [Review][Patch] `load_baseline`: несколько форм невалидной схемы вылетают как голый `TypeError` ВНЕ guarded-блока вместо `ValueError` → команда падает трейсбеком вместо чистого `CommandError` (нарушение AC-3 «невалидная схема → ValueError», т.к. `_load_baseline` ловит только `ValueError`). Формы: `rows: null`/число (нитерабельно — цикл `for raw in rows` вне try), значения ячеек `null`/`float`/`str` (нет int-валидации → падение позже в `diff_day` или дробная дельта мис-классифицируется), unhashable `division_code` (`["A"]` → `TypeError` на `if code in by_code`). Фикс: обернуть весь разбор ряда + валидировать типы. [donor_diff.py:137-149]
- [x] [Review][Patch] Некорректный `--division` (не-UUID строка, пустая, неверной длины) кидает необработанный django `ValidationError` вместо `CommandError` — `Division.objects.filter(id=...)` падает на build-этапе, `.exists()` не достигается; пользователь видит сырой трейсбек. Тесты покрывают только валидный-но-несуществующий UUID. Фикс: ловить `ValidationError`/валидировать формат UUID → `CommandError`. [strength_report.py:132]

**Резолюция (2026-06-16, все 5 патчей применены и проверены):**

- **C1 (gate soundness):** в `_classify_cell` убран необоснованный авто-`model/single_winner`; непарный донор-избыток в type-колонке (не-timing) → `unclassified` (блокер). Брутфорс по 4.2M комбинаций подтвердил: консервативный дефолт гарантирует, что любой нетто-недосчёт VAPS даёт хотя бы одну блокирующую ячейку → отдельная синтетическая Σ-ячейка была бы недостижимым dead-code, поэтому не добавлялась (принцип Σ-сохранения реализован самим per-cell правилом, доказуемо). Категория `model/single_winner` удалена как неотличимая от потери данных (истинная дискриминация — донорский timing-replay, E7 7.8).
- **C2:** `diff_day` кидает `ValueError` на коллизии `Division.code` (симметрично donor-side STOP); команда конвертирует в `CommandError`.
- **C3/C8:** `load_baseline` STOP на дубликат `date` между блоками `days[]`.
- **C4/C7/C9/C10:** `load_baseline` валидирует `rows` (list), `division_code` (str), значения ячеек (int, не bool/float/str/None) → все невалидные формы дают `ValueError` (AC-3), не голый `TypeError`.
- **C5:** не-UUID `--division` → `CommandError`.
- **Тесты:** +14 (31 в `test_donor_diff.py`, +2 в `test_strength_report_command.py`); `TestSingleWinner` переписан в `TestAmbiguousDonorSurplusBlocksGate` (бывший single_winner-кейс теперь корректно блокирует гейт). Гейт усилён, не ослаблен.
- **Verify:** `make gate` зелёный (342 passed, 7 deselected, 8s), `ruff` чистый, `makemigrations --check` → No changes.

### Review Findings — Проход 2 (2026-06-16, верификация фиксов C1–C10)

_Независимый второй проход (bmad-code-review; 68 агентов: 3 Blind Hunter (только дифф) + 2 Edge Case Hunter + 2 Acceptance Auditor → 28 сырых → 20 канонических → 3-линзовая адверсариальная верификация code-reality · impact · spec-sanction, majority ≥2/3). **Production-логика фиксов C1–C10 верифицирована как корректная** — ни одного подтверждённого дефекта в `donor_diff.py`/`strength_report.py` (6 PASS-проверок: gate-soundness C1, полнота AC-3 C4/C7/C9/C10, симметрия C2, C5, AC-6 read-only, чистота удаления `single_winner` в репо). Подтверждены 2 находки гигиены (doc/test), 4 отложены, 14 отклонены (ложные срабатывания + ниты)._

**Patch (2):**

- [x] [Review][Patch] Устаревший каталог Dev Notes всё ещё числит `model/single_winner` живой категорией после удаления C1: строка таблицы «Несколько статусов / двойной счёт» (стр. ~119), примечания маппинга колонок «дифф только timing/single_winner» (стр. ~124-125, ~131), Completion Notes «исчерпывает … model/single_winner» (стр. ~263), задача-каталог CATEGORY_RULES (стр. ~39). Код после C1 эту категорию эмитить не может (`donor_diff.py:326-336` → `timing/half_open_end` либо `unclassified`); каталог — контракт «исчерпываемости», по которому сверяется Acceptance Auditor, и сейчас описывает несуществующую категорию → будущий dev может реоткрыть дыру C1. Фикс: привести каталог/маппинг/Completion Notes в соответствие с резолюцией C1. [1-8-…md:119,124-125,131,263,39]
- [x] [Review][Patch] Тест `test_non_list_rows_raises_value_error_not_typeerror` ложно-зелёный для параметра `"xxx"` и без `match=`: строка итерируема, `for raw in rows` даёт символы → `raw["division_code"]` кидает `TypeError` → ловится внутренним `except` → `ValueError` даже БЕЗ guard'а (строка 136); тест проходит независимо от фикса и проверяет иной путь, чем заявлено в имени. None/42 сохраняют «зубы». Фикс: добавить `match="must be a list"` (отсечёт ложный путь `"xxx"`) и убрать/перепривязать `"xxx"`. [test_donor_diff.py: TestLoadBaseline.test_non_list_rows_raises_value_error_not_typeerror]

**Defer (4):**

- [x] [Review][Defer] Guard счётчиков C4 принимает отрицательные целые — `if not isinstance(value, int) or isinstance(value, bool)` отклоняет None/float/str/bool, но пропускает отрицательный head-count (`isinstance(-3, int)` → True). Fail-safe (нетто-несоответствие → `unclassified` → гейт блокирует, молча не зеленит); фикстура синтетическая/контролируемая → не достижимо на реальном пути. Усиление (`or value < 0`) — при формализации freeze-пути E7. [donor_diff.py:165-169] — deferred, low (fail-safe). **ОБНОВЛЕНО (ревью спайка 1.9, пр.3, 2026-06-16): defer ЗАКРЫТ, НЕ E7.** «Fail-safe» опровергнуто — воспроизведён обход гейта (`in_service=1, vacation=-1` vs VAPS-нули → обе ячейки `model/aggregator_inferred` → `has_unclassified=False`, гейт проходит вместо блока на потере данных; достижимо по `--diff-baseline`). Исправлено `or value < 0` + регресс-тест `test_negative_cell_value_raises_value_error`.
- [x] [Review][Defer] `diff_day` try/except в команде ловит только `ValueError`, но `diff_day` делает bracket-доступ `code_by_division_id.get(violation["division_id"])` — отсутствие ключа `division_id` в violation даст `KeyError` мимо `except ValueError` → сырой трейсбек. Недостижимо сейчас (`violations` из `StrengthReportService` 1.7 всегда содержат `division_id`). [strength_report.py:75-82; donor_diff.py:368] — deferred, latent
- [x] [Review][Defer] Ветка `timing/half_open_end` для type-колонки срабатывает на голом булеве `vaps_in_service_higher` без Σ-проверки величины — асимметрично строгому правилу IN_SERVICE (`delta == timing_surplus and timing_surplus > 0`). Гейт остаётся sound; это качество метки прото-классификатора. Полный timing-replay донорской интервальной логики — E7 7.8 (Решение №6). [donor_diff.py:326-336] — deferred to E7 7.8
- [x] [Review][Defer] Gate-тест проверяет присутствие строк-категорий, а не полноту «каталог ↔ фикстура» — дрейф между каталогом и фикстурой (как F13) тест не ловит. Усиление: ассерт на исчерпываемость каталога/совпадение с фикстурой. [test_strength_report_command.py:92-98] — deferred, test-hardening
...
| Несколько статусов на человека | **двойной счёт** (попадает в обе колонки), `inferred=max(0,total-known)` гасит | ровно один победитель по приоритету (BR-001) | ~~`model/single_winner`~~ → `unclassified` (удалён C1: неотличим от потери донор-строки) |
...
| Учёба (study) | `training` агрегируется в свою колонку | донор `training`→`STUDY`→колонка **TRAINING** | 1:1 (тот же источник), дифф только `timing/half_open_end` |
| Больн./Командир./Прочие/Откоманд. | `sick_leave`/`business_trip`/`other_absence`/`seconded_out` агрегируются | `SICK`/`COMMAND`/`OTHER`/`DETACHED` 1:1 | дифф только `timing/half_open_end` |
...
**Маппинг колонок донор→VAPS (что реально сравнивается):** `staff_unit→staff_total`; `sick_leave→SICK`, `business_trip→COMMAND`, `other_absence→OTHER`, `seconded_out→DETACHED`, `training→TRAINING` — донор-агрегатор разбивает эти типы, VAPS 1:1 (дифф только `timing/half_open_end`; авто-`single_winner` удалён C1 → непарный донор-избыток в type-колонке = `unclassified`). `vacation→VACATION` и (`training` уже учтён, а вот) колонка VAPS TRAINING/VACATION систематически больше донорских из-за folding `leave_by_report`/`competition` → `model/aggregator_inferred`. `in_service→IN_SERVICE` — у донора catch-all (inferred + все неразбитые типы), систематически больше VAPS. `seconded_in→attached` — разные источники (`model/attached_source`). **НЕ сравниваются:** `list_total`/`vacancies` (донор не эмитит), `present_total`/`presence_pct` (донор-производные, VAPS не считает).
...
- 2026-06-16: Code review (bmad-code-review, адверсариальный multi-layer, 78 агентов) → 1 decision + 4 patch применены. Закрыт критичный C1 (DoD-гейт молча зеленел на потере донор-строки в type-колонке): убран авто-`model/single_winner`, ambiguous донор-избыток → `unclassified`-блокер (Решение Bratan: вариант 1, Σ-conservation консервативным дефолтом — доказано брутфорсом, что недосчёт всегда блокирует). C2 (дубль `Division.code` STOP), C3/C8 (дубль `date` STOP), C4/C7/C9/C10 (`load_baseline` → `ValueError` вместо `TypeError`, AC-3), C5 (не-UUID `--division` → `CommandError`). +14 тестов. `make gate` зелёный (342 passed, 8s). Status review → done.
