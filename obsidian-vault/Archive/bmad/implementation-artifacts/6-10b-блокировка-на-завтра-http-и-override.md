...
3. **POST override-эндпоинт.** Given `POST /api/operations/expense-reports/override-tomorrow-block/` (или action) с `{business_date, reason}` и правом `daily_report.override_block`, When вызываю, Then сервис `override_tomorrow_block(business_date, actor, reason)` пишет `TomorrowBlockOverride`; после активного override `tomorrow_block(date).blocked=False` → выпуск на завтра проходит (laggards остаются видимы записью). `ValueError` из сервиса: плохой вход/пустая причина → 400 `VALIDATION_ERROR`; дубль (различим через `__cause__` IntegrityError) → 409 `TOMORROW_BLOCK_ALREADY_OVERRIDDEN` (Д2-дефолт восстановлен ревью 2026-07-13, D2). Division-scope НЕ применяется — обход уровня ДНЯ, в payload нет division (текст исправлен на ревью 2026-07-13, D3: блок орг-уровневый по природе → и обход орг-уровневый; грубый гейт правом `override_block` у ORGD/OMD).
...
<!-- Ревью 2026-07-13 (Fable 5, cross-model vs спека+dev Opus 4.8, против HEAD с фиксами 6.10a). Слои: Blind Hunter / Edge Case Hunter / Acceptance Auditor. Вердикт AC: 1-pass, 2-pass (тест частичен), 3-pass-с-отклонениями, 4-PARTIAL, 5-PARTIAL, 6-pass. Auditor прогнал целевые тесты на HEAD: 496 passed. -->

- [x] [Review][Decision→Patch] ✅ РЕШЕНО Bratan + ИСПРАВЛЕНО 2026-07-13 (**блок по отфильтрованному списку**: новый селектор `CoreDivisionTreeSelector.active_ids` (существует И is_active), фильтр ДО решения, пусто после фильтра → блок снят; тесты: stale-only → 409, inactive → 409, mixed → 422 только live) **Фильтр протухших laggards косметический (blind+edge+auditor):** `blocked` вычисляется в `tomorrow_block` по НЕфильтрованному списку, фильтр живёт только в отдаче HTTP → призрачный required-id (удалённое подразделение в `SubmissionControlSettings`) даёт ВЕЧНЫЙ 422 с `laggards: []` — блок, который не снимается никакой сдачей, с сообщением, противоречащим собственному detail; тест `test_stale_laggard_is_filtered_out` пиннит дефект как контракт. Плюс AC-5 требует фильтровать и «неактивные» (`is_active=False`) — `divisions_map` не смотрит is_active (Task 3 [x] при половине работы). Решить: (а) блок по ОТФИЛЬТРОВАННОМУ списку (существующие+активные; пусто после фильтра → блок снят), (б) оставить блокировку по-старому, чинить только сообщение. [Backend/VAPS/apps/operations/submissions/services/tomorrow_gate.py]
- [x] [Review][Decision→Patch] ✅ РЕШЕНО Bratan + ИСПРАВЛЕНО 2026-07-13 (**409 + новый код `TOMORROW_BLOCK_ALREADY_OVERRIDDEN`** в реестре тем же PR; различение через `exc.__cause__` IntegrityError; тест дубля переписан на 409; AC-3-текст обновлён) **Дубль активного override → 400 вместо Д2-дефолта 409 (blind+auditor):** спека Д2 «дубль→409», арх-канон «409 = конфликт состояния (дубль)», Completion Note пишет «дубль→400 [Д2]» — инверсия дефолта подписана как сам дефолт, Q1 решён молча. 409 реализуем: `block_override.py:60-61` кидает `ValueError ... from exc` с IntegrityError-причиной → различимо через `exc.__cause__`. Решить: (а) 409 с НОВЫМ кодом `TOMORROW_BLOCK_ALREADY_OVERRIDDEN` (канон DAY_ALREADY_SUBMITTED/DOCUMENT_ALREADY_ISSUED; в реестр тем же PR), (б) принять 400 и исправить спеку. [Backend/VAPS/apps/operations/submissions/api/views.py:355-363]
- [x] [Review][Decision→Patch] ✅ РЕШЕНО Bratan + ИСПРАВЛЕНО 2026-07-13 (**day-level принят**: блок орг-уровневый по природе → и обход орг-уровневый; текст AC-3 приведён к факту, семантика зафиксирована в описании OpenAPI) **Day-level override без division-scope vs буква AC-3/Task 2 «Scope в сервисе» (blind major):** scoped-ORGD (право на одно подразделение) снимает блок на весь день для всей организации; реализация осознанна (в payload нет division, блок — уровня дня; коммент в RBAC-матрице), но Task 2/Task 5 отмечены [x] при обратном тексте («Scope в сервисе», «чужой scope → 403»). Решить: (а) принять day-level семантику + исправить текст AC-3/Task 2/Task 5 (зеркало D3 6.10a), (б) требовать глобальный scope (scope_division_id NULL) для override. [Backend/VAPS/apps/operations/submissions/api/views.py]
- [x] [Review][Decision] ✅ РЕШЕНО Bratan 2026-07-13 (**принято орг-wide**: UUID-only без имён, оперативная ценность; зафиксировано в докстринге tomorrow_gate) **Орг-wide laggards уходят за scope запрашивающего (edge+blind):** 422-ответ отдаёт ПОЛНЫЙ список отстающих required-подразделений (UUID, без имён) любому scoped-держателю `daily_report.generate`; прецеденты уже́ (5.5a — только дети запрошенного, 5.7 — адресно). Решить: (а) принять (UUID-only, оперативная ценность: оператор видит, кого ждать) + зафиксировать в спеке, (б) сузить laggards до пересечения с visible_division_ids актора. [Backend/VAPS/apps/operations/submissions/services/tomorrow_gate.py:240]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (`MAX_OVERRIDE_HORIZON_DAYS = 31`: `business_date > today+31` → 400 с `max_days_ahead` в details; граничный тест +31→201/+32→400) Верхняя граница даты override отсутствует (AC-4/Д3 «не абсурдно далёкое», Task 2 [x] при пропуске без эскалации): `business_date=3000-01-01` → 201 + неотзываемая запись (5.6b без revocation) → через года блок FR-18 молча снят древней опечаткой. Fix: `business_date > today + 31д → 400` (Д3-дефолт) [Backend/VAPS/apps/operations/submissions/api/views.py:342]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (+8 тестов/переписано 2, итого 14 в файле: override→реальный 201 (двух-division) + прямой ассерт записи TomorrowBlockOverride; границы ==today на гейте и override; missing business_date→400; горизонт 31/32; дубль→409; stale/inactive/mixed laggards; ORGD-пин в test_seed (9 прав + idempotent count)) Тест-дыры Task 5 (отмечен [x]): «override снимает блок → выпуск ПРОХОДИТ» ассертит 409, а не 201 (одно-division сетап делает 201 недостижимым — нужен двух-division: required-B не сдала → 422, override → 201 по сдавшей A); нет прямого ассерта записи TomorrowBlockOverride (overridden_by/reason); граница `business_date == today` не пиннится ни на гейте, ни на override-400; «пусто» в date-валидации не покрыто; ORGD-выдача `override_block` не зафиксирована в test_seed (только OMD 7→8) [Backend/VAPS/apps/operations/submissions/tests/test_tomorrow_block_api.py]
- [x] [Review][Patch] ✅ ИСПРАВЛЕНО 2026-07-13 (@extend_schema: request=TomorrowBlockOverrideSerializer, 201 inline-response, 400/409-семантика в description; `make schema` (+51/−3) + `npm run generate:api`; оба гейта зелёные) OpenAPI override-эндпоинта дегенеративна и врёт: `'200': No response body` при реальном 201 с телом; request-схема отсутствует (нет @extend_schema — create/list/period получили его фиксом 6.10a, override пропущен). Fix: @extend_schema + make schema + generate:api [Backend/VAPS/apps/operations/submissions/api/views.py:329-335; Backend/VAPS/schema.yaml:1368-1376]
- [x] [Review][Defer] `_Audited`-контракт override-роута выполнен только сервис-тестом (5.9), HTTP-смока эмиссии через роут нет — тот же паттерн, что у ops-expense-report-list (6.10a) — deferred, паттерн-уровень, закрыть скопом при e2e E10
- [x] [Review][Defer] Override неотзываем (5.6b: no revocation, unique по дате) — ошибочный override лечится только правкой БД; API отзыва = за пределами 6.10b — deferred, кандидат в E10/13.x при появлении реальной потребности
