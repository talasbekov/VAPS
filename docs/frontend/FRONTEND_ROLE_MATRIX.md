...
`ops.analytics.view` / `ops.analytics.drilldown` / `ops.analytics.personal_detail` (§22.26,
Этап 43, FRONTEND_DECISIONS A75) — три права одного экрана, и §22.26 перечисляет их
раздельно. `view` даёт дашборд (агрегаты), `drilldown` — раскрытие показателя до строк
(смены и объекты), `personal_detail` — сотрудника в этих строках. `analyst` получил первые
два, третьего у него НЕТ: на этом разделении держится демонстрация §22.30 «privacy
suppressed» — он видит, сколько смен стоит за числом и какие, но не видит, кто в них стоял.
ФИО при этом вырезает СЕРВЕР (в ответе его нет), а не вёрстка.

## §28 «Обратная связь» — четыре права (Этап 47, FRONTEND_DECISIONS A79)

`ops.feedback.create` — завести обращение; `ops.feedback.view` — читать реестр (свои
обращения); `ops.feedback.view_all` — видеть ЧУЖИЕ отправленные; `ops.feedback.view_confidential`
— видеть содержание чужого обращения, помеченного конфиденциальным.

Раздача по persona выбрана так, чтобы КАЖДОЕ разделение было наблюдаемо, а не только описано:

- `event_planner` — `create` + `view`. Автор: видит только свои обращения.
- `analyst` — `create` + `view` + `view_all`, БЕЗ `view_confidential`. Снова контролёр (как с
  `personnel.identity.audit`): видит чужие обращения строкой, но их содержание сервер
  вырезает. Это единственная persona, на которой признак «конфиденциально» вообще наблюдаем,
  поэтому автор конфиденциального обращения в сиде — НЕ аналитик (иначе показывать было бы
  нечего).
- `admin` — wildcard, видит всё, **кроме чужого черновика**: черновик не открывается ни одним
  правом, потому что автор его не отправлял.

## §28 detail — ещё два права (Этап 48, FRONTEND_DECISIONS A80)

`ops.feedback.triage` — разбирать обращение (ответственный, рабочий приоритет, переход
статуса, закрытие); `ops.feedback.internal_note` — вести и читать внутренние заметки.

Разделение наблюдаемо: persona `analyst` получила `triage` и НЕ получила `internal_note` —
она разбирает обращения, но заметки ей не приезжают ни текстом, ни событием ленты (проверено
и по телам сетевых ответов). Полный набор есть только у wildcard-персоны.

Публичный ответ пишет разбирающий ИЛИ автор обращения: обращение — разговор, а не форма,
отправленная в один конец. Посторонний с правом читать чужие обращения ответить не может.

## §29 — права раздела «Настройки» (Этапы 49-50, FRONTEND_DECISIONS A88/A93)

`ops.settings.view` — видеть политику и журнал её изменений; `ops.settings.manage` — менять
значения.

Разделение наблюдаемо: persona `analyst` получила `view` и НЕ получила `manage` — она видит
политику, по которой считается её же блок «Требует внимания», видит журнал чужих правок, но
кнопок изменения не получает, а прямой PATCH от неё отклоняется сервером (403). Полный набор
есть только у wildcard-персоны.

Журнал изменений намеренно доступен по праву ПРОСМОТРА: проверять администратора должен тот,
кто сам администратором не является.

### Этап 50: третье право — правила конфликтов

`ops.settings.manage_conflict_rules` отделено от `ops.settings.manage`: правка правил
конфликтов §21.34-21.35 меняет ИСХОД чужой операции (назначение на дежурство отвергается
либо требует обоснования), а не окраску экрана. Одно право на оба раздела означало бы, что
доступ к безобидной панели порогов открывает и правила, которыми блокируется работа.

Persona `analyst` получила `ops.settings.manage` БЕЗ `manage_conflict_rules` — без такой
комбинации разделение недемонстрируемо (у администратора wildcard, и оба раздела всегда
открывались бы с кнопками). На одном экране у неё видны ДВЕ разные причины недоступности:
«нет права» у правил конфликтов и «правило не редактируется» (§21.34) у пересечения —
последняя не снимается НИКАКИМ правом, включая wildcard.
