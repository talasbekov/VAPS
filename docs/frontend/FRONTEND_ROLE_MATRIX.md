...
### Этап 51: четвёртое право — политика паспортов

`ops.settings.manage_passport_policy` выдано persona «Ведение объектов» (§21.7: политикой
объектов владеет тот, кто их ведёт). Наблюдения и правила конфликтов ей закрыты.

Разделение теперь демонстрируется ЗЕРКАЛЬНО и проверяется с обеих сторон:

| Persona | Наблюдения §22.11 | Правила конфликтов §21.34 | Свежесть паспортов §21.7 |
|---|---|---|---|
| `analyst` | правит | только смотрит | только смотрит |
| `objects_admin` | только смотрит | только смотрит | правит |
| `admin` (wildcard) | правит | правит, кроме запертого правила §21.34 | правит |

## Оценивание и рейтинг (§19.22, Этап 57)

| Persona | Выставление оценки `ops.rating.evaluate` | Просмотр агрегата `ops.rating.view_aggregate` | Вкладка «Сводка мероприятия» §19.14 |
|---|---|---|---|
| `event_planner` | да | нет | нет |
| `analyst` | нет | да | — (в раздел не входит) |
| `admin` (wildcard) | да | да | да |

Разделение намеренное: оценщик обязан выставить оценку, не видя, как она сложится в
рейтинг человека. Sensitive-просмотр отдельных оценок (§19.21 «контролёр рейтинга») и
право исправления (§19.18) НЕ заведены — операций за ними пока нет.

Исправление (§19.18) и просмотр цепочки исправлений — ещё два отдельных права:

| Persona | `ops.rating.correct` | `ops.rating.view_correction_chain` |
|---|---|---|
| `event_planner` | да | да |
| `recon_officer` | нет (оценивает, но не исправляет) | нет |
| `admin` (wildcard) | да | да |

Исправление ограничено СОБСТВЕННОЙ записью: чужая требует organization scope, event
scope и срока полномочия (§19.21), которых в сборке нет.

Выгрузка (§19.29) — ещё одно отдельное право:

| Persona | `ops.rating.export` |
|---|---|
| `analyst` | да (читает агрегаты — файл с той же сводкой доступа не расширяет) |
| `event_planner` | нет (оценивает, но не выгружает) |
| `admin` (wildcard) | да |

Права на sensitive export НЕТ ни у кого, и оно не заведено: индивидуальная выгрузка не
выдаётся вовсе (§19.21 — нет ни scope, ни срока полномочия), а право под неотданную
операцию у wildcard-администратора немедленно открыло бы её.
