...
## `/ratings/export` — выгрузка рейтинга (§19.29, Этап 63)

Право входа — `ops.rating.export`, своё и отдельное: держатель сводки (`view_aggregate`) и
оценщик (`evaluate`) сюда не попадают. Секцией на `/ratings` маршрут быть не мог: там правом
входа служит право ЧТЕНИЯ агрегата, а файл переживает экран, уходит из системы и требует
audit-записи — это другое действие.

Скачивание идёт ОТДЕЛЬНОЙ операцией (`/api/ops/rating-export-artifacts/:id/download/`), а не
ссылкой в строке: §19.29 «Не добавляй фиктивную ссылку на файл».

## `/ratings/audit` — журнал оценивания (§19.27, Этап 61)

Право входа — `ops.rating.view_audit`, своё и отдельное: держатель сводки и оценщик сюда
не попадают. Не секция общего `/audit`: тот читают люди без права на рейтинг, и события
закрытого раздела раздавались бы вместе с ним (§19.27).
