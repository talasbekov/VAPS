...
- `styles/globals.css` **мёртв** (не импортируется ниоткуда, написан под v4 с другой палитрой). Править только `app/globals.css`.
...
Найти файл, рисующий плитки на `/dashboard` (искать по подписи «Всего сотрудников»: `grep -rn "Всего сотрудников" --include=*.tsx app components features`). Заменить каждую плитку на `<StatCard label=… value=… caption=… tone=… />`, сохранив подписи и значения дословно.

- [ ] **Step 5: Перевести плитки структуры организации**

В `app/organization/page.tsx` заменить блок плиток («Занято», «Департаментов», «Управлений», «Отделов», «Штатных единиц», «Вакансий») на `StatCard`. Подписи и числа не менять.
...
Надзаголовки: «Личный состав» и «Официальные документы». Тексты `title`/`description` сохранить дословно.
