...
## Smart Josparlau — Этап 8 (календарь смен + назначение с конфликтами)

| Проверка | Файл | Статус |
|---|---|---|
| Календарная арифметика: сдвиг дат, границы месяца/года, високосный февраль, понедельник как начало недели, подписи диапазона | features/duties/model/calendar.test.ts | Verified (9 тестов) |
| Та же арифметика в МИНУСОВОЙ таймзоне: `TZ=America/Anchorage npx vitest run …/calendar.test.ts` — зелено; красная проба (замена `getUTCDay()` на `getDay()`) валит 2 теста в −09 и остаётся зелёной в +05 | features/duties/model/calendar.test.ts (прогон вручную с TZ) | Verified (гвард не вакуумен) |
| duties repository: права на чтение/назначение, сортировка списка (3 элемента, порядок вставки ≠ ожидаемому), бизнес-дата сервера, справочник | features/duties/mocks/repository.test.ts | Verified (14 тестов) |
| createShift: 400 по полям, 422 mismatch вида/объекта, 422 двойное назначение (не обходится даже с override), 409 смежные сутки (код ∈ OVERRIDABLE_CODES), конфликт только по СВОЕМУ сотруднику, override с причиной создаёт смену, +2 суток — не конфликт | features/duties/mocks/repository.test.ts | Verified |
| Календарь UI: неделя от бизнес-даты СЕРВЕРА, строки — весь ростер (в т.ч. без смен), ‹/› переключают неделю и смена не протекает | features/duties/pages/DutyPlanPage.calendar.test.tsx | Verified (5 тестов) |
| Живой протокол 409: клик по пустой клетке → форма → 409 → общий `ConflictDialog` → причина → повтор с `override` в КОРНЕ тела (ассерт по обоим телам запросов, не по счётчику) | features/duties/pages/DutyPlanPage.calendar.test.tsx | Verified |
| 422 hard-block рендерится как неотменяемый отказ, диалог обхода НЕ появляется | features/duties/pages/DutyPlanPage.calendar.test.tsx | Verified |
| Без `ops.duty.manage` клетки не кликабельны (красная проба: `canManage = true` валит тест) | features/duties/pages/DutyPlanPage.calendar.test.tsx | Verified (гвард не вакуумен) |
| Ручная браузерная проверка (dev:mock, 2026-07-28): календарь на неделе 20–26 июля, назначение на 22.07 → живой 409 → форма закрылась (один уровень модалок) → обход с причиной → смена появилась в клетке; попытка на 20.07 (занят) → 422 внутри формы без предложения обхода | — (нет Playwright-теста) | Verified вручную, Not started как автотест |
