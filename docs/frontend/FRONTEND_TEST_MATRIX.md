...
| setEntryActive(false) на значении с ЖИВЫМИ связями → RepositoryConflictError, и сообщение НАЗЫВАЕТ зависимость; без связей — проходит и персистентна; деактивация неотслеживаемого значения разрешена (действие обратимо); реактивация НЕ блокируется связями; неизвестный id → RepositoryNotFoundError | features/dictionaries/mocks/repository.test.ts | Verified |
| Связи считаются по общему снимку: тип записи журнала — по ЧУЖОМУ слайсу ОМ с носителем; ноль связей — TRACKED, а не «не отслеживается»; отсутствие слайса ОМ → UNKNOWN, а НЕ ноль; причина возврата → NOT_TRACKED с названной причиной; группа требований — по своему слайсу | features/dictionaries/mocks/repository.test.ts | Verified |
| deleteEntry (§30): со связями → 409 с зависимостью в details; неотслеживаемое → DICTIONARY_USAGE_NOT_TRACKED; недоступный источник → DICTIONARY_USAGE_UNKNOWN; отслеживаемое без связей → удаляется и персистентно; цепочка «группа с требованием + само требование» не удаляется ни с одной стороны | features/dictionaries/mocks/repository.test.ts | Verified |
| Созданное требование СРАЗУ становится связью своей группы (ответ несёт пересчитанное значение, а не кэш) | features/dictionaries/mocks/repository.test.ts | Verified |
| Handler-уровень: DELETE совпадает с путём КЛИЕНТА и не перехватывается соседним `/:code/entries/`; set-active не перехватывается маршрутом удаления; 409 доезжает с зависимостью в details; неизвестный id → 404 | features/dictionaries/mocks/handlers.test.ts | Verified |
| E2E: запись журнала штаба на стадии «Проведение» ОМ ЗАПИРАЕТ значение справочника в другом разделе (связь через общий demo-снапшот); отслеживаемое значение без связей удаляется; неотслеживаемое не удаляется, но деактивируется | e2e-mock/dictionaries.spec.ts | Verified |
...
## Этап 49 — Настройки §29

| Проверка | Файл | Статус |
|---|---|---|
| Версия политики: сдвиг числового хвоста, добавление хвоста без него, инвариант «результат ВСЕГДА отличается от исходного» | features/settings/lib/policy.test.ts | Verified (15 тестов файла) |
| Валидация значения по диапазону ИЗ ЗАПИСИ (не по общей константе), включительные границы, отказ дробному/нечислу; причина считается после trim | features/settings/lib/policy.test.ts | Verified |
| Порядок порогов: нарушение видно только В ПАРЕ, работает с обеих сторон, равенство допустимо, соседний детектор в проверку не попадает | features/settings/lib/policy.test.ts | Verified |
| Репозиторий: права на чтение/изменение врозь, `canManage` с сервера, серверный порядок записей, персистентность повторным чтением, журнал новыми сверху, повтор значения не растит журнал и версию, отказ не оставляет следа | features/settings/mocks/repository.test.ts | Verified (13 тестов) |
| Журнал изменений виден и тому, кто менять не вправе (контроль отделён от администрирования) | features/settings/mocks/repository.test.ts | Verified |
| **Сопоставление маршрутов MSW** реальными URL клиента (`setupServer` + `apiClient`): GET списка/журнала, PATCH по пути из `settingPath`, ошибки доезжают своими статусами, журнал не перехватывается маршрутом одной настройки | features/settings/mocks/handlers.test.ts | Verified (4 теста) |
| Экран: версия политики, значения с единицами, «просмотр без изменения» по `canManage` с сервера, отправка значения с причиной, блокировка короткой причины (и РАЗБЛОКИРОВКА достаточной), 422 как неотменяемый отказ, 400 по полям, история и её пустое состояние | features/settings/pages/SettingsPage.test.tsx | Verified (8 тестов) |
| Аналитика: пороги приезжают ИЗ НАСТРОЕК (та же выборка при другом пороге даёт другое наблюдение), версия политики берётся из настроек и попадает В КАЖДЫЙ элемент, без слайса настроек блок честно недоступен | features/service-analytics/mocks/repository.test.ts | Verified (3 новых теста) |
| Маршрут `/settings` в карте гейтов §34 | app/smart-josparlau-routing.qa.test.tsx | Verified |
| Ручная браузерная проверка (dev:mock, 2026-07-28): правка 3 → 8 сут. → журнал «3 → 8» с причиной и версией `.2` → текст наблюдения аналитики стал «Срок упреждения — 8 дн.»; persona `analyst` — `canManage:false`, журнал видит | — | Verified вручную |

**Дефект, найденный живым прогоном (не тестами)**: паттерн handler'а собирался
`settingPath(':settingCode')`, а фабрика делает `encodeURIComponent` — в шаблон уезжало
`%3AsettingCode`, PATCH не совпадал ни с чем (500 от `onUnhandledRequest`). Ни юнит-тесты
репозитория (зовут функции напрямую), ни тест страницы (подменяет handler'ы своими) этого не
видели. Закрыто новым уровнем `mocks/handlers.test.ts` — **впервые в проекте handler'ы
фичи проверяются реальным `setupServer` против путей, которые строит клиент**.

**Красные пробы Этапа 49 (3, все покраснели)**: аналитика игнорирует политику ✓,
`canManage` захардкожен ✓, дефект пути handler'а возвращён ✓.

## Этап 50 — правила конфликтов §29 и владение режимом отдыха §21.35

| Свойство | Файл | Статус |
|---|---|---|
| severity нарушения отдыха задаёт ДЕЙСТВУЮЩАЯ политика: те же две смены при разных режимах дают HARD и SOFT | features/duties/lib/monthlyPlan.test.ts | Verified (красная проба) |
| Вид дежурства на severity больше не влияет: разные виды при одном режиме дают одинаковую severity (ловит возврат к прежнему владению) | features/duties/lib/monthlyPlan.test.ts | Verified (красная проба) |
| Каждый конфликт несёт версию правил, по которым посчитан | features/duties/lib/monthlyPlan.test.ts | Verified (красная проба) |
| Проекция чужого слайса: строгий дефолт §21.35 без политики и при непонятом коде, `null`-версия, запись чужого раздела режимом не считается | features/duties/mocks/settingsSlice.test.ts | Verified (5 тестов, красная проба) |
| Аналитика читает ТУ ЖЕ политику (один конфликт не может быть жёстким на плане и мягким в аналитике) | features/service-analytics/lib/analytics.test.ts + mocks/repository.test.ts | Verified (красная проба) |
| Правка режима — отдельное право: тот же актор порог МЕНЯЕТ, а правило нет | features/settings/mocks/repository.test.ts | Verified (красная проба) |
| Запертое правило §21.34 не правится даже wildcard; причина отказа одна для всех — замок проверяется первым (нашёл реальный дефект порядка) | features/settings/mocks/repository.test.ts | Verified (красная проба) |
| Версии разделов растут порознь — в обе стороны | features/settings/mocks/repository.test.ts | Verified (красная проба) |
| Журнал печатает подпись режима и раздел, а не код варианта | features/settings/mocks/repository.test.ts + lib/policy.test.ts | Verified (красная проба) |
| Значение-режим принимается только из списка вариантов записи; число режимом не является | features/settings/lib/policy.test.ts | Verified (красная проба) |
| **Сопоставление маршрута** для PATCH правила (код с точками через `settingPath`), 422 запертого правила по своему коду | features/settings/mocks/handlers.test.ts | Verified (приём Этапа 49 переиспользован) |
| Экран: две секции с раздельными версиями, причина отказа у КАЖДОЙ записи от сервера (нет права ≠ правило заперто) | features/settings/pages/SettingsPage.test.tsx | Verified |
| **e2e: правка правила МЕНЯЕТ ИСХОД** — то же назначение из «обход с обоснованием» становится отказом без обхода; версии разделов; запертое правило; журнал | e2e-mock/settings-conflict-rules.spec.ts | Verified (4 теста, красная проба на живом e2e) |

## Этап 51 — свежесть паспорта §21.7 и контракт demo-снапшота

| Свойство | Файл | Статус |
|---|---|---|
| Порог «скоро» ПРИХОДИТ ОТ ПОЛИТИКИ: тот же объект, интервал и день — состояние меняет только порог | features/objects/lib/passportFreshness.test.ts | Verified (красная проба) |
| Расширение интервала растягивает окно предупреждения — доля, а не дни | features/objects/lib/passportFreshness.test.ts | Verified |
| Проекция: интервал/порог/версия из чужого слайса; строгий отказ от нечислового и неположительного; версия-заглушка при непрочитанной политике; запись чужого раздела не считается | features/objects/mocks/settingsSlice.test.ts | Verified (5 тестов, 2 красные пробы) |
| Реестр объектов берёт политику ИЗ НАСТРОЕК: другой интервал → другое состояние и KPI; без слайса расчёт не выдаётся за действующую редакцию | features/objects/mocks/repository.test.ts | Verified (красная проба; прежняя версия теста была вакуумной — фикстура совпадала с сидом) |
| Третье право раздела в ОБЕ стороны: ведущий объекты правит свой раздел и не трогает чужие, администратор наблюдений — наоборот | features/settings/mocks/repository.test.ts | Verified (красная проба) |
| Версия двигается только у своего раздела (карта разделов) | features/settings/mocks/repository.test.ts | Verified (красная проба) |
| **Контракт demo-снапшота**: все узкие проекции читают РЕАЛЬНЫЙ сид владельца, а не свою фикстуру | app/mocks/settings-projections.contract.test.ts | Verified (красная проба на переименование ключа у владельца) |
| **e2e: правка интервала меняет состояние объекта и KPI в другом разделе**; порог-доля не трогает срок; раздел закрыт без своего права и открыт владельцу объектов | e2e-mock/settings-passport-policy.spec.ts | Verified (3 теста) |
| Починены 5 спек с ожиданиями снятой per-type модели severity (дефект Этапа 50, скрытый ложно-зелёным `tail`) | e2e-mock/duty-*.spec.ts | Verified |
| Динамика §19.20: смена методики рвёт линию; период без агрегата не становится вершиной и не печатается нулём | features/ratings/lib/dynamics.test.ts, features/ratings/pages/RatingDynamicsSection.test.tsx | Verified (две красные пробы, каждая валит оба уровня; фикстура экрана разводит пропуск и смену методики по разным периодам — иначе ассерт вакуумен) |
| Точки динамики отдаются КАК ЗАПИСАНЫ, не пересчитываются под текущую методику; текущая редакция не подписывает ни одной точки | features/ratings/mocks/repository.test.ts, app/mocks/settings-projections.contract.test.ts | Verified (красная проба: подписать точку текущей редакцией) |
| Выбор сотрудника в динамике уходит ЗАПРОСОМ на сервер, а не режет полученный ряд | features/ratings/pages/RatingDynamicsSection.test.tsx | Verified (красная проба: `employeeId` убран из ключа кэша) |
| **e2e: ряд закрытых периодов, граница методики и переключение сотрудника в реальном chromium** | e2e-mock/ratings.spec.ts | Verified |
| Аналитика §22.17: группа меньше порога подавлена, снижение порога её РАСКРЫВАЕТ, пустая группа — третье состояние | features/ratings/lib/analytics.test.ts, features/ratings/mocks/repository.test.ts | Verified (красная проба: подавление отключено) |
| Отчёт §22.16 охраняет право аналитики: держатель сводки не пускается, аналитик без сводки — пускается | features/ratings/mocks/repository.test.ts, mocks/handlers.test.ts, e2e-mock/ratings.spec.ts | Verified (красная проба: право сводки вместо права аналитики) |
| Подавленное значение не восстанавливается из отчёта: общего среднего нет ни одним полем | features/ratings/lib/analytics.test.ts, mocks/repository.test.ts | Verified (ассерт по всему JSON) |
| Задание отбирается по ОЦЕНЩИКУ на сервере: чужого нет в очереди, чужая отправленная оценка не видна, оценщик не едет наружу ни одним полем | features/ratings/mocks/repository.test.ts | Verified (2 красные пробы: снят отбор по оценщику; `evaluatorUserId` добавлен в проекцию) |
| Правило формы §19.9-19.10 одно на клиента и сервер; порядок проверок закреплён; основание не отменяет комментарий | features/ratings/lib/workspace.test.ts, features/ratings/mocks/repository.test.ts | Verified (красная проба: серверная проверка отключена) |
| Форма НЕ отправляется до прохождения клиентской проверки — утверждается отсутствием запроса, а не появлением надписи | features/ratings/pages/EvaluationWorkspacePage.test.tsx, e2e-mock/evaluation-workspace.spec.ts | Verified (красная проба: снят ранний возврат) |
| Повторная отправка и устаревшая редакция задания отвергаются своими кодами, причём редакция проверяется РАНЬШЕ правил формы | features/ratings/mocks/repository.test.ts | Verified (2 красные пробы: снят гард статуса; снята сверка редакции) |
| Вкладка «Сводка мероприятия» приходит только с правом на агрегат, и в ней нет ни одного значения оценки | features/ratings/mocks/repository.test.ts, features/ratings/pages/EvaluationWorkspacePage.test.tsx, features/ratings/lib/workspace.test.ts | Verified (красная проба: право не проверяется; ассерт по всему JSON) |
| Порядок очереди задаёт сервер и задаёт его по подписи участника, а не по начальной оценке | features/ratings/mocks/repository.test.ts | Verified (красная проба: сортировка по идентификатору) |
...
| **e2e: §19.25 «две вкладки» — исправление во второй вкладке даёт первой diff, а не ложный успех** | e2e-mock/evaluation-workspace.spec.ts | Verified (Этап 61; на Этапе 60 проба ошибочно считалась невозможной — закрывала вторую вкладку до коммита) |
| Журнал §19.27 охраняет СВОЁ право: ни оценщику, ни держателю агрегата он не открыт | features/ratings/mocks/repository.test.ts, mocks/handlers.test.ts, e2e-mock/rating-audit.spec.ts | Verified (красная проба: журнал охраняется правом сводки) |
| В записях журнала нет значений оценок и комментариев — ассерт по всему JSON и по перечню колонок | features/ratings/mocks/repository.test.ts, pages/RatingAuditPage.test.tsx, e2e-mock/rating-audit.spec.ts | Verified (красная проба: в запись добавлено значение) |
| Отказ остаётся в журнале, хотя состояние откатилось; успех пишется в транзакции коммита | features/ratings/mocks/repository.test.ts | Verified (2 красные пробы: отказ пишется внутри отклонённой мутации; запрещённая попытка не журналируется) |
| «Изменение относительно начального» — отдельное событие; отправка ровно начального его не создаёт | features/ratings/mocks/repository.test.ts | Verified (красная проба: событие не пишется) |
...
| Право на выгрузку §19.29 — своё: держатель сводки файл не заказывает и списка не видит | features/ratings/mocks/repository.test.ts, mocks/handlers.test.ts, e2e-mock/rating-export.spec.ts | Verified (красная проба: право маршрута подменено на `ops.rating.evaluate` — валит все три e2e) |
| Состояния серверные: QUEUED → GENERATING → READY; файл появляется только на READY и больше не пересобирается | features/ratings/mocks/repository.test.ts | Verified (красная проба: READY перестаёт быть терминальным) |
| В файле нет закрытых величин — ассерт по ВСЕМУ ответу вместе с содержимым | features/ratings/mocks/repository.test.ts, e2e-mock/rating-export.spec.ts | Verified (красная проба: комментарий оценки подставлен в строку файла) |
| Индивидуальная выгрузка не выдаётся НИКОМУ, включая wildcard-администратора; отказ ложится в журнал своей транзакцией | features/ratings/lib/export.test.ts, features/ratings/mocks/repository.test.ts | Verified (красная проба: гард режима снят — валит три теста) |
| Повтор заказа с тем же ключом не создаёт вторую работу; второй заказ с экрана уходит с ДРУГИМ ключом | features/ratings/mocks/repository.test.ts, pages/RatingExportPage.test.tsx | Verified (2 красные пробы: ключ игнорируется; ключ не обновляется после заказа) |
| Отмена работает на незавершённой работе и отвергается на готовой; чужая работа отвечает «не найдено», а не «нет прав» | features/ratings/mocks/repository.test.ts | Verified (красная проба: проверка владельца снята) |
| Выдача файла перепроверяет состояние работы заново: у неготовой файл не отдаётся, даже если артефакт собран | features/ratings/mocks/repository.test.ts, pages/RatingExportPage.test.tsx | Verified (2 красные пробы: проверка состояния снята на сервере; экран предлагает «Скачать» по наличию артефакта) |
| Выключенный рейтинг роняет работу в FAILED с кодом, а не собирает файл молча | features/ratings/mocks/repository.test.ts, pages/RatingExportPage.test.tsx | Verified (красная проба: кнопка заказа не гасится) |
| **e2e: заказ доходит до READY, файл выдаёт сервер, отмена оставляет работу без файла** | e2e-mock/rating-export.spec.ts | Verified (3 теста) |
