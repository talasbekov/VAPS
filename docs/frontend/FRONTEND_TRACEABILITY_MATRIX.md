...
## NEXT ACTION
`e2e-mock/` покрывает ВСЕ реализованные экраны (Этапы 11-13, расширены Этапом 14 — оперативный профиль, Этапом 15 — боевые группы на Трассе, 13 спек) — весь жизненный цикл ОМ, personnel, objects, dictionaries, calendar, audit, analytics, duties (включая боевые группы). Дальше — только функциональный объём: уведомления/полный §24 боевых групп (потребность на период, Conflict Repository, revision)/accessibility-tablet-Firefox (не реализованы, а не «не покрыты тестами»), по решению пользователя.
