...
| Обратная связь: карточка обращения (§28 detail — статус, рабочий приоритет, ответственный, публичный ответ, внутренняя заметка по праву, related screen, technical info, duplicate, close, timeline, audit) | feedbackDetail | features/feedback | getFeedback / addComment / triageFeedback / closeFeedback | e2e-mock/feedback-card.spec.ts | Verified |
...
## Покрытие
Итог: **51 из ~70 экранов Verified** (12 — жизненный цикл ОМ, 2 — сотрудники, 2 — объекты/паспорт, 1 — аудит, 1 — аналитика службы, 2 — план дежурств [индивидуальные дежурства + боевые группы на Трассе], 4 — справочники, 1 — календарь по дням, 1 — печатная форма расстановки, 1 — архив дела закрытого ОМ, 1 — версия паспорта объекта, 2 — привязка ОМ к версии паспорта и импорт постов §9.6, 1 — привязка дежурства к версии паспорта §9.6, 1 — месячный план дежурств §21.27-21.30, 1 — создание индивидуального дежурства с подбором кандидатов и конфликтами §21.31/§21.33/§21.34, 1 — карточка дежурства §21.32, 1 — правка и отмена смены §21.31, 1 — матрица доступности по сотрудникам §21.30, 1 — список дежурств и история §21.30, 1 — KPI реестра объектов §21.7, 1 — lifecycle месячного плана §21.27-21.28, 1 — раскрытие sensitive identity §20.27/§20.33, 1 — отчётный реестр службы §22.18-22.25, 1 — история отчётов §22.25, 1 — карточка работы отчёта §22.26-22.28, 1 — снимок аналитики службы §22.3-22.12, 1 — блок «Требует внимания» §22.11, 1 — аналитика ОМ §22.13/§22.15, 1 — воронка §22.14 и журнал переходов, 3 — обратная связь §28: реестр, создание и карточка с разбором, 1 — правила конфликтов §29/§21.35 с переносом владения режимом отдыха, 1 — политика свежести паспортов §29/§21.7). Остальные ~39 экранов (notifications/feedback/мой профиль/KPI/схемы/нагрузка/рейтинг/должности-звания как отдельный справочник/остальные режимы календаря/полный §24 боевых групп) — Not started. Ложным «Done» не помечать (запрет §35).
...
## §29 Настройки (Этап 49)

| Экран/действие | Route | Feature | Статус |
|---|---|---|---|
| Настройки: политика наблюдений §22.11 (допуски и пороги) | settings | features/settings | Verified (unit + handler + экранные тесты + ручная проверка) |
| Журнал изменений настроек (кто, когда, old/new, причина, версия политики) | settings | features/settings | Verified |
| Настройки: правила конфликтов §21.34-21.35 (режим обязательного отдыха) | settings | features/settings | Verified (unit + handler + экранные + e2e `settings-conflict-rules.spec.ts`; владение перенесено из вида дежурства, A90-A94) |
| Настройки: правило пересечения §21.34 — показано запертым | settings | features/settings | Verified (не редактируется никем, включая wildcard; 422 `SETTING_RULE_LOCKED`) |
| Настройки: свежесть паспорта §21.7 (интервал проверки + порог «скоро») | settings | features/settings | Verified (unit + проекция + контракт снапшота + e2e `settings-passport-policy.spec.ts`; владение перенесено из слайса объектов, A96-A97) |
| Настройки: пределы §22.5 (глубина произвольного периода аналитики, период отчёта, срок хранения файла) | settings | features/settings + проекции service-analytics/service-reports | listSettings / updateSetting; readAnalyticsCustomPeriodLimit / readReportLimits | features/settings/mocks/repository.test.ts, features/service-analytics/mocks/repository.test.ts, features/service-reports/mocks/repository.test.ts, app/mocks/settings-projections.contract.test.ts, e2e-mock/settings-limits.spec.ts | Verified |
| Настройки: типы статусов, справочники | dictionaries | features/dictionaries | Реализовано разделом §30 — вторым экраном не дублируется (A82) |
| Настройки: методика оперативного рейтинга §19.19 (период, минимум оценок) | settings | features/settings + проекция features/ratings | listSettings / updateSetting; readRatingPolicy | features/ratings/mocks/repository.test.ts, app/mocks/settings-projections.contract.test.ts, e2e-mock/ratings.spec.ts | Verified |
| Настройки: notification recipients | — | — | Not started (раздела уведомлений в сборке нет — настройка без предмета мертва, §35) |
| Оперативный рейтинг: сводка агрегатов §19.19/§19.21/§22.16 | ratings | features/ratings | listOperationalRatings | lib/rating.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingsPage.test.tsx, e2e-mock/ratings.spec.ts | Verified (только ЧТЕНИЕ агрегатов: формы оценивания §19.7-19.13, исправление §19.18, sensitive-просмотр §19.21 и динамика §19.20 — Not started, названы блоком §35 на самом экране) |
| Оперативный рейтинг: динамика агрегата §19.20 | ratings | features/ratings | getRatingDynamics | lib/dynamics.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingDynamicsSection.test.tsx, app/mocks/settings-projections.contract.test.ts, e2e-mock/ratings.spec.ts | Verified (ряд закрытых периодов, граница смены методики, разрыв на периоде без агрегата; прогноз/сглаживание — Not started осознанно, A114) |
| Оценивание участников: задания, форма и отправка §19.7-19.10/§19.14 | ratings | features/ratings | getEvaluationWorkspace / submitEvaluation | lib/workspace.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/EvaluationWorkspacePage.test.tsx, e2e-mock/evaluation-workspace.spec.ts | Verified (групповая оценка §19.13, исправление §19.18, фильтры благодарностей §19.11 и сводный экран §19.15-19.17 — Not started, названы блоком §35 на самом экране) |
| Исправление оценки и карточка записи §19.17-19.18 | ratings | features/ratings | getSubmittedEvaluationDetail / correctEvaluation | lib/correction.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/SubmittedEvaluationCard.test.tsx, e2e-mock/evaluation-workspace.spec.ts | Verified (исправление СВОЕЙ записи; чужая требует scope и срока полномочия §19.21 — Not started, названо блоком §35; архивное дополнение — Not started, архива нет) |
| Реестр итоговых оценок и карточка агрегата §19.15-19.17 | ratings | features/ratings | listEvaluationRegistry / getRatingEmployeeDetail | lib/registry.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/EvaluationRegistryPage.test.tsx, pages/RatingEmployeeDetailPage.test.tsx, e2e-mock/evaluation-registry.spec.ts | Verified (aggregate-only ветка §19.16; sensitive-колонки, фильтры благодарностей/замечаний и документов — Not started, названы блоком §35 на самом экране) |
| Идемпотентность и конфликты оценивания §19.25-19.26 | ratings | features/ratings | submitEvaluation / correctEvaluation | lib/idempotency.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/SubmittedEvaluationCard.test.tsx, pages/EvaluationWorkspacePage.test.tsx | Verified (конфликты `target no longer eligible`/`assignment replaced`/`event scope changed`/`archive locked` — Not started: источника нет) |
| Журнал оценивания §19.27 | ratings | features/ratings | listRatingAudit (+ запись из submit/correct/отказов) | mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingAuditPage.test.tsx, e2e-mock/rating-audit.spec.ts | Verified (audit privacy §19.27 и экспорт журнала §19.29 — Not started, названы блоком §35 на самом экране) |
| Экспорт рейтинга §19.29 | ratings | features/ratings | listRatingExports, createRatingExport, cancelRatingExport, downloadRatingExport | lib/export.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingExportPage.test.tsx, e2e-mock/rating-export.spec.ts | Verified (агрегированная выгрузка в CSV; XLSX/PDF и индивидуальная выгрузка — Not started с причиной: генератора нет, sensitive требует scope и срока полномочия §19.21) |
| Уведомления оценивания §19.28 | ratings | features/ratings | listRatingNotifications (+ запись из submit/correct) | mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingNotificationsSection.test.tsx, e2e-mock/rating-notifications.spec.ts | Verified (уведомление о сводке и уведомления сотруднику — Not started: нет адресатов и связи persona↔сотрудник, названы блоком §35) |
| Аналитика рейтинга §22.16-22.17 (распределение, покрытие, агрегаты групп, privacy suppression) | ratings | features/ratings (`/ratings/analytics`, право `ops.analytics.view`) | getRatingAnalytics | lib/analytics.test.ts, mocks/repository.test.ts, mocks/handlers.test.ts, pages/RatingAnalyticsPage.test.tsx, e2e-mock/ratings.spec.ts | Verified (подавление малых групп; общего среднего в ответе нет — A117) |
| Настройки: demo feature flags, reset demo data | — | DemoToolbar | Осознанно вне `/settings`: `mock-only-demo` runtime §8.3, не продуктовый API (A82) |
