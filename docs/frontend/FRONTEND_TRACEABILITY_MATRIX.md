...
## Покрытие
Итог (после разрешения продолжить): **19 из ~70 экранов Verified** (12 — жизненный цикл ОМ, 2 — сотрудники, 2 — объекты/паспорт, 1 — аудит, 1 — аналитика службы, 1 — план дежурств). Остальные ~51 экран (calendar/notifications/feedback/dictionaries/мой профиль/KPI/схемы/нагрузка/рейтинг/боевые группы) — Not started. Ложным «Done» не помечать (запрет §35).
