...
| Боевые группы: потребность на период, передача смены/HANDOVER, Conflict Repository (§24.17 за пределами DOUBLE_ASSIGNMENT), формальный revision | — | — | Not started (см. A51/A52/A53) |
...
## NEXT ACTION
`e2e-mock/` покрывает ВСЕ реализованные экраны (Этапы 11-13, расширены Этапом 14 — оперативный профиль, Этапом 15 — боевые группы на Трассе, Этапом 16 — execution-lifecycle, Этапом 17 — замены §24.21, 15 спек) — весь жизненный цикл ОМ, personnel, objects, dictionaries, calendar, audit, analytics, duties (включая боевые группы: подача→рассмотрение→ознакомление→заступление→факт→замена). Дальше — только функциональный объём: уведомления/остаток §24 боевых групп (потребность на период, передача смены, Conflict Repository, формальный revision)/accessibility-tablet-Firefox (не реализованы, а не «не покрыты тестами»), по решению пользователя.
