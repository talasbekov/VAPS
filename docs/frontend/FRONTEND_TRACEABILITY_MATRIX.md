...
| Календарь смен (сотрудники × дни, §21.4 третье представление) | duties | features/duties | Verified (unit + UI-тесты + ручная проверка) |
| Назначение смены с проверкой конфликтов (§24, hard 422 / soft 409+override) | duties | features/duties | Verified (unit + UI-тесты + ручная проверка) |
| Проекция статусов «до/на/после дежурства» на карточке смены | — | — | Not started (длительность BEFORE_DUTY — открытый вопрос §37, прототип сам подписывает «условно») |
| Hard-block по кадровым статусам (SICK_LEAVE/VACATION/COMMAND) | — | — | Not started (нет источника кадровых статусов в duties, см. A42) |

## Покрытие
Итог (Этап 8): **21 из ~70 экранов Verified** (12 — жизненный цикл ОМ, 2 — сотрудники, 2 — объекты/паспорт, 1 — аудит, 1 — аналитика службы, 1 — план дежурств, 2 — календарь смен и назначение). Остальные ~49 экранов (calendar/notifications/feedback/dictionaries/мой профиль/KPI/схемы/нагрузка/рейтинг/боевые группы) — Not started. Ложным «Done» не помечать (запрет §35).

## NEXT ACTION
Расширить `e2e-mock/` на personnel/objects/audit/analytics/duties и на середину жизненного цикла ОМ (Рекогносцировка→Потребность→Запрос сил→Расстановка); либо продолжить функциональный объём (уведомления/справочники/боевые группы) по решению пользователя.
