...
## Покрытие
Итог (после разрешения продолжить): **22 из ~70 экранов Verified** (12 — жизненный цикл ОМ, 2 — сотрудники, 2 — объекты/паспорт, 1 — аудит, 1 — аналитика службы, 1 — план дежурств, 2 — справочники, 1 — календарь по дням). Остальные ~48 экранов (notifications/feedback/мой профиль/KPI/схемы/нагрузка/рейтинг/боевые группы/остальные справочники/остальные режимы календаря) — Not started. Ложным «Done» не помечать (запрет §35).
