...
**ОТКРЫТОЕ (решение за владельцем продукта):** в `seed_operations` НИ ОДНА роль
кроме ADMIN не несёт `object.view`/`duty.view`/`event.view` — на чистом стенде
раздел ОМ виден только администратору. Роль `OPS_READER` (object.view +
duty.view) заведена на стенде вручную ради этой проверки и в сид НЕ внесена:
состав ролей — продуктовое решение, не догадка разработчика.
