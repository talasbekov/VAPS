...
| `/security-ops/calendar/` | `/api/ops/combat-duty-shifts/` (duty-shifts переехали срезом C1 — календарь читает их с живого бэка в live-режиме) | «Не подключено — Календарь смен…» (только про боевые группы) |
