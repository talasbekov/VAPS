...
### ОТКРЫТО: «Ответственный за ОМ» — это идентификатор учётки, а не ФИО

Обнаружено при переносе блока (17.08.2026), НЕ починено. `create_event`
пишет `owner_name=resolve_actor_id(request)`, то есть в поле ложится id
пользователя: на стенде «Ответственный за ОМ: 1». То же значение уже видно в
шапке карточки и в фильтре реестра «ответственный» (`owners` сервер собирает
из этих же строк), так что дефект не новый — блок лишь назвал его вслух.

Чинить придётся вместе с данными: у уже заведённых мероприятий в поле лежат
идентификаторы, а сопоставление «учётка → ФИО» упирается в `Employee.user`,
которую сид не заполняет (см. ямы подъёма стенда). Отдельный срез.
