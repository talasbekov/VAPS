...
| ~~`/ops/*` (встроенная SPA)~~ | **выведена из навигации**: SPA дублировала переписанные нативные страницы `/security-ops/*` (живой `/api/ops/*`); группа `/ops/*` снята из сайдбара, адреса редиректят на нативные экраны и живые хостовые (`/employees`, `/organization`, `/reports`, `/feedback`). Vite-источник `frontend/` и sync-josparlau не тронуты | врезки нет: экранов SPA в навигации больше нет |
