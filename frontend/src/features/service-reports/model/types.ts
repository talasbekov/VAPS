...
export interface ReportArtifact {
  ...
  /** Редакция политики хранения, которой назначен `expiresAt`. Срок замораживается
   * в момент сборки: правка политики потом не отнимает уже выданный файл и не
   * воскрешает истёкший, но артефакт обязан помнить, по какой редакции считался. */
...
export interface ReportTypeDefinition {
  ...
  /**
   * Максимальная глубина периода. Приходит из ПОЛИТИКИ «Настроек» (§29,
   * раздел «Пределы отчётности»), а не лежит в определении типа: §22.5 прямо
   * запрещает хардкодить квартал и срок, а число в собственном слайсе отчётов
   * — тот же хардкод, только на шаг дальше.
   *
   * `null` — про этот тип политика молчит. Тогда отчёт не формируется вовсе:
   * «предела нет» означало бы выгрузку любой глубины, чего никто не решал.
   */
  maxPeriodDays: number | null
  /** §35: почему тип недоступен, если предел не задан. Формулировку выбирает
   * сервер — той же строкой он откажет и в запуске. */
  unavailableReason: string | null
}

/**
 * Политика хранения артефактов (§22.22). Владелец — «Настройки»; здесь она
 * едет клиенту готовой. `retentionDays: null` — политика молчит, и файл не
 * собирается: артефакт без `expiresAt` жил бы вечно.
 */
