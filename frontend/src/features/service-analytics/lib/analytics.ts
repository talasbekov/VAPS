...
/** §35: блоки §22.6/§22.11, которых в этом срезе нет. */
export const UNAVAILABLE_HEADER_BLOCKS = [
  ...
  {
    code: 'ATTENTION_ITEMS',
    label: 'Блок «Требует внимания» (§22.11)',
    reason:
      'Каждый элемент блока §22.11 возвращается сервером вместе с `policyVersion` и `detectedAt` — это отдельный серверный детектор с собственной политикой порогов, а не перепечатка показателей этого снимка. Собрать его из уже посчитанных KPI значило бы выдать фронтовую перестановку за серверное наблюдение.',
  },
  {
    code: 'EXPORT',
