...
export type SubmissionField = 'score' | 'basisCode' | 'basisNote' | 'comment'
...
export const UNAVAILABLE_WORKSPACE_VIEWS: readonly {
  ...
  {
    code: 'EVALUATION_CORRECTION',
    label: 'Исправление отправленной оценки',
    reason:
      '§19.18 требует новую неизменяемую запись, correction chain, повторную загрузку актуальной revision, diff и подтверждение. Это отдельная операция со своим правом; пока её нет, отправленная оценка не редактируется вовсе — и это честнее кнопки, которая тихо переписала бы строку.',
  },
  {
