...
// запросом (шаг 3 — «повторно загрузить актуальную revision»), кнопка
// «Исправить оценку» появляется только если право прислал сервер (`canCorrect`),
// перед подтверждением показывается diff (шаг 8), и лишь затем уходит мутация с
// той редакцией, которую человек видел (шаг 10). Успешным исправление
// считается ПОСЛЕ ответа (шаг 11) — оптимистичного обновления здесь нет.
//
...
export function SubmittedEvaluationCard({
  ...
  if (query.isLoading) return <p className="text-sm text-muted-foreground">Загрузка карточки…</p>
  if (query.error !== null) return <p className="text-sm text-destructive">{query.error.message}</p>
  ...
  function review(event: React.FormEvent) {
    ...
    // Шаг 8: сначала diff, и только потом подтверждение. Отправить исправление
    // сразу по «Сохранить» значило бы пропустить шаг, ради которого §19.18 и
    // требует показать «было → стало».
    setConfirming(true)
  }
  ...
  return (
    <section className="mt-3 rounded-lg border bg-background p-3" aria-label="Отправленная оценка">
      ...
      {data.chain === null ? (
        <p className="mt-3 text-xs text-muted-foreground">
          История исправлений не показывается: право на просмотр цепочки исправлений не выдано.
        </p>
      ) : (
        <div className="mt-3">
          ...
          <ul className="flex flex-col gap-1">
            {data.chain.map((link) => (
              <li key={link.evaluationId} className="text-xs text-muted-foreground">
                <span className="font-mono">{link.evaluationId}</span> — оценка {link.score}
                {link.current
                  ? ' · действующая запись'
      ...
      {!editing && !data.canCorrect && (
        <p className="mt-2 text-xs text-muted-foreground">
          Исправление недоступно: право на исправление оценки не выдано. Отправленная запись не
          редактируется — исправление создаёт новую (§19.18).
        </p>
      )}

      {editing && !confirming && (
        <form className="mt-3" onSubmit={review} noValidate>
          <label className="mb-1 block text-xs font-semibold" htmlFor={`correct-score-${workItemId}`}>
            Новая оценка
          </label>
          <select
          ...
          <label className="mb-1 block text-xs font-semibold" htmlFor={`correct-basis-${workItemId}`}>
            Основание
          </label>
          <select
          ...
          <label className="mb-1 block text-xs font-semibold" htmlFor={`correct-comment-${workItemId}`}>
            Комментарий
          ...
          <label className="mb-1 block text-xs font-semibold" htmlFor={`correct-reason-${workItemId}`}>
