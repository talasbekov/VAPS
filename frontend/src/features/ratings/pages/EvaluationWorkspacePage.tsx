...
// сводка мероприятия.
//
// ЭКРАН НЕ СОЗДАЁТ ЗАДАНИЙ И НЕ ПРИДУМЫВАЕТ ЗНАЧЕНИЙ. §19.7 запрещает фронту
// определять список target, генерировать идентификаторы и создавать задание
// после refresh; §19.8 — фиксировать восьмёрку «вызовом setState» и добавлять
// кнопку «Применить 8 всем». Начальная оценка, перечень оснований, состав
// очереди и счётчики приходят готовыми.
//
// Вкладки «Мне нужно оценить» и «Отправленные мной» показывают РАБОТУ
// СМОТРЯЩЕГО. Оценок, полученных им от других лиц, здесь нет и быть не может:
...
/** Шкала §19.9: `1 2 3 … 10`. Нуля в ней нет — «Не используй 0». */
...
/**
 * Ошибку сервера ставим рядом с тем же полем, что и клиентскую (§19.9), и
 * узнаём её ПО КОДУ: разбирать текст сообщения значило бы держать копию правил
 * в подстроках.
 */
...
function EvaluationForm({ item, bases, onClose }: FormProps) {
  ...
  const serverErrorCode =
    mutation.error !== null && 'errorCode' in mutation.error ? mutation.error.errorCode : null
  const serverFieldRaw =
    serverErrorCode === null ? null : (SERVER_ERROR_FIELD[serverErrorCode] ?? null)
  // Поле пояснения существует не всегда. Отказ, адресованный СКРЫТОМУ полю,
  // иначе исчез бы совсем: человек нажал «Отправить», ничего не произошло и
  // ничего не сказано. Такой отказ печатается общим сообщением формы.
  const serverField =
    serverFieldRaw === 'basisNote' && selectedBasis?.requiresNote !== true ? null : serverFieldRaw

  function errorFor(field: SubmissionField): string | null {
    if (violation !== null && violation.field === field) return violation.message
    if (serverField === field && mutation.error !== null) return mutation.error.message
    return null
  }
  ...
  function submit(event: React.FormEvent) {
    ...
    // §19.9: «форма не отправляется до прохождения frontend validation».
    // Тот же вызов делает сервер на своих данных — правило одно на двоих.
  ...
  return (
    <form className="mt-3 rounded-lg border bg-background p-3" onSubmit={submit} noValidate>
      <div className="mb-3 grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
        ...
        <span>
          Фактический интервал: {dateTime(item.actualStartsAt)} — {dateTime(item.actualEndsAt)}
        </span>
        <span>Факт участия: {item.participated ? 'участвовал' : 'не участвовал'}</span>
        <span>Начальная оценка: {item.initialScore}</span>
      </div>

      <label className="mb-1 block text-xs font-semibold" htmlFor={`score-${item.id}`}>
        Оценка
      </label>
      <select
        id={`score-${item.id}`}
      ...
      <label className="mb-1 block text-xs font-semibold" htmlFor={`basis-${item.id}`}>
        Основание
      </label>
      <select
        id={`basis-${item.id}`}
      ...
      {selectedBasis?.requiresNote === true && (
        <>
          <label className="mb-1 block text-xs font-semibold" htmlFor={`basis-note-${item.id}`}>
          ...
          {errorFor('basisNote') !== null && (
            <p className="mb-2 text-xs text-destructive">{errorFor('basisNote')}</p>
          )}
        </>
      )}

      <label className="mb-1 block text-xs font-semibold" htmlFor={`comment-${item.id}`}>
        Комментарий
...
export function EvaluationWorkspacePage() {
  ...
  const tabs: TabKey[] =
    data?.eventProgress != null ? ['pending', 'submitted', 'progress'] : ['pending', 'submitted']
  ...
  return (
    <div>
      <header className="mb-6">
        ...
        <h1 className="text-2xl font-bold tracking-tight">Оценивание участников</h1>
        <span className="text-sm text-muted-foreground">
          Задания на оценивание формирует сервер. Оценки, полученные вами от других лиц, здесь не
          показываются и не запрашиваются.
        </span>
      </header>

      {query.isLoading && <p className="text-sm text-muted-foreground">Загрузка заданий…</p>}
        ...
        <>
          {data.unavailableReason === 'FEATURE_DISABLED' && (
            <p className="mb-4 rounded-xl border bg-card p-4 text-sm">
              Оценивание выключено сервером (ENABLE_OPERATIONAL_RATINGS). Заданий нет не потому,
            ...
            <label className="mb-4 block text-xs font-semibold" htmlFor="workspace-event">
              ...
              >
                {data.events.map((event) => (
                  <option key={event.securityEventId} value={event.securityEventId}>
          ...
          {data.selectedEvent !== null && (
            <section className="mb-4 rounded-xl border bg-card p-4" aria-label="Мероприятие">
              ...
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-xs text-muted-foreground sm:grid-cols-[auto_1fr_auto_1fr]">
                ...
                {/* Счётчики шапки — МОИ задания. Работа других оценщиков живёт
                    во вкладке «Сводка мероприятия» и охраняется своим правом. */}
                ...
                <dd className="font-mono">{data.policy?.policyVersion ?? 'не определена'}</dd>
                <dt className="font-semibold">Синхронизация</dt>
                <dd>
                  {query.isFetching ? 'Обновление…' : `Данные на ${dateTime(data.loadedAt)}`}
                </dd>
              </dl>
            </section>
          )}

          <div className="mb-3 flex gap-2" role="tablist" aria-label="Разделы оценивания">
            {tabs.map((key) => (
              <button
                key={key}
                type="button"
                  ...
                  {data.pending.map((item) => (
                    <li key={item.id} className="rounded-lg border p-3">
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <span className="text-sm font-medium">{item.targetSafeLabel}</span>
          ...
          {activeTab === 'submitted' && (
            <section className="rounded-xl border bg-card p-4" aria-label={TAB_LABEL.submitted}>
              {data.submitted.length === 0 ? (
                <p className="text-sm text-muted-foreground">Отправленных оценок нет.</p>
              ) : (
                <ul className="flex flex-col gap-3">
                  {data.submitted.map((entry: SubmittedEvaluationView) => (
                    <li key={entry.evaluationId} className="rounded-lg border p-3">
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <span className="text-sm font-medium">{entry.targetSafeLabel}</span>
                        <span className="text-xs text-muted-foreground">
                          {entry.postLabel} · {DIRECTION_LABEL[entry.evaluationDirection]}
                        </span>
                      </div>
                      <p className="mt-1 text-sm">
                        Оценка: <span className="tabular-nums">{entry.score}</span>
                        {entry.basisLabel !== null && ` · Основание: ${entry.basisLabel}`}
                      </p>
          ...
          {activeTab === 'progress' && data.eventProgress !== null && (
            <section className="rounded-xl border bg-card p-4" aria-label={TAB_LABEL.progress}>
              <p className="text-sm">
              ...
              <ul className="mt-2 flex flex-col gap-1">
                {data.eventProgress.byDirection.map((row) => (
                  <li key={row.direction} className="text-xs text-muted-foreground">
                    {DIRECTION_LABEL[row.direction]}: отправлено {row.counters.submitted} из{' '}
                    {row.counters.total}
                  </li>
                ))}
              </ul>
              <p className="mt-2 text-xs text-muted-foreground">
                Это прогресс оценивания, а не его содержание: значений оценок, оценщиков и
                комментариев здесь нет — они закрыты (§19.21).
              </p>
            </section>
          )}
