...
export const DEMO_PERSONAS: readonly DemoPersona[] = [
  ...
  },
  {
    ...
    permissions: [
      ...
      'ops.feedback.triage',
      // §29: аналитик ВИДИТ политику наблюдений, по которой считается его же
      // блок «Требует внимания», но менять её не вправе
      // (`ops.settings.manage` ему НЕ выдан). Без такой persona состояние
      // «просмотр без изменения» было бы недостижимо: у администратора
      // wildcard, и экран всегда открывался бы с кнопками.
      'ops.settings.view',
    ],
