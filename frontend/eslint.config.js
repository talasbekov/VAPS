  ...
  },
  {
    ...
    settings: {
      tailwindcss: {
        ...
        //
        // ⚠️ `dist-e2e` исключён отдельной строкой (QA-добор 10.5): `!**/dist`
        // матчит РОВНО `dist`, а сборка харнесов кладёт CSS в `dist-e2e`, и
        // первый же харнес, монтирующий таблицу маршрутов, притаскивает туда
        // `print.css` через `PrintTestPage`. Тогда `.print-root` снова
        // становится «известным классом» — и канон-самотест перестаёт
        // ДОКАЗЫВАТЬ срабатывание no-custom-classname (`lint-canon` краснеет
        // ровно этим). Артефакт сборки не имеет права влиять на скан канона.
        cssFiles: [
          '**/*.css',
          '!**/node_modules',
          '!**/.*',
