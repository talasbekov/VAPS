...
//    конфликт нарушения отдыха (severity — из глобальной политики §21.35,
//    в сиде SOFT_OVERRIDE, см. раздел «Настройки» §29);
...
test.describe('Матрица доступности по сотрудникам (§21.30, mock-режим)', () => {
  test('четыре выводимых слоя видны, два невыводимых названы с причиной', async ({ page }) => {
    ...
    await expect(row.getByText(/^2026-07-25, дежурство,.*мягких конфликтов: 1$/)).toBeVisible()
