...
test.describe('Accessibility (axe-core): второй слой аудита по всем реализованным экранам (mock-режим)', () => {
  test('нет critical/serious нарушений на экранах верхнего уровня', async ({ page }) => {
    ...
    const topLevelScreens: Array<[string, string]> = [
      ...
      ['/audit', 'Аудит'],
      // §28: реестр обращений сканируется вместе с РАСКРЫТОЙ формой — она
      // не сворачивается, поэтому отдельного шага не нужно (в отличие от
      // формы создания дежурства).
      ['/feedback', 'Обратная связь'],
      // §19.32: экраны оценивания в аудит не входили вовсе — найдено ревизией
      // Этапа 68. Карточка агрегата сканируется с реальным сеяным id.
      ['/ratings', 'Оперативный рейтинг'],
      ['/ratings/workspace', 'Оценивание'],
      ['/ratings/evaluations', 'Итоговые оценки'],
      ['/ratings/employees/employee-1', 'Карточка агрегата'],
      ['/ratings/audit', 'Журнал оценивания'],
      ['/ratings/export', 'Экспорт рейтинга'],
      ['/ratings/analytics', 'Аналитика рейтинга'],
    ]
  ...
  test('нет critical/serious нарушений на детальных/composed экранах', async ({ page }) => {
    ...
    // §28 карточка обращения — сканируется У РАЗОБРАННОГО обращения с
    // перепиской: у пустой карточки нет ни ленты, ни форм разбора и закрытия,
    // и покрытие было бы мнимым (тот же урок, что форма создания дежурства).
    await page.goto('/feedback')
    await page.getByLabel('Поиск по обращениям').fill('прямой ссылке')
    await page
      .getByRole('link', { name: 'Не открывается карточка мероприятия по прямой ссылке' })
      .click()
    await expect(page.getByRole('group', { name: 'Разбор обращения' })).toBeVisible()
    await assertNoSeriousViolations(page, 'Обращение — карточка с разбором')
