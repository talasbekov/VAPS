...
Публикуется только nginx на :3118; все контейнеры находятся в Docker-сети
`internal:true`, PostgreSQL и Redis портов хоста не занимают. Чужие стенды
...
Bootstrap ролей/боевых справочников — отдельный блок3. `seed_operations`
не запускать: он добавляет демонстрационные предметные данные. Браузерная
проверка входа портала/админки по IP хоста выполняется после создания учётки;
полная приёмка и рунбук относятся к следующим блокам.
